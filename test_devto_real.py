    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if any(indicator in real_article_data["title"].lower() for indicator in tech_indicators):
                ...
                all_skills = ["Technical Writing", "Content Marketing", "Developer Relations"] + derived_skills[:5]
                ...
                print(f"   ✅ Dynamic: impact_score = {impact_score}, complexity_score = {complexity_score}")
                print("   ❌ Hardcoded: skills = ['Technical Writing', 'Content Marketing', 'Developer Relations']")
        ...
        print("Start the service with: cd services/tech_doc_generator && python app/main.py")
...
def show_dashboard_delta_comparison():
    ...
    # Example data
    recent_achievements = [
        {"complexity_score": 85.0, "skills_demonstrated": ["Python", "AI", "Docker", "K8s"], "performance_improvement_pct": 15.0},
        {"complexity_score": 80.0, "skills_demonstrated": ["Python", "FastAPI", "PostgreSQL"], "performance_improvement_pct": 12.0},
        {"complexity_score": 75.0, "skills_demonstrated": ["JavaScript", "React", "Node.js"], "performance_improvement_pct": 8.0}
    ]
    
    older_achievements = [
        {"complexity_score": 70.0, "skills_demonstrated": ["Python", "Flask"], "performance_improvement_pct": 5.0},
        {"complexity_score": 65.0, "skills_demonstrated": ["HTML", "CSS"], "performance_improvement_pct": 3.0}
    ]
    
    # Calculate current metrics
    current_complexity = sum(a["complexity_score"] for a in recent_achievements) / len(recent_achievements)
    old_complexity = sum(a["complexity_score"] for a in older_achievements) / len(older_achievements)
    complexity_delta = current_complexity - old_complexity
    
    current_skills = sum(len(a["skills_demonstrated"]) for a in recent_achievements) / len(recent_achievements)
    old_skills = sum(len(a["skills_demonstrated"]) for a in older_achievements) / len(older_achievements)
