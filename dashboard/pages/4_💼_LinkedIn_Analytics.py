...
# Page configuration
st.set_page_config(
    page_title="LinkedIn Analytics - Threads Agent",
    page_icon="💼",
    layout="wide"
)
...
if achievements:
    for achievement in achievements:
        ...
        if 'linkedin' in tags_str or 'linkedin' in title_desc or achievement.get('platform') == 'LinkedIn':
...
with col2:
    ...
    avg_engagement = sum(a.get('impact_score', 0) for a in linkedin_achievements) / len(linkedin_achievements) if linkedin_achievements else 0
...
with col3:
    ...
    total_impressions = sum(
        min(10000, max(1000, float(str(a.get('business_value', 0)).replace('$', '').replace(',', '')) / 10))
...
tab1, tab2, tab3, tab4 = st.tabs(["Engagement Trends", "Content Types", "Best Times", "Top Posts"])
...
with tab2:
    ...
    if linkedin_achievements:
        ...
        for achievement in linkedin_achievements:
            ...
            type_engagement[content_type].append(achievement.get('impact_score', 0) / 10)
        ...
        if type_data:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name='Posts', x=df_types['Content Type'], y=df_types['Posts']))
            fig.add_trace(go.Bar(name='Avg Engagement %', x=df_types['Content Type'], 
                               y=df_types['Avg Engagement %'], yaxis='y2'))
...
with tab4:
    ...
    if linkedin_achievements:
        ...
        top_posts = sorted(linkedin_achievements, key=lambda x: x.get('impact_score', 0), reverse=True)[:5]
        ...
        for idx, post in enumerate(top_posts, 1):
            ...
            with col2:
                ...
                impressions = min(5000, max(500, float(str(post.get('business_value', 0)).replace('$', '').replace(',', '')) / 20))
...
with col1:
    ...
    fig = px.line(df_growth, x='Date', y='Followers', 
                  title="LinkedIn Follower Growth (30 days)")
...
with col2:
    ...
    insights = [
        {"Insight": "Tutorial posts", "Impact": "85%", "Action": "Create more how-to content"},
        {"Insight": "Morning posts", "Impact": "72%", "Action": "Schedule 8-10 AM"},
        {"Insight": "Career updates", "Impact": "68%", "Action": "Share monthly progress"},
