...
with col3:
    achievement_type = st.multiselect(
        "Type",
        ["AI/ML", "Performance", "Infrastructure", "Feature", "Bug Fix", "Documentation", "Testing"],
        default=[]
    )
...
def get_filtered_achievements(days: int, min_value: float) -> List[Dict]:
    ...
    try:
        ...
        # Apply type filter
        if achievement_type:
            achievements = [a for a in achievements if a.get('category') in achievement_type]
...
this_week = [a for a in achievements if 'created_at' in a and pd.to_datetime(a['created_at']) > week_ago]
...
with col1:
    st.metric("Total Achievements", f"{total_achievements}", f"+{len(this_week)} this week")
with col2:
    st.metric("Total Business Value", f"${total_value/1000:.0f}K", f"+${this_week_value/1000:.0f}K")
with col3:
    st.metric("Published Articles", f"{published_count}", f"+{this_week_published}")
with col4:
    st.metric("Avg Value/Achievement", f"${avg_value/1000:.1f}K", f"+${(this_week_value/len(this_week) if this_week else 0)/1000:.1f}K")
...
if achievements:
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "Type": st.column_config.SelectboxColumn(
                "Type",
                options=["AI/ML", "Performance", "Infrastructure", "Feature", "Bug Fix", "Documentation", "Testing"],
                required=True,
            ),
    ...
    st.info("No achievements found matching the current filters. Try adjusting your filters or add new achievements below.")
...
with tab1:
    if achievements:
        ...
        values = [a.get('parsed_value', 0) for a in achievements if a.get('parsed_value', 0) > 0]
        if values:
            fig = px.histogram(
                x=values,
                nbins=20,
...
with tab2:
    if achievements:
        ...
        type_counts = pd.Series([a.get('category', 'Unknown') for a in achievements]).value_counts()
...
with tab3:
    if achievements:
        ...
        if 'created_at' in timeline_df.columns:
            ...
            # Group by month
            monthly = timeline_df.groupby(pd.Grouper(key='created_at', freq='ME')).agg({
                'parsed_value': 'sum',
                'cumulative_value': 'last'
            }).reset_index()
...
with st.form("new_achievement"):
    ...
    with col1:
        ...
        achievement_type = st.selectbox("Type", ["AI/ML", "Performance", "Infrastructure", "Feature", "Bug Fix", "Documentation", "Testing"])
        started_at = st.date_input("Start Date", value=datetime.now() - timedelta(days=7))
    ...
    with col2:
        ...
        tags = st.multiselect("Tags", ["Python", "Kubernetes", "AI", "Performance", "API", "Database", "Docker", "ML", "DevOps"])
    ...
    if submitted and title:
        ...
        if 'error' not in result:
            ...
            st.error(f"Failed to create achievement: {result.get('error', 'Unknown error')}")
        ...
        **Auto-detect from PRs**
        ...
        if st.button("🔍 Scan Recent PRs"):
            with st.spinner("Analyzing GitHub activity..."):
                ...
                if pr_achievements:
                    st.success(f"Found {len(pr_achievements)} potential achievements from recent PRs!")
                else:
