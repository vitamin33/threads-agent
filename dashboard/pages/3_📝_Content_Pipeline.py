...
with col5:
    ...
    if isinstance(engagement_avg, list):
        engagement_avg = (
            sum(engagement_avg) / len(engagement_avg) if engagement_avg else 0.0
        )
...
with col1:
    with st.form("content_generation"):
        ...
        if source == "Recent Achievements":
            ...
            angle = st.selectbox(
                "Angle", ["Technical Deep Dive", "Tutorial", "Opinion", "Case Study"]
            )
        ...
        if generate_btn:
            # Import SSE tracking
            try:
                from services.content_sse_client import (
                    ContentGenerationTracker,
                    create_mock_sse_demo,
                )
            ...
            with st.spinner("🧠 AI is creating your content..."):
                ...
                if result.get("success"):
                    st.success(
                        f"✅ {result.get('message', 'Content generated successfully!')}"
                    )
                    ...
                    if task_id and result.get("status") == "queued":
                        st.info(
                            f"🔄 Content generation task queued in orchestrator (ID: {task_id})"
                        )

                        # Show link to drafts management
                        st.success(
                            "✅ Task queued successfully! Content will appear in drafts once generated."
                        )
                        ...
                        if sse_available:
                            ...
                            with st.expander(
                                "📊 Real-Time Generation Progress", expanded=True
                            ):
                    ...
                    # Show demo option
                    if sse_available and st.checkbox(
                        "🎬 Show demo of real-time progress tracking"
                    ):
                    ...
                    st.info(
                        "**Fallback Generated:** How I Reduced API Latency by 78% Using Smart Caching"
                    )
    ...
    if achievements:
        ...
        for achievement in achievements:
            ...
            for platform in platforms:
                ...
                platform_stats[platform]["total_impact"] += achievement.get(
                    "impact_score", 0
                )
        ...
        for platform, stats in platform_stats.items():
            avg_engagement = (
                (stats["total_impact"] / stats["posts"] / 10)
                if stats["posts"] > 0
                else 0
            )
        ...
        if platform_data:
            ...
            fig = go.Figure()
            fig.add_trace(
                go.Bar(name="Posts", x=platform_df["Platform"], y=platform_df["Posts"])
            )
            fig.add_trace(
                go.Bar(
                    name="Avg Engagement %",
                    x=platform_df["Platform"],
                    y=platform_df["Avg Engagement"],
                )
            )
...
with tab2:
    ...
    if achievements:
        ...
        for achievement in achievements:
            ...
            type_stats[content_type]["total_impact"] += achievement.get(
                "impact_score", 0
            )
        ...
        for content_type, stats in type_stats.items():
            avg_engagement = (
                (stats["total_impact"] / stats["count"] / 10)
                if stats["count"] > 0
                else 0
            )
            type_data.append(
                {"Type": content_type, "Engagement": round(avg_engagement, 1)}
            )
...
with tab3:
    ...
    if achievements:
        ...
        top_achievements = sorted(
            achievements, key=lambda x: x.get("impact_score", 0), reverse=True
        )[:5]
    ...
    queue_data = pd.DataFrame(
        {
            "Status": ["Draft"],
            "Title": [
                "No content in queue - add achievements to generate content ideas"
            ],
            "Platform": ["N/A"],
            "Schedule": ["N/A"],
            "Quality Score": [0.0],
