...
st.set_page_config(
    page_title="Achievement Analytics - Threads Agent",
    page_icon="🎯",
    layout="wide"
)
...
if stats:
    ...
    with col1:
        st.metric(
            "Total Achievements",
            f"{stats.get('total_achievements', 0):,}",
            delta=None
        )
...
if stats and stats.get('by_category'):
    ...
    with col2:
        ...
        if achievements:
            ...
            if 'category' in df_achievements.columns and 'business_value' in df_achievements.columns:
                value_by_category = df_achievements.groupby('category')['business_value'].sum().reset_index()
...
if achievements:
    achievement_titles = [f"{a.get('title', 'Unknown')} (ID: {a.get('id', 'N/A')})" for a in achievements]
        ...
        if achievement_id:
            try:
                ...
                with httpx.Client(timeout=5.0) as client:
                    response = client.get(f"http://localhost:8000/achievements/{achievement_id}/calculation-transparency")
                    if response.status_code == 200:
                        ...
                        st.warning("Calculation transparency data not available for this achievement")
            except Exception as e:
...
if achievements:
    ...
    if 'completed_at' in df.columns:
        ...
        tab1, tab2, tab3 = st.tabs(["Impact Over Time", "Value Generation", "Complexity Trends"])
        ...
        with tab1:
            ...
            if 'impact_score' in df.columns:
                ...
                fig.add_hline(y=70, line_dash="dash", line_color="green", annotation_text="High Impact Threshold")
                fig = apply_plotly_theme(fig)
                st.plotly_chart(fig, use_container_width=True)
...
try:
    ...
    if pr_achievements:
        ...
        with col2:
            ...
            total_prs = len(pr_df)
            high_impact_prs = len(pr_df[pr_df['impact_score'] > 70]) if 'impact_score' in pr_df.columns else 0
            avg_pr_value = pr_df['business_value'].mean() if 'business_value' in pr_df.columns else 0
        ...
        st.info("No PR achievements found. Start tracking your pull requests to see analytics here!")
