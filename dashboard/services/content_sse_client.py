...
class ContentGenerationSSE:
    ...
    def connect_to_task(
        self, task_id: str, progress_callback: Callable, completion_callback: Callable
    ):
        ...
        def stream_events():
            try:
                ...
                completion_callback(
                    {"success": False, "error": f"SSE connection error: {str(e)}"}
                )
        ...
        with container:
            ...
            if not state["complete"]:
                ...
                if progress > 0:
                    elapsed = (
                        datetime.now()
                        - st.session_state.get(f"start_time_{task_id}", datetime.now())
                    ).seconds
                ...
                if result["success"]:
                    ...
                    with col3:
                        st.metric(
                            "Quality Score", f"{content.get('quality_score', 0):.1f}"
                        )
...
def create_mock_sse_demo(container):
    ...
    progress_states = [
        ...
        {
            "progress": 25,
            "message": "Analyzing trending topics...",
            "stage": "fetch_trends",
        },
        {
            "progress": 40,
            "message": "Generating viral hook...",
            "stage": "generate_hook",
        },
        {
            "progress": 60,
            "message": "Writing main content...",
            "stage": "generate_body",
        },
        {
            "progress": 80,
            "message": "Optimizing for engagement...",
            "stage": "optimize",
        },
    ...
    with container.container():
        ...
        st.info(
            "🎣 **Hook**: Stop what you're doing! Here's how I cut our API response time by 78%..."
        )
        st.text_area(
            "Content",
