...
st.markdown(
    "**Real-time monitoring and control center for your AI-powered content automation system**"
)
...
try:
    ...
    try:
        ...
        total_skills = sum(
            [len(a.get("skills_demonstrated", [])) for a in achievements]
        )
        avg_skills_per_project = total_skills / len(achievements) if achievements else 0

        performance_improvements = [
            a.get("performance_improvement_pct", 0)
            for a in achievements
            if a.get("performance_improvement_pct", 0) > 0
        ]
        ...
        old_complexity_scores = [
            a.get("complexity_score", 0)
            for a in older_achievements
            if a.get("complexity_score", 0) > 0
        ]
        old_avg_complexity = (
            sum(old_complexity_scores) / len(old_complexity_scores)
            if old_complexity_scores
            else 0
        )

        old_total_skills = sum(
            [len(a.get("skills_demonstrated", [])) for a in older_achievements]
        )
        old_avg_skills = (
            old_total_skills / len(older_achievements) if older_achievements else 0
        )

        old_performance_improvements = [
            a.get("performance_improvement_pct", 0)
            for a in older_achievements
            if a.get("performance_improvement_pct", 0) > 0
        ]
        old_avg_performance = (
            sum(old_performance_improvements) / len(old_performance_improvements)
            if old_performance_improvements
            else 0
        )

        # Calculate deltas
        complexity_delta = (
            avg_complexity - old_avg_complexity if old_avg_complexity > 0 else 0
        )
        skills_delta = (
            avg_skills_per_project - old_avg_skills if old_avg_skills > 0 else 0
        )
        performance_delta = (
            avg_performance_gain - old_avg_performance if old_avg_performance > 0 else 0
        )
...
with col3:
    ...
    impact_scores = (
        [a.get("impact_score", 0) for a in achievements if a.get("impact_score", 0) > 0]
    ...
    old_impact_scores = (
        ...
        if "older_achievements" in locals()
        else []
    )
    old_avg_impact = (
        sum(old_impact_scores) / len(old_impact_scores) if old_impact_scores else 0
    )
...
with col4:
    ...
    ai_readiness = (
        (avg_complexity + avg_impact + (avg_skills_per_project * 5)) / 3
        if avg_complexity > 0
        else 0
    )
    old_ai_readiness = (
        (old_avg_complexity + old_avg_impact + (old_avg_skills * 5)) / 3
        if "old_avg_complexity" in locals() and old_avg_complexity > 0
        else 0
    )
...
with col1:
    ...
    if "achievements" in locals() and achievements:
        ...
        sorted_achievements = sorted(
            achievements, key=lambda x: x.get("created_at", "")
        )
        cumulative_value = 0
        dates = []
        values = []
        ...
        if dates and values:
            trend_df = pd.DataFrame({"Date": dates, "Business Value": values})
        else:
            # Fallback to empty data
            trend_df = pd.DataFrame({"Date": [datetime.now()], "Business Value": [0]})
    else:
        # No data available
        trend_df = pd.DataFrame({"Date": [datetime.now()], "Business Value": [0]})
            ...
            platform_data = pd.DataFrame(
                {
                    ...
                    "Engagement Rate": [0]
                    * len(platform_counts),  # No engagement data available
                }
            )
        else:
            # Empty data
            platform_data = pd.DataFrame(
                {"Platform": ["No Data"], "Articles": [0], "Engagement Rate": [0]}
            )
    else:
        # No data available
        platform_data = pd.DataFrame(
...
if "achievements" in locals() and achievements:
    ...
    recent_achs = sorted(
        achievements, key=lambda x: x.get("created_at", ""), reverse=True
    )[:5]
...
styled_df = recent_achievements.style.map(style_status, subset=["Status"]).format(
    {"Business Value": "${:,.0f}"}
)
