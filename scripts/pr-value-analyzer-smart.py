...
class SmartPRAnalyzer:
    ...
    def should_analyze(self, pr_number: int, force: bool = False) -> Tuple[bool, str]:
        ...
        if key in self.history:
            ...
            return (
                False,
                f"Already analyzed at {analysis_time} for SHA {current_sha[:7]}",
            )
    ...
    def analyze(self, pr_number: int, force: bool = False) -> Optional[Dict]:
        ...
        if not should_analyze and not force:
            ...
            if pr_info:
                ...
                if key in self.history:
                    print(
                        f"   Using cached analysis from {self.history[key].get('timestamp')}"
                    )
    ...
    def get_pr_analysis_summary(self, pr_number: int) -> Dict:
        ...
        summary = {"pr_number": pr_number, "analyses": [], "latest": None}
                ...
                summary["analyses"].append(
                    {
                        ...
                        "portfolio_value": data["analysis"]
                        .get("summary", {})
                        .get("portfolio_value", 0),
                    }
                )
...
def main():
    ...
    if show_summary:
        ...
        for analysis in summary["analyses"]:
            print(
                f"   - SHA {analysis['sha']}: ${analysis['portfolio_value']:,.0f} ({analysis['timestamp']})"
            )
