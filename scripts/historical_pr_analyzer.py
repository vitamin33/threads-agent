...
class HistoricalPRAnalyzer:
    ...
    ) -> Tuple[List[PRMetrics], List[BusinessValue]]:
    ...
    def export_results(
        self, pr_metrics: List[PRMetrics], business_values: List[BusinessValue]
    ):
        ...
        with open(csv_file, "w", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=list(asdict(business_values[0]).keys())
            )
            writer.writeheader()
    ...
    def _generate_summary(self, business_values: List[BusinessValue]) -> Dict:
        ...
        return {
            ...
            "total_productivity_hours": sum(
                bv.productivity_hours for bv in business_values
            ),
            "category_breakdown": category_breakdown,
            "high_confidence_prs": len(
                [bv for bv in business_values if bv.confidence_level == "high"]
            ),
            "top_value_prs": sorted(
