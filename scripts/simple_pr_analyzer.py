...
class SimplePRAnalyzer:
    def __init__(self):
        self.value_patterns = {
            'ai_ml': ['ai', 'ml', 'llm', 'model', 'neural', 'rag', 'embedding', 'gpt', 'claude'],
            'performance': ['performance', 'optimize', 'speed', 'latency', 'throughput', 'cache'],
            'infrastructure': ['kubernetes', 'docker', 'ci/cd', 'deployment', 'monitoring', 'k8s'],
    ...
    def _categorize_pr(self, pr_metrics: PRMetrics) -> str:
        ...
        for label in pr_metrics.labels:
            if any(pattern in label.lower() for pattern in self.value_patterns['ai_ml']):
                return 'ai_ml'
...
def analyze_repository(owner: str = "vitamin33", repo: str = "threads-agent"):
    ...
    avg_roi = sum(bv.roi_percent for bv in business_values) / len(business_values) if business_values else 0
    ...
    print(f"Status: {'✅ VALIDATED' if 200000 <= total_value <= 350000 else '⚠️ NEEDS ADJUSTMENT'}")
    ...
    for category, stats in sorted(categories.items(), key=lambda x: x[1]['value'], reverse=True):
        print(f"• {category.title()}: {stats['count']} PRs, ${stats['value']:,.0f} value")
    
    # Top 5 achievements
    top_achievements = sorted(business_values, key=lambda x: x.portfolio_value, reverse=True)[:5]
    ...
    for i, bv in enumerate(top_achievements, 1):
        ...
        print(f"   💰 ${bv.portfolio_value:,.0f} value | {bv.roi_percent:.0f}% ROI | {bv.value_category}")
