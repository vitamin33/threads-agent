...
class HistoricalPRIntegrator:
    ...
    def create_achievement_from_pr(self, pr_data: Dict, business_value: Dict) -> Optional[Achievement]:
        """Create achievement record from PR analysis data"""
        try:
            # Check if achievement already exists
            existing = self.db.query(Achievement).filter(
                Achievement.pr_number == str(pr_data['pr_number'])
            ).first()
            
            if existing:
                print(f"  ⚠️ PR #{pr_data['pr_number']} already exists, updating metrics...")
            ...
            achievement_data = AchievementCreate(
                ...
                impact_score=min(100, business_value['business_impact_score'] * 10),  # Scale to 0-100
                technical_details={
                ...
                completed_at=datetime.fromisoformat(pr_data.get('merged_at', pr_data['created_at']).replace('Z', '+00:00')),
                metadata_json={
                    'value_category': business_value['value_category'],
                    'confidence_level': business_value['confidence_level'],
                    'business_impact_score': business_value['business_impact_score'],
            ...
            print(f"  ✅ Created achievement for PR #{pr_data['pr_number']}: {pr_data['title'][:50]}...")
            return db_achievement
            
        except Exception as e:
            print(f"  ❌ Error creating achievement for PR #{pr_data['pr_number']}: {e}")
            self.db.rollback()
            return None
    ...
    def _extract_skills(self, pr_data: Dict, business_value: Dict) -> List[str]:
        ...
        if 'docker' in title_lower or 'kubernetes' in title_lower or 'k8s' in title_lower:
    ...
    def import_historical_prs(self):
        ...
        total_value = self.db.query(Achievement).filter(
            Achievement.metrics != None
        ).all()
        
        portfolio_sum = sum(
            a.metrics.get('portfolio_value', 0) 
            for a in total_value 
            if a.metrics
        )
        
        print(f"\n💰 Total Portfolio Value in Achievement Collector: ${portfolio_sum:,.2f}")
        print("✅ Integration complete! Your achievements now have business value metrics.")
