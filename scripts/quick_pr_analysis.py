...
def quick_portfolio_analysis():
    ...
    for category, stats in sorted(categories.items(), key=lambda x: x[1]['value'], reverse=True):
        ...
        print(f"• {category.upper()}: {stats['count']} PRs, ${stats['value']:,.0f} ({pct:.1f}%)")
    
    print("\n🏆 TOP AI JOB INTERVIEW ACHIEVEMENTS:")
    print("-" * 60)
    
    # Sort by value
    sorted_results = sorted(analysis_results, key=lambda x: x['portfolio_value'], reverse=True)
    
    for i, result in enumerate(sorted_results[:5], 1):
        print(f"{i}. PR #{result['pr_number']}: {result['title'][:45]}...")
        print(f"   💰 ${result['portfolio_value']:,.0f} value | {result['roi_percent']:.0f}% ROI | {result['category'].upper()}")
        print(f"   📈 ${result['cost_savings']:,.0f} cost savings | {result['productivity_hours']:.0f}h saved")
        print()
    ...
    summary = {
        ...
        'validation_status': 'validated' if 200000 <= projected_total <= 350000 else 'exceeds_target',
        'average_roi': avg_roi,
