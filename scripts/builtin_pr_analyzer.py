...
class BusinessValueAnalyzer:
    def __init__(self):
        self.value_patterns = {
            'ai_ml': ['ai', 'ml', 'llm', 'model', 'neural', 'rag', 'embedding', 'gpt', 'claude', 'openai', 'anthropic'],
            'performance': ['performance', 'optimize', 'speed', 'latency', 'throughput', 'cache', 'faster'],
            'infrastructure': ['kubernetes', 'docker', 'ci/cd', 'deployment', 'monitoring', 'k8s', 'helm', 'airflow'],
            'cost': ['cost', 'savings', 'reduce', 'efficiency', 'resource', 'finops', 'budget'],
            'feature': ['feature', 'implement', 'add', 'new', 'capability', 'endpoint', 'service'],
    ...
    def _categorize_pr(self, pr_metrics: PRMetrics) -> str:
        ...
        return max(category_scores, key=category_scores.get) if category_scores else 'feature'
...
def analyze_real_repository(owner: str = "vitamin33", repo: str = "threads-agent"):
    ...
    if not token:
        print("⚠️ No GITHUB_TOKEN found. Using unauthenticated requests (limited to 60/hour)")
    ...
    print(f"• High Confidence PRs: {len(high_confidence)} ({len(high_confidence)/len(business_values)*100:.1f}%)")
    print(f"• Total Cost Savings: ${sum(bv.cost_savings for bv in business_values):,.0f}")
    print(f"• Total Productivity Hours: {sum(bv.productivity_hours for bv in business_values):,.0f}")
    
    print(f"\n🏷️ VALUE BY CATEGORY:")
    for category, stats in sorted(categories.items(), key=lambda x: x[1]['value'], reverse=True):
        pct = (stats['value'] / total_value) * 100
        print(f"• {category.title()}: {stats['count']} PRs, ${stats['value']:,.0f} ({pct:.1f}%)")
    
    # Top achievements for AI job interviews
    top_achievements = sorted(business_values, key=lambda x: x.portfolio_value, reverse=True)[:10]
    ...
    for i, bv in enumerate(top_achievements, 1):
        ...
        print(f"    💰 ${bv.portfolio_value:,.0f} | {bv.roi_percent:.0f}% ROI | {bv.value_category} | {bv.confidence_level}")
        if i <= 5:  # Show details for top 5
            print(f"    📈 {bv.performance_improvement:.0f}% perf gain | {bv.productivity_hours:.0f}h saved | ${bv.cost_savings:,.0f} cost reduction")
        print()
    ...
    results = {
        ...
        'validation_status': 'validated' if 200000 <= total_value <= 350000 else 'needs_adjustment',
        ...
        'top_10_achievements': [
            {
                ...
                'title': next(pr.title for pr in pr_metrics_list if pr.pr_number == bv.pr_number),
