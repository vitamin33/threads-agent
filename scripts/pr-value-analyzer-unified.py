...
class UnifiedPRValueAnalyzer:
    ...
    def detect_pr_value_categories(self, pr_data: Dict) -> Dict[str, float]:
        ...
        business_keywords = ["roi", "revenue", "cost", "savings", "customer", "user growth", "conversion", "retention"]
        ...
        # Quality indicators
        quality_keywords = ["test", "coverage", "bug", "fix", "error", "crash", "stability", "reliability"]
        ...
        doc_files = [f for f in files if any(ext in f.get("path", "").lower() for ext in [".md", "readme", "docs/"])]
        if doc_files:
            doc_score += 0.4 + (len(doc_files) * 0.1)
        categories["documentation"] = min(doc_score, 1.0)
        
        # Infrastructure indicators
        infra_keywords = ["kubernetes", "k8s", "docker", "ci", "cd", "deployment", "infrastructure", "helm", "terraform"]
        infra_score = sum(1 for kw in infra_keywords if kw in title + body) * 0.2
        infra_files = [f for f in files if any(path in f.get("path", "").lower() 
        ...
        # Security indicators
        security_keywords = ["security", "vulnerability", "authentication", "authorization", "encryption", "cve", "patch"]
        security_score = sum(1 for kw in security_keywords if kw in title + body) * 0.25
        categories["security"] = min(security_score, 1.0)
        
        # User experience indicators
        ux_keywords = ["ui", "ux", "user experience", "usability", "accessibility", "frontend", "design"]
        ux_score = sum(1 for kw in ux_keywords if kw in title + body) * 0.2
        categories["user_experience"] = min(ux_score, 1.0)
        
        # Technical debt indicators
        debt_keywords = ["refactor", "cleanup", "technical debt", "legacy", "deprecate", "modernize"]
        ...
        innovation_keywords = ["novel", "new approach", "innovative", "breakthrough", "ai", "ml", "rag", "llm"]
    ...
    def extract_relevant_metrics(self, pr_body: str, categories: Dict[str, float]) -> Dict[str, Any]:
        ...
        if categories.get("performance", 0) > 0.3:
            ...
            # Latency
            latency_match = re.search(r"<(\d+)ms|(\d+)ms\s*latency", pr_body, re.IGNORECASE)
            if latency_match:
                metrics["performance"]["latency_ms"] = int(latency_match.group(1) or latency_match.group(2))
            
            # Throughput improvement
            throughput_match = re.search(r"(\d+)x\s*(?:faster|throughput|improvement)", pr_body, re.IGNORECASE)
            if throughput_match:
                metrics["performance"]["throughput_multiplier"] = int(throughput_match.group(1))
        
        # Business metrics (only if business-related)
        if categories.get("business", 0) > 0.3:
            # Cost savings
            cost_match = re.search(r"\$(\d+(?:,\d+)*(?:\.\d+)?)\s*(?:k|K)?\s*(?:savings|saved|reduction)", pr_body)
            if cost_match:
            ...
            # Revenue impact
            revenue_match = re.search(r"(\d+)%\s*(?:revenue|conversion)\s*(?:increase|improvement)", pr_body, re.IGNORECASE)
            if revenue_match:
                metrics["business"]["revenue_increase_percent"] = int(revenue_match.group(1))
        
        # Quality metrics (only if quality-related)
        if categories.get("quality", 0) > 0.3:
            # Test coverage
            coverage_match = re.search(r"(\d+)%\s*(?:test\s*)?coverage", pr_body, re.IGNORECASE)
            if coverage_match:
                metrics["quality"]["test_coverage"] = int(coverage_match.group(1))
            
            # Bug fixes
            bugs_match = re.search(r"(?:fixes?|resolved?)\s*(\d+)\s*bugs?", pr_body, re.IGNORECASE)
            if bugs_match:
                metrics["quality"]["bugs_fixed"] = int(bugs_match.group(1))
            
            # Test count
            tests_match = re.search(r"(?:added|created)\s*(\d+)\s*(?:unit\s*)?tests?", pr_body, re.IGNORECASE)
            if tests_match:
        ...
        if categories.get("documentation", 0) > 0.3:
            ...
            docs_match = re.search(r"(\d+)\s*(?:pages?|sections?|guides?)\s*(?:added|created|written)", pr_body, re.IGNORECASE)
            if docs_match:
                metrics["documentation"]["pages_added"] = int(docs_match.group(1))
            
            # API endpoints documented
            api_match = re.search(r"(\d+)\s*(?:api\s*)?endpoints?\s*documented", pr_body, re.IGNORECASE)
            if api_match:
                metrics["documentation"]["endpoints_documented"] = int(api_match.group(1))
        
        # Infrastructure metrics (only if infrastructure-related)
        if categories.get("infrastructure", 0) > 0.3:
            # Deployment time reduction
            deploy_match = re.search(r"(\d+)%\s*(?:faster|reduced)\s*deployment", pr_body, re.IGNORECASE)
            if deploy_match:
                metrics["infrastructure"]["deployment_time_reduction"] = int(deploy_match.group(1))
            
            # Availability improvement
            avail_match = re.search(r"(\d+\.?\d*)%\s*(?:availability|uptime)", pr_body, re.IGNORECASE)
            if avail_match:
                metrics["infrastructure"]["availability_percent"] = float(avail_match.group(1))
        
        return metrics

    def calculate_business_value(self, metrics: Dict[str, Any], categories: Dict[str, float]) -> Dict[str, Any]:
        ...
        if categories.get("business", 0) > 0.3 and metrics.get("business"):
            ...
            if "revenue_increase_percent" in biz:
                ...
                value["revenue_impact"] = 1000000 * (biz["revenue_increase_percent"] / 100)
        ...
        total_value = sum(v for k, v in value.items() if isinstance(v, (int, float)) and "percent" not in k)
        if total_value > 0:
            value["total_business_value"] = total_value
        
        return value

    def calculate_unified_score(self, categories: Dict[str, float], metrics: Dict[str, Any], 
                              business_value: Dict[str, Any]) -> Tuple[float, Dict[str, float]]:
        """
        ...
        if categories.get("performance", 0) > 0.3:
            ...
            scores["performance"] = (rps_score + latency_score) / 2 if rps_score or latency_score else 0
        ...
        if categories.get("documentation", 0) > 0.3:
            ...
            scores["documentation"] = max(pages_score, endpoints_score, categories["documentation"] * 8)
        ...
        if categories.get("infrastructure", 0) > 0.3:
            ...
            scores["infrastructure"] = max(deploy_score, avail_score, categories["infrastructure"] * 7)
        ...
        if categories.get("security", 0) > 0.3:
            scores["security"] = categories["security"] * 9  # High value for security fixes
        ...
        if scores:
            ...
            multi_category_bonus = min(2, len([s for s in scores.values() if s > 5]) * 0.5)
    ...
    def generate_achievement_tags(self, categories: Dict[str, float], metrics: Dict[str, Any]) -> List[str]:
    ...
    def analyze_code_changes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["gh", "pr", "view", self.pr_number, "--json", "files,additions,deletions"],
                capture_output=True,
                text=True,
            )
            ...
            if result.returncode == 0:
                ...
                return {
                    ...
                    "total_changes": data.get("additions", 0) + data.get("deletions", 0),
                }
            return {}
    ...
    def run_analysis(self):
        ...
        try:
            result = subprocess.run(
                ["gh", "pr", "view", self.pr_number, "--json", "title,body,labels,files,additions,deletions,author"],
                capture_output=True,
                text=True,
            )
            ...
            active_categories = [cat for cat, score in categories.items() if score > 0.3]
            ...
            self.metrics["documentation_metrics"] = extracted_metrics.get("documentation", {})
            self.metrics["infrastructure_metrics"] = extracted_metrics.get("infrastructure", {})
            
            # Calculate business value
            business_value = self.calculate_business_value(extracted_metrics, categories)
            ...
            self.metrics["kpis"] = {
                ...
                "best_category": max(category_scores.items(), key=lambda x: x[1])[0] if category_scores else "none",
    ...
    def print_summary(self):
        ...
        for cat, confidence in sorted(categories.items(), key=lambda x: x[1], reverse=True):
            if confidence > 0.3:
        ...
        if self.metrics.get("technical_metrics"):
            print("  • Performance metrics:", ", ".join(self.metrics["technical_metrics"].keys()))
        if self.metrics.get("business_metrics"):
            print("  • Business metrics:", ", ".join(self.metrics["business_metrics"].keys()))
        if self.metrics.get("quality_metrics"):
            print("  • Quality metrics:", ", ".join(self.metrics["quality_metrics"].keys()))
