...
async def analyze_threads_agent():
    ...
    try:
        ...
        infrastructure_prs = [bv for bv in business_values if bv.value_category == 'infrastructure']
        ...
        print(f"• AI/ML PRs: {len(ai_ml_prs)} (${sum(bv.portfolio_value for bv in ai_ml_prs):,.0f} value)")
        print(f"• Infrastructure PRs: {len(infrastructure_prs)} (${sum(bv.portfolio_value for bv in infrastructure_prs):,.0f} value)")
        ...
        for i, bv in enumerate(top_achievements, 1):
            ...
            print(f"   💰 Value: ${bv.portfolio_value:,.0f} | ROI: {bv.roi_percent:.0f}% | Category: {bv.value_category}")
