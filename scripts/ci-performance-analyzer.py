...
class CIPerformanceAnalyzer:
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    workflow,
                    "--json",
                    "databaseId,status,conclusion,createdAt,updatedAt,name",
                    "--limit",
                    str(limit),
                ],
                capture_output=True,
    ...
    def suggest_optimizations(self, metrics: Dict) -> List[str]:
        ...
        if metrics.get("avg_duration", 0) > 10:
            suggestions.append(
                "⏱️  Consider parallelizing jobs - average runtime > 10 minutes"
            )

        # Check success rate
        if metrics.get("success_rate", 100) < 90:
            suggestions.append(
                "🔧 Low success rate - add retry logic or fix flaky tests"
            )
        ...
        if "dev-ci" in metrics.get("workflow", ""):
            if metrics.get("avg_duration", 0) > 5:
                suggestions.append(
                    "🚀 dev-ci: Consider skipping k3d for non-infra changes"
                )
    ...
    def generate_report(self):
        ...
        for workflow in self.workflows:
            ...
            print(
                f"   • Range: {metrics['min_duration']} - {metrics['max_duration']} min"
            )
