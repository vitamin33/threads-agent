    ...
    try:
        ...
        with urllib.request.urlopen(request) as response:
            if response.status in [200, 201]:
                ...
                print(f"   Value: ${result.get('metrics', {}).get('portfolio_value', 0):,.0f}")
    ...
    try:
        ...
        if response.status == 200:
            ...
            if achievements:
                ...
                if achievements:
                    top = max(achievements, 
                             key=lambda x: x.get('metrics', {}).get('portfolio_value', 0))
                    print(f"\n📊 Top Achievement:")
                    print(f"   {top.get('title', 'N/A')}")
                    print(f"   Value: ${top.get('metrics', {}).get('portfolio_value', 0):,.0f}")
        ...
        with urllib.request.urlopen(request) as response:
            if response.status == 200:
                ...
                if 'metrics' in data:
                    ...
                    print(f"   Value Score: {metrics.get('kpis', {}).get('business_value_score', 0)}/10")
                else:
