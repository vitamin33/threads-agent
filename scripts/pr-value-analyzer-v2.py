...
class PRValueAnalyzerV2:
    ...
    def detect_pr_type(self, pr_data: Dict) -> str:
        ...
        if any(
            word in title + body for word in ["bug", "error", "crash", "broken", "fix"]
        ):
            return "bugfix"
        ...
        if all(
            ".md" in path or "README" in path or "docs/" in path
        ...
        if any(
            path.startswith(".github/") or "Dockerfile" in path or "ci/" in path
            for path in file_paths
        ):
            return "ci"
    ...
    def calculate_documentation_score(self, pr_data: Dict, pr_type: str) -> float:
        ...
        doc_files = [
            f
            for f in files
            if ".md" in f.get("path", "").lower()
            or "readme" in f.get("path", "").lower()
        ]
    ...
    def calculate_innovation_score(self, pr_data: Dict, pr_type: str) -> float:
        ...
        keyword_count = sum(
            1 for keyword in innovation_keywords if keyword in title + body
        )
    ...
    def print_summary(self):
        ...
        for metric, score in scores.items():
            ...
            print(
                f"  {metric.replace('_', ' ').title()}: {score:.1f}/10 (weight: {weight:.0%}) = {weighted:.1f}"
            )
