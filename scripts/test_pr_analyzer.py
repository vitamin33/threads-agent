...
def test_pr_analysis():
    ...
    print(f"Projected Full Portfolio: ${total_value * 20:,.2f}")  # Estimate for ~100 PRs
    print(f"Target Range: $200,000 - $350,000")
    print(f"Status: {'✅ LIKELY VALIDATED' if (total_value * 20) >= 200000 else '⚠️ NEEDS MORE DATA'}")
    ...
    sorted_achievements = sorted(business_values, key=lambda x: x.portfolio_value, reverse=True)
    for i, bv in enumerate(sorted_achievements, 1):
        ...
        print(f"   📈 {bv.performance_improvement:.0f}% performance gain | {bv.confidence_level} confidence")
        ...
        'achievements': [
            {
                ...
                'title': next(pr.title for pr in mock_prs if pr.pr_number == bv.pr_number),
