        ...
        try:
            ...
            for test in test_cases:
                ...
                if result.value_category == test['expected_category']:
                    ...
                    print_error(f"Wrong category: expected {test['expected_category']}, got {result.value_category}")
                    all_passed = False
                ...
                if test['min_roi'] <= result.roi_percent <= test['max_roi']:
                    ...
                    print_error(f"ROI out of range: {result.roi_percent}% (expected {test['min_roi']}-{test['max_roi']}%)")
                    all_passed = False
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if 'Portfolio Value' in output:
                    ...
                    print_info(f"Portfolio Value: ${metrics.get('portfolio_value', 0):,.2f}")
        ...
        try:
            ...
            if response.status == 200:
                ...
                for endpoint in endpoints:
                    try:
                        resp = urllib.request.urlopen(f"http://localhost:8001{endpoint}")
                        if resp.status == 200:
                            print_success(f"Endpoint available: {endpoint}")
                        else:
                            print_warning(f"Endpoint returned {resp.status}: {endpoint}")
                    except:
...
"""
    ...
    def run_all_tests(self):
        ...
        if success:
            self.test_results['passed'] += 1
            if mock_data:
                results['mock_portfolio_value'] = mock_data.get('total_portfolio_value', 0)
        ...
        if self.test_results['failed'] == 0:
            ...
            print_success(f"Portfolio Value: ${results.get('quick_portfolio_value', 0):,.2f}")
