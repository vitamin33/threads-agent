...
def parse_github_logs(log_content: str) -> Dict[str, Any]:
    ...
    for i, line in enumerate(lines):
        ...
        # Parse pod events
        if current_section == "fake_threads" and (
            "Warning" in line or "Normal" in line
        ):
        ...
        if any(
            ...
            for error_keyword in ["error:", "failed:", "timeout", "cannot"]
        ):
            analysis["errors"].append(
                {"line": line, "line_number": i, "section": current_section}
            )
...
def generate_findings(analysis: Dict[str, Any]) -> List[str]:
    ...
    if analysis["deployment_timeline"]:
        ...
        if len(timeline) > 10:  # Stuck for a while
            findings.append(
                f"🕐 Deployment stuck for {len(timeline)} checks - readiness probe failing consistently"
            )
    ...
    failed_health_checks = [
        h for h in analysis["health_check_attempts"] if not h["success"]
    ]
    if failed_health_checks:
        findings.append(
            f"🏥 {len(failed_health_checks)} health check failures detected"
        )
...
def classify_error(error_line: str) -> str:
    ...
    if "timeout" in error_line_lower:
        return "timeout"
    elif "connection" in error_line_lower and (
        "refused" in error_line_lower or "failed" in error_line_lower
    ):
        return "connection"
    elif "health" in error_line_lower:
        return "health_check"
    elif (
        "resource" in error_line_lower
        or "memory" in error_line_lower
        or "cpu" in error_line_lower
    ):
        return "resource"
...
def main():
    ...
    print(
        f"📊 Total deployment readiness checks: {report['summary']['total_deployment_checks']}"
    )
