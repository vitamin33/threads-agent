...
def test_comment_monitoring_e2e():
    ...
    test_comments = [
        {
            "text": "This is amazing! How can I get early access?",
            "author": "buyer_001"
        },
        {
            "text": "Wow, I'd love to learn more about this!",
            "author": "buyer_002"
        },
        {
    ...
    for comment_data in test_comments:
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"   ✅ Created comment: {created_comment['id']} by {created_comment['author']}")
            else:
    ...
    try:
        response = httpx.post(
            f"{orchestrator_url}/comment-monitoring/process/{post_id}",
            timeout=5
        )
    ...
    print(f"   - Purchase Intent Comments: ~{len([c for c in test_comments if 'DM' in c['text'] or 'access' in c['text'] or 'learn more' in c['text']])}")
    print(f"   - Pipeline Status: {'✅ OPERATIONAL' if len(created_comments) > 0 else '❌ FAILED'}")
    print("="*60)
