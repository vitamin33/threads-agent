...
async def track_devto_article(article_url: str, title: str):
    ...
    article_tags = [tag.strip() for tag in tags_input.split(",")] if tags_input else ["technical-writing"]
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(f"📊 Analytics URL: http://localhost:8001/manual-publish/devto/analytics/{tracking_id}")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(f"   📈 Engagement Rate: {analytics['current_metrics']['engagement_rate']}%")
                print(f"   ⏱️  Reading Time: {analytics['current_metrics']['reading_time']} min")
                print()
