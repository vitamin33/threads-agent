    ...
    if deploy['success']:
        ...
        for line in lines:
            if 'Events:' in line:
                ...
                print("Events:")
                continue
            if in_events and (line.strip().startswith('Type') or line.strip().startswith('Normal') or line.strip().startswith('Warning')):
                print(f"  {line}")
        ...
        try:
            ...
            for pod in pod_data.get('items', []):
                ...
                for container in container_statuses:
                    ...
                    print(f"  Container {name}: ready={ready}, state={list(state.keys())}")
                    ...
                    if 'terminated' in state:
                        ...
                        print(f"    Terminated: {reason} (exit {exit_code}) - {message}")
...
def simulate_ci_conditions():
    ...
    limits_cmd = "kubectl describe limitrange 2>/dev/null || echo 'No resource limits set'"
