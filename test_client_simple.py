...
async def test_client():
    ...
    async with client:
        ...
        if achievement:
            print("✅ Successfully fetched achievement!")
        ...
        response = await client.list_achievements(
            portfolio_ready_only=True, min_impact_score=80, page_size=10
        )

        print(f"✅ Found {response.total} portfolio-ready achievements")
        print(
            f"   Showing page {response.page} with {len(response.achievements)} items"
        )
