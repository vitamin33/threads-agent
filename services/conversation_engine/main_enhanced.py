...
from services.conversation_engine.personalization import (
...
async def process_message(conversation_id: str, request: MessageRequest):
    ...
    context = ConversationContext(
        ...
        created_at=datetime.fromisoformat(
            memory.metadata.get("created_at", datetime.utcnow().isoformat())
        ),
    ...
    personalized_message = await personalization_engine.generate_response(
    ...
    if (
        state_response.suggested_next_state
        and not state_response.requires_human_handoff
    ):
    ...
    redis_client.publish(f"conversation:{conversation_id}", personalized_message)
...
@app.get(
    "/conversations/{conversation_id}/analytics", response_model=ConversationAnalytics
)
async def get_conversation_analytics(conversation_id: str):
    ...
    return ConversationAnalytics(conversation_id=conversation_id, **analytics)
...
async def handoff_to_human(conversation_id: str):
    ...
    redis_client.publish("human_handoff", f"{conversation_id}:{memory.user_id}")
...
async def process_message_async(
    conversation_id: str, message: str, metadata: Dict[str, Any]
):
