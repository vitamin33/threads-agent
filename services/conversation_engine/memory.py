...
class ConversationMemorySystem:
    ...
    def __init__(self):
        # PostgreSQL setup
        self.db_url = os.getenv("DATABASE_URL", "postgresql://user:pass@localhost/conversations")
    ...
    async def get_conversation(self, conversation_id: str) -> Optional[ConversationMemory]:
        ...
        with self.SessionLocal() as session:
            db_conv = session.query(ConversationStateDB).filter_by(
            ...
            turns = session.query(ConversationTurnDB).filter_by(
                conversation_id=conversation_id
            ).order_by(ConversationTurnDB.turn_number).all()
    ...
    async def add_conversation_turn(
        ...
        with self.SessionLocal() as session:
            ...
            max_turn = session.query(ConversationTurnDB).filter_by(
                conversation_id=conversation_id
            ).count()
    ...
    async def get_active_conversations(self, hours: int = 24) -> List[str]:
        ...
        with self.SessionLocal() as session:
            active = session.query(ConversationStateDB.conversation_id).filter(
                ConversationStateDB.last_interaction >= cutoff
            ).all()
    ...
    async def get_user_conversations(self, user_id: str) -> List[ConversationMemory]:
        ...
        with self.SessionLocal() as session:
            convs = session.query(ConversationStateDB).filter_by(
                user_id=user_id
            ).order_by(ConversationStateDB.last_interaction.desc()).all()
    ...
    async def _get_from_cache(self, conversation_id: str) -> Optional[ConversationMemory]:
    ...
    def analyze_conversation_patterns(self, conversation_id: str) -> Dict[str, Any]:
        ...
        with self.SessionLocal() as session:
            ...
            avg_response_time = sum(t.response_time_ms or 0 for t in turns) / total_turns
