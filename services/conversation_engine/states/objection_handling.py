...
class ObjectionHandlingHandler(BaseStateHandler):
    ...
    async def handle(self, context: ConversationContext) -> StateResponse:
        ...
        objection_type = context.metadata.get("objections_raised") or analysis["objection_type"]
        ...
        if analysis["objection_resolved"]:
            ...
            suggested_state = "objection_handling" if not analysis["softening"] else "price_negotiation"
            confidence = 0.8
        else:
    ...
    def analyze_message(self, message: str, context: ConversationContext) -> Dict[str, Any]:
        ...
        resolution_keywords = [
            "makes sense", "i see", "got it", "understand", "fair enough",
            "good point", "okay", "alright", "that helps"
        ]
        
        # Softening indicators
        softening_keywords = [
            "but", "however", "although", "maybe", "possibly",
            "might consider", "could work", "interesting"
        ]
        
        # Strong objection indicators
        strong_objection_keywords = [
            "definitely not", "no way", "never", "absolutely not",
            "not interested", "waste of", "don't believe"
        ]
    ...
    def _handle_specific_objection(self, objection_type: str, params: Dict[str, Any], analysis: Dict[str, Any]) -> str:
    ...
    def _handle_price_objection(self, params: Dict[str, Any], analysis: Dict[str, Any]) -> str:
    ...
    def _handle_complex_objection(self, objections: list, params: Dict[str, Any]) -> str:
