...
class AgileBusinessValueCalculator:
    ...
    def extract_business_value(self, pr_description: str, pr_metrics: Dict = None) -> Optional[Dict]:
    ...
    def _extract_explicit_value(self, description: str, metrics: Dict) -> Optional[Dict]:
    ...
    def _extract_performance_improvements(self, description: str, metrics: Dict) -> Optional[Dict]:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                monthly_savings = monthly_infra_cost * (percentage / 100) * 0.5  # 50% correlation
    ...
    def _extract_automation_value(self, description: str, metrics: Dict) -> Optional[Dict]:
        ...
        if any(re.search(pattern, description, re.IGNORECASE) for pattern in automation_keywords):
            # Estimate based on PR size and typical automation benefits
            files_changed = metrics.get('changed_files', metrics.get('files_changed', 1))
    ...
    def _extract_quality_improvements(self, description: str, metrics: Dict) -> Optional[Dict]:
        ...
        for pattern in quality_patterns:
            ...
            if match:
                ...
                if 'coverage' in match.group(0).lower():
                    ...
                    defects_prevented = value * 0.1  # 10% coverage ≈ 1 defect prevented per month
                    annual_value = defects_prevented * 12 * self.config.defect_fix_cost
                else:
                    # Direct defect reduction
                    if '%' in match.group(0):
                        # Percentage reduction
                        baseline_defects_monthly = 5  # Assume 5 defects per month baseline
    ...
    def _extract_technical_debt_reduction(self, description: str, metrics: Dict) -> Optional[Dict]:
        """Extract technical debt reduction value."""
        debt_keywords = [
            'technical debt', 'refactor', 'clean up', 'moderniz', 'upgrad',
            'legacy', 'deprecated', 'maintenance', 'simplif'
        ]
        ...
        if any(keyword in description.lower() for keyword in debt_keywords):
            ...
            base_value = 2000
            complexity_multiplier = min(3.0, (lines_changed / 1000) + (files_changed / 10))
    ...
    def _extract_risk_mitigation(self, description: str, metrics: Dict) -> Optional[Dict]:
        ...
        for pattern in risk_patterns:
            if re.search(pattern, description, re.IGNORECASE):
                ...
                if re.search(r'critical|security|breach|vulnerability', description, re.IGNORECASE):
                    risk_value = self.config.critical_incident_cost
                    confidence = self.config.high_confidence
                elif re.search(r'compliance|regulation|audit', description, re.IGNORECASE):
                ...
                return {
                    ...
                    "breakdown": {
                        "risk_category": "security" if "security" in description.lower() else "operational",
