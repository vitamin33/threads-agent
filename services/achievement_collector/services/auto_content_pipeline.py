...
class ContentGenerationRequest(BaseModel):
    ...
    target_platforms: List[str] = Field(
        default_factory=lambda: ["devto", "linkedin", "medium"]
    )
...
class AutoContentPipeline:
    ...
    def should_generate_content(self, pr_data: Dict[str, Any]) -> bool:
        ...
        if files and all(
            f.get("filename", "").endswith((".md", ".txt", ".rst")) for f in files
        ):
            return False
    ...
    async def generate_content_with_cta(
        self, achievement_data: Dict[str, Any], request: ContentGenerationRequest
    ) -> ContentResult:
    ...
    async def add_utm_tracking(
        ...
        utm_params = {
            "utm_source": platform,
            "utm_medium": "social"
            if platform in ["linkedin", "twitter", "threads"]
            else "article",
        ...
        portfolio_link = f"{base_url}/portfolio?" + "&".join(
            f"{k}={v}" for k, v in utm_params.items()
        )
        contact_link = f"{base_url}/contact?" + "&".join(
            f"{k}={v}" for k, v in utm_params.items()
        )
        ...
        ).replace("serbyn.pro/contact", contact_link)

        return tracked_content

    async def generate_platform_content(
        self, achievement_data: Dict[str, Any], platform: str
    ) -> str:
    ...
    async def publish_to_all_platforms(
        self, request: PublishingRequest
    ) -> PublishingResult:
    ...
    async def generate_and_publish(self, pr_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        content_request = ContentGenerationRequest(
            achievement_id=pr_data.get("number", 0), utm_campaign="pr_automation"
        )

        content_result = await self.generate_content_with_cta(
            achievement_data, content_request
        )
    ...
    def _generate_authority_hook(self, achievement_data: Dict[str, Any]) -> str:
        ...
        if "$" in business_value:
            ...
            return (
                "After years of building production AI systems, here's what I learned:"
            )

    def _generate_business_focused_content(
        ...
        if technical_details:
            formatted_tech = "\n".join(
                f"• {k}: {v}" for k, v in technical_details.items()
            )
    ...
    def _generate_conversion_ctas(self, request: ContentGenerationRequest) -> str:
        ...
        if request.include_serbyn_pro_cta:
            ...
            contact_cta = (
                "💬 Let's discuss your AI infrastructure needs: serbyn.pro/contact"
            )
...
class AIPoweredPREvaluator:
    ...
    async def evaluate_technical_significance(
        self, pr_data: Dict[str, Any]
    ) -> TechnicalSignificanceScore:
    ...
    async def score_professional_positioning(
        ...
        cache_key = self._get_cache_key(
            pr_data, f"positioning_{target_role}_{target_location}"
        )
        if cache_key in self._cache:
            return self._cache[cache_key]
    ...
    async def predict_content_potential(
        self, pr_data: Dict[str, Any]
    ...
    async def calculate_marketing_value(
        ...
        cache_key = self._get_cache_key(
            pr_data, f"marketing_value_{target_role}_{target_location}"
        )
        if cache_key in self._cache:
            return self._cache[cache_key]
        ...
        positioning_score = await self.score_professional_positioning(
            pr_data, target_role, target_location
        )
        ...
        result = MarketingValueScore(
            ...
            content_potential_score=(
                content_score.engagement_score + content_score.conversion_score
            )
            / 2,
...
class ContentOptimizer:
    ...
    async def generate_engaging_hooks(
        self, achievement_type: str, business_impact: str
    ) -> List[str]:
...
class AIHiringManagerContentEngine:
    ...
    def optimize_for_hiring_managers(
        self, content: str, achievement_data: Dict[str, Any], target_company: str = None
    ) -> str:
        ...
        if target_company and target_company.lower() in self.company_targeting:
            optimized_content = self._apply_company_targeting(
                optimized_content, target_company.lower()
            )

        # Enhance authority positioning for hiring managers
        optimized_content = self._enhance_hiring_authority(
            optimized_content, achievement_data
        )
    ...
    def _add_hiring_manager_keywords(
        self, content: str, achievement_data: Dict[str, Any]
    ) -> str:
    ...
    def _apply_company_targeting(self, content: str, company: str) -> str:
        ...
        if cta_index != -1:
            content = (
                content[:cta_index] + company_section + "\n\n" + content[cta_index:]
            )
        else:
    ...
    def generate_hiring_manager_hook(self, achievement_data: Dict[str, Any]) -> str:
        ...
        if cost_match and performance_match:
            ...
            return self.hiring_hooks.get(
                category, "Here's how I solve complex AI infrastructure challenges:"
            )
...
class PlatformOptimizer:
    ...
    async def optimize_for_platform(
        ...
        if platform == "linkedin":
            ...
            content = "🎯 **Business Impact Showcase**\n\n"
            content += f"**Challenge:** {achievement_data.get('description', '')}\n\n"
            content += f"**Solution:** {achievement_data.get('title', '')}\n\n"
            content += (
                f"**Business Impact:** {achievement_data.get('business_value', '')}\n\n"
            )
            content += "**Leadership Approach:** Focused on measurable ROI and team scalability\n\n"
            ...
            content += (
                f"class {achievement_data.get('title', 'Solution').replace(' ', '')}:\n"
            )
...
class AIHiringManagerContentOptimizer:
    ...
    async def analyze_hiring_manager_personas(
        self, achievement_data: Dict[str, Any], target_companies: List[str]
    ...
    async def optimize_job_search_keywords(
        self, achievement_data: Dict[str, Any], target_roles: List[str]
        ...
        try:
            ...
            return KeywordOptimizationResult(
                ...
                keyword_analysis={
                    kw: 0.8 for kw in self.high_value_keywords["technical"][:5]
                },
                seo_score=75.0,
    ...
    async def optimize_hiring_manager_hooks(
        self, achievement_data: Dict[str, Any], target_audience: str
        ...
        try:
            ...
            for hook_data in result["optimized_hooks"]:
                optimized_hooks.append(
                    OptimizedHook(
                        content=hook_data["content"],
                        hiring_manager_appeal_score=hook_data[
    ...
    async def generate_company_specific_variants(
        ...
        for company in target_companies:
            ...
            try:
                ...
                company_variants[company_key] = CompanyVariant(
                    ...
                    key_messaging=[
                        f"Relevant to {company}'s mission",
                        "Technical excellence",
                    ],
                    value_alignment={
                        company_info.get("values", ["innovation"])[
                            0
                        ]: "Direct alignment through technical achievement"
                    },
                )

        # Calculate overall targeting score
        total_score = sum(
            variant.company_alignment_score for variant in company_variants.values()
        )
        ...
        return CompanyTargetingResult(
            ...
            targeting_insights={
                company: f"Optimized for {company}'s values"
                for company in target_companies
            },
        )
    ...
    async def optimize_hiring_manager_ctas(
        ...
        try:
            ...
            return CTAOptimizationResult(
                ...
                conversion_analysis={
                    "best_performing": "Direct value proposition",
    ...
    async def generate_complete_hiring_manager_optimized_content(
        ...
        keyword_optimization = await self.optimize_job_search_keywords(
            achievement_data, target_roles
        )
        hook_optimization = await self.optimize_hiring_manager_hooks(
            achievement_data, "AI/MLOps hiring managers"
        )
        company_targeting = await self.generate_company_specific_variants(
            achievement_data, target_companies
        )
        authority_optimization = await self.optimize_professional_authority(
            achievement_data, "technical_leadership"
        )
        cta_optimization = await self.optimize_hiring_manager_ctas(
            achievement_data, target_roles, target_companies
        )

        # Select best components
        best_hook = max(
            hook_optimization.optimized_hooks,
            key=lambda h: h.hiring_manager_appeal_score,
        )
        best_cta = max(
            cta_optimization.optimized_ctas, key=lambda c: c.conversion_score
        )
        ...
        overall_conversion_score = (
            ...
            + (best_hook.hiring_manager_appeal_score * 10)
            * 0.3  # Convert to 0-100 scale
