...
async def batch_get_achievements(
    request: BatchAchievementRequest, db: Session = Depends(get_db)
):
    """
    ...
    achievements = (
        ...
        .filter(AchievementModel.id.in_(request.achievement_ids))
        .all()
    )
...
async def get_company_targeted_achievements(
    ...
    if keyword_conditions:
        try:
            ...
            logger.warning(
                f"Keyword search failed for {company_name}, using category fallback"
            )
            pass

    # Order by impact score and limit
    achievements = (
        query.order_by(desc(AchievementModel.impact_score)).limit(limit).all()
    )
...
async def get_content_opportunities(db: Session = Depends(get_db)):
    """
    ...
    unprocessed = max(
        0, total_portfolio_ready - 5
    )  # Assume 5 have been processed as example
    ...
    return {
        ...
        "content_generation_rate": f"{((total_ready - unprocessed) / total_ready * 100):.1f}%"
        if total_ready > 0
        else "0%",
    }
