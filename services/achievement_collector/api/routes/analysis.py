...
async def analyze_achievement(
    ...
    achievement = db.query(Achievement).filter(
        Achievement.id == request.achievement_id
    ).first()
    ...
    try:
        ...
        logger.info(f"Analyzed achievement {achievement.id}: impact={analysis_result.impact_score}")
...
async def batch_analyze_achievements(
    ...
    query = db.query(Achievement).filter(
        Achievement.ai_summary.is_(None)
    )
...
async def get_insights(
    ...
    top_achievements = db.query(Achievement).filter(
