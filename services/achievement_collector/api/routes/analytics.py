...
async def analyze_skill_gaps(
    ...
    """
    try:
        analysis = await career_predictor.analyze_skill_gaps(db, target_role, user_id)
        return analysis
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_career_insights(
    ...
    try:
        ...
        top_percentile = max((b.percentile for b in benchmarks), default=50)
...
async def get_trending_skills():
    ...
    try:
        ...
        sorted_skills = sorted(market_data, key=lambda s: s.market_ratio, reverse=True)
