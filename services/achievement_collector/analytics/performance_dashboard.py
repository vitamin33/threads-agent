...
class PerformanceDashboard:
    ...
    def _calculate_career_velocity(self, achievements: List[Achievement]) -> float:
        ...
        first_rate = len(first_half) / max(
            (first_half[-1].completed_at - first_half[0].completed_at).days, 1
        )
        second_rate = len(second_half) / max(
            (second_half[-1].completed_at - second_half[0].completed_at).days, 1
        )
    ...
    def _generate_impact_progression(
        ...
        for a in achievements:
            if a.impact_score:
                ...
                result.append(
                    TimeSeriesMetric(
                        timestamp=a.completed_at, value=cumulative_impact, label=a.title
                    )
                )
    ...
    def _generate_skill_radar(
        ...
        for skill, scores in skill_scores.items():
            ...
            if len(scores) > 1:
                ...
                growth_trend = (second_half_avg - first_half_avg) / max(
                    first_half_avg, 1
                )
            else:
                growth_trend = 0
    ...
    def _generate_impact_heatmap(
        ...
        for a in achievements:
            ...
            if key not in heatmap_data:
                heatmap_data[key] = {"impact_total": 0, "count": 0}
    ...
    def _calculate_improvement_rate(self, achievements: List[Achievement]) -> float:
        ...
        first_avg_impact = (
            np.mean([a.impact_score for a in first_quarter if a.impact_score])
            if first_quarter
            else 0
        )

        last_avg_impact = (
            np.mean([a.impact_score for a in last_quarter if a.impact_score])
            if last_quarter
            else 0
        )
    ...
    def _calculate_momentum_score(self, achievements: List[Achievement]) -> float:
        ...
        impact_factor = (
            np.mean([a.impact_score for a in recent_achievements if a.impact_score])
            * 0.5
    ...
    def generate_executive_summary(
        ...
        avg_complexity = (
            np.mean([a.complexity_score for a in achievements if a.complexity_score])
            if achievements
            else 0
        )
    ...
    def _generate_executive_recommendations(
        ...
        if len(achievements) > 0:
            days_span = (
                achievements[0].completed_at - achievements[-1].completed_at
            ).days
