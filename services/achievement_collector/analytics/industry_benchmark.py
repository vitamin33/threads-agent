...
class IndustryBenchmarker:
    ...
    def _calculate_user_metrics(
        ...
        metrics = {
            ...
            'average_impact_score': np.mean([
                a.impact_score for a in achievements 
            ...
            'technical_depth': np.mean([
                a.complexity_score for a in achievements 
    ...
    def _benchmark_metric(
        ...
        industry_stats = self.industry_data.get(metric_name, {
            'average': 3.0,
            'median': 2.5,
            'percentiles': {
                10: 1.0,
    ...
    def _calculate_percentile(
        ...
        if value <= percentiles[10]:
            ...
            return 10 + int((value - percentiles[10]) / 
                           (percentiles[25] - percentiles[10]) * 15)
        elif value <= percentiles[50]:
            return 25 + int((value - percentiles[25]) / 
                           (percentiles[50] - percentiles[25]) * 25)
        elif value <= percentiles[75]:
            return 50 + int((value - percentiles[50]) / 
                           (percentiles[75] - percentiles[50]) * 25)
        elif value <= percentiles[90]:
            return 75 + int((value - percentiles[75]) / 
                           (percentiles[90] - percentiles[75]) * 15)
        else:
            return min(90 + int((value - percentiles[90]) / 
                              percentiles[90] * 10), 99)
    ...
    def _generate_recommendation(
        ...
        if percentile >= 90:
            ...
            return f"Great performance! Focus on consistency to reach the top 10%."
        elif percentile >= 50:
            return f"Above average. Increase frequency or impact to improve further."
        elif percentile >= 25:
    ...
    async def benchmark_compensation(
        ...
        skill_premium = sum(
            self._get_skill_premium(skill) for skill in skills
        ) / 100  # Convert to multiplier
        ...
        estimated_value = int(
            base_salary * (1 + skill_premium) * 
            location_multiplier * achievement_multiplier
        )
    ...
    def _calculate_salary_percentile(
        ...
        if salary <= market_data['low']:
            ...
            return 25 + int((salary - market_data['low']) / 
                           (market_data['median'] - market_data['low']) * 25)
        elif salary <= market_data['high']:
            return 50 + int((salary - market_data['median']) / 
                           (market_data['high'] - market_data['median']) * 40)
        else:
            return min(90 + int((salary - market_data['high']) / 
                              market_data['high'] * 10), 99)
