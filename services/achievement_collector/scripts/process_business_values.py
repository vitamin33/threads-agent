...
from services.achievement_collector.core.logging import setup_logging  # noqa: E402

logger = setup_logging(__name__)


async def process_achievements(
    limit: int = 50, days_back: int = 30, dry_run: bool = False, force_all: bool = False
):
    """
    ...
    try:
        ...
        for achievement in achievements:
            try:
                logger.info(
                    f"Processing achievement {achievement.id}: {achievement.title}"
                )
