...
def extract_business_values_task(limit: int = 20, days_back: int = 7) -> dict:
    """
    ...
    try:
        ...
        result = asyncio.run(_process_achievements_async(limit, days_back))
        return result
...
async def _process_achievements_async(limit: int, days_back: int) -> dict:
    ...
    try:
        ...
        achievements = (
            ...
            .filter(
                ...
                (Achievement.business_value.is_(None))
...
@shared_task(name="process_single_achievement", max_retries=2, default_retry_delay=60)
def process_single_achievement_task(achievement_id: int) -> Optional[str]:
    ...
    """
    try:
        return asyncio.run(_process_single_achievement_async(achievement_id))
    except Exception as e:
        logger.error(f"Error processing achievement {achievement_id}: {e}")
        raise process_single_achievement_task.retry(exc=e)


async def _process_single_achievement_async(achievement_id: int) -> Optional[str]:
