...
class ResumeGenerator(BaseExporter):
    ...
    def _create_header(self, user_info: Optional[Dict[str, str]]) -> List:
        ...
        # Name
        story.append(Paragraph(user_info.get('name', 'Professional'), self.styles['CustomTitle']))
    ...
    def _create_skills_section(self, achievements: List) -> List:
        ...
        if skill_counts:
            ...
            if proficient_skills:
                skill_text += f"<b>Proficient:</b> {', '.join(proficient_skills[:8])}<br/>"
            if familiar_skills:
                skill_text += f"<b>Familiar:</b> {', '.join(familiar_skills[:8])}"
                
            story.append(Paragraph(skill_text, self.styles['CustomBody']))
        else:
            story.append(Paragraph("Various technical skills demonstrated", self.styles['CustomBody']))
    ...
    def _create_achievements_section(self, achievements: List) -> List:
        ...
        for achievement in sorted_achievements:
            ...
            # Skills used
            if achievement.skills_demonstrated:
                skills_text = f"<i>Skills: {', '.join(achievement.skills_demonstrated[:5])}</i>"
    ...
    def _create_metrics_section(self, achievements: List) -> List:
        ...
        avg_complexity = sum(a.complexity_score or 0 for a in achievements) / len(achievements) if achievements else 0
