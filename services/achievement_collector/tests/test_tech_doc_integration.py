...
class TestTechDocIntegrationEndpoints:
    ...
    def test_batch_get_achievements(
        self, mock_get_db, client, mock_db, sample_achievements
    ):
        """Test batch get achievements endpoint"""
        # Setup mock
        mock_get_db.return_value = mock_db
        mock_db.query.return_value.filter.return_value.all.return_value = (
            sample_achievements[:2]
        )

        response = client.post(
            "/tech-doc-integration/batch-get", json={"achievement_ids": [1, 2]}
        )
    ...
    @patch("services.achievement_collector.api.routes.tech_doc_integration.get_db")
    def test_content_ready_achievements(
        self, mock_get_db, client, mock_db, sample_achievements
    ):
    ...
    def test_batch_get_empty_ids(self, client):
        ...
        response = client.post(
            "/tech-doc-integration/batch-get", json={"achievement_ids": []}
        )
    ...
    def test_sync_status_achievement_not_found(self, mock_get_db, client, mock_db):
        ...
        response = client.post(
            "/tech-doc-integration/sync-status", params={"achievement_id": 999}
        )
...
class TestValidation:
    ...
    def test_batch_get_request_validation(self, client):
        ...
        # Test invalid JSON
        response = client.post(
            "/tech-doc-integration/batch-get", json={"invalid_field": [1, 2, 3]}
        )
