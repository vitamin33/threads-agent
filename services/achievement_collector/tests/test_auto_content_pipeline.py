...
class TestAutoContentPipeline:
    ...
    async def test_content_includes_serbyn_pro_cta_with_utm_tracking(
        self, mock_auto_pipeline
    ):
        ...
        assert (
            "utm_source=devto" in result.content
            or "utm_source=linkedin" in result.content
        )
        assert "utm_campaign=pr_automation" in result.content
        assert "Currently seeking remote US AI/MLOps roles" in result.content

    async def test_publishes_to_all_six_platforms_automatically(
        self, mock_auto_pipeline
    ):
    ...
    async def test_filters_prs_that_should_not_generate_content(
        self, mock_auto_pipeline
    ):
    ...
    async def test_content_adapts_tone_for_different_platforms(
        self, mock_auto_pipeline
    ):
    ...
    async def test_utm_tracking_parameters_are_unique_per_platform(
        self, mock_auto_pipeline
    ):
    ...
    async def test_content_includes_call_to_action_for_hiring(self, mock_auto_pipeline):
        ...
        found_indicators = [
            indicator for indicator in hiring_indicators if indicator in content_lower
        ]
...
class TestContentQualityAndConversion:
    ...
    async def test_content_follows_proven_viral_patterns(self):
        ...
        assert any(
            pattern in hook.lower() for hook in hooks for pattern in hook_patterns
        )
...
class TestAIPoweredPREvaluation:
    ...
    def business_impact_pr(self):
        ...
        return {
            "pull_request": {
                ...
                "files": [{"filename": "src/cache/invalidation.py", "changes": 85}],
            }
        }
    ...
    def trivial_pr(self):
        ...
        return {
            "pull_request": {
                ...
                "labels": [{"name": "typo"}],
                "files": [{"filename": "src/config.py", "changes": 1}],
            }
        }

    async def test_ai_significance_detector_identifies_complex_technical_achievement(
        self, complex_technical_pr
    ):
        ...
        with patch.object(
            evaluator.openai_client.chat.completions, "create", side_effect=mock_create
        ):
            # AI should detect this as technically significant despite lack of obvious keywords
            significance_score = await evaluator.evaluate_technical_significance(
                complex_technical_pr
            )
    ...
    async def test_ai_significance_detector_rejects_trivial_changes(self, trivial_pr):
        ...
        with patch.object(
            ...
            assert (
                significance_score.technical_merits is None
                or len(significance_score.technical_merits) == 0
            )

    async def test_ai_business_impact_analyzer_detects_hidden_value(
        self, business_impact_pr
    ):
        ...
        found_benefits = [
            benefit for benefit in expected_benefits if benefit in reasoning_lower
        ]
    ...
    async def test_ai_business_impact_analyzer_identifies_cost_savings(self):
        ...
        performance_pr = {
            "pull_request": {
                ...
                "files": [{"filename": "src/api/optimization.py", "changes": 200}],
            }
        }
        ...
        assert (
            "cost" in business_impact.reasoning.lower()
            or "savings" in business_impact.reasoning.lower()
        )

        # Should quantify impact
        assert business_impact.potential_savings is not None
        assert (
            "$" in business_impact.potential_savings
            or "%" in business_impact.potential_savings
        )
    ...
    async def test_ai_professional_positioning_scorer_for_mlops_roles(self):
        ...
        positioning_score = await evaluator.score_professional_positioning(
            mlops_pr, target_role="MLOps Engineer", target_location="Remote US"
        )
        ...
        skills_found = [
            ...
            if skill in positioning_score.relevant_skills
        ]
    ...
    async def test_ai_professional_positioning_scorer_rejects_irrelevant_work(self):
        ...
        frontend_pr = {
            "pull_request": {
                ...
                "files": [{"filename": "src/components/Button.css", "changes": 20}],
            }
        }

        positioning_score = await evaluator.score_professional_positioning(
            frontend_pr, target_role="MLOps Engineer", target_location="Remote US"
        )
        ...
        assert (
            "focus" in positioning_score.positioning_advice.lower()
            or "consider" in positioning_score.positioning_advice.lower()
        )
    ...
    async def test_ai_content_potential_predictor_identifies_viral_elements(self):
        ...
        content_potential = await evaluator.predict_content_potential(
            viral_potential_pr
        )
        ...
        assert (
            "engineers" in content_potential.target_audience.lower()
            or "developers" in content_potential.target_audience.lower()
        )
    ...
    async def test_ai_content_potential_predictor_rejects_low_engagement_content(self):
        ...
        boring_pr = {
            "pull_request": {
                ...
                "labels": [{"name": "refactor"}],
                "files": [{"filename": "src/utils/helper.py", "changes": 15}],
            }
        }
        ...
        assert content_potential.recommendation in [
            "skip",
            "enhance",
            "combine_with_other_work",
        ]
    ...
    async def test_ai_marketing_value_calculator_combines_all_scores(self):
        ...
        marketing_value = await evaluator.calculate_marketing_value(
            high_value_pr, target_role="MLOps Engineer", target_location="Remote US"
        )
    ...
    async def test_ai_evaluation_caching_avoids_duplicate_api_calls(self):
        ...
        with patch.object(
            evaluator.openai_client.chat.completions, "create"
        ) as mock_openai:
            mock_response = AsyncMock()
            mock_response.choices = [
                AsyncMock(
                    message=AsyncMock(
                        content='{"score": 7.5, "reasoning": "Good caching implementation", "technical_merits": ["caching"]}'
                    )
                )
            ]
    ...
    async def test_ai_powered_should_generate_content_replaces_keyword_matching(
        self, complex_technical_pr, trivial_pr
    ):
        ...
        should_generate_complex = await pipeline.should_generate_content_ai_powered(
            complex_technical_pr
        )
        assert should_generate_complex is True

        # Trivial PR should be rejected by AI
        should_generate_trivial = await pipeline.should_generate_content_ai_powered(
            trivial_pr
        )
...
class TestAIHiringManagerContentOptimization:
    ...
    async def test_hiring_manager_persona_analysis_returns_persona_insights(
        self, sample_achievement_data
    ):
        ...
        assert (
            anthropic_persona.appeal_score >= 0.0
            and anthropic_persona.appeal_score <= 10.0
        )
        ...
        assert (
            "ai safety" in str(anthropic_persona.key_interests).lower()
            or "responsible ai" in str(anthropic_persona.key_interests).lower()
        )

    async def test_ai_hiring_keyword_optimization_for_job_search_seo(
        self, sample_achievement_data
    ):
        ...
        assert len(keyword_optimization.optimized_content) > len(
            sample_achievement_data.get("description", "")
        )
        ...
        assert (
            keyword_optimization.seo_score >= 0.0
            and keyword_optimization.seo_score <= 100.0
        )

    async def test_hiring_manager_content_hooks_optimization(
        self, sample_achievement_data
    ):
    ...
    async def test_company_specific_content_targeting(self, sample_achievement_data):
        ...
        assert (
            "responsible" in anthropic_variant.content.lower()
            or "safety" in anthropic_variant.content.lower()
        )
        assert anthropic_variant.company_alignment_score >= 8.0

        notion_variant = company_targeting.company_variants["notion"]
        assert (
            "productivity" in notion_variant.content.lower()
            or "collaboration" in notion_variant.content.lower()
        )

        stripe_variant = company_targeting.company_variants["stripe"]
        assert (
            "scale" in stripe_variant.content.lower()
            or "reliability" in stripe_variant.content.lower()
        )

        # Should provide targeting effectiveness metrics
        assert company_targeting.overall_targeting_score >= 8.0

    async def test_professional_authority_building_optimization(
        self, sample_achievement_data
    ):
        ...
        found_elements = [
            element for element in authority_elements if element in content_lower
        ]
        ...
        found_achievements = [
            achievement
            for achievement in impressive_achievements
            if achievement in authority_optimization.authority_enhanced_content
        ]
        assert len(found_achievements) >= 3

    async def test_hiring_manager_specific_cta_optimization(
        self, sample_achievement_data
    ):
        ...
        for cta in cta_optimization.optimized_ctas:
            ...
            found_elements = [
                element for element in conversion_elements if element in cta_lower
            ]
            assert len(found_elements) >= 2  # Should have conversion language

        # Should include professional portfolio links with UTM tracking
        best_cta = max(
            cta_optimization.optimized_ctas, key=lambda x: x.conversion_score
        )
        assert "serbyn.pro" in best_cta.content
        assert "utm_" in best_cta.content or "tracking" in str(
            vars(best_cta)
        )  # UTM tracking

    async def test_integrated_hiring_manager_content_pipeline(
        self, sample_achievement_data
    ):
        ...
        complete_optimization = (
            await optimizer.generate_complete_hiring_manager_optimized_content(
                achievement_data=sample_achievement_data,
                target_companies=["Anthropic", "Notion", "Stripe"],
                target_roles=["MLOps Engineer", "AI Platform Engineer"],
        ...
        assert (
            complete_optimization.overall_conversion_score >= 95.0
        ...
        found_authority = [
            signal for signal in authority_signals if signal in content_lower
        ]
        ...
        found_metrics = [
            ...
            if metric in complete_optimization.optimized_content
        ]
        assert len(found_metrics) >= 2

        # Professional CTA
        assert "serbyn.pro" in complete_optimization.optimized_content
        assert "utm_" in str(vars(complete_optimization)) or "tracking" in str(
            vars(complete_optimization)
        )
    ...
    async def test_ai_hiring_manager_optimizer_integration_with_existing_pipeline(self):
        ...
        result = await pipeline.generate_content_with_cta(
            achievement_data, enhanced_request
        )
        ...
        found_companies = [
            indicator for indicator in company_indicators if indicator in content_lower
        ]
        ...
        found_authority = [
            enhancement
            for enhancement in authority_enhancements
            if enhancement in content_lower
        ]
        assert len(found_authority) >= 2

        # Should maintain backward compatibility
        basic_request = ContentGenerationRequest(
            achievement_id=124, enable_hiring_manager_optimization=False
        )

        basic_result = await pipeline.generate_content_with_cta(
            achievement_data, basic_request
        )
        assert (
            basic_result.conversion_score < result.conversion_score
        )  # Hiring manager optimization should be better
