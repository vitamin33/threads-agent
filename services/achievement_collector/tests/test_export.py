...
class TestResumeGenerator:
    ...
    async def test_resume_generation(self, db_session, export_achievements, tmp_path):
        ...
        # Mock AI analyzer
        with patch.object(
            generator.ai_analyzer, "generate_professional_summary"
        ) as mock_summary:
...
class TestPortfolioGenerator:
    ...
    async def test_portfolio_generation(
        self, db_session, export_achievements, tmp_path
    ):
        """Test portfolio PDF generation."""
...
class TestLinkedInIntegration:
    ...
    async def test_achievement_post_generation(self, db_session, export_achievements):
        ...
        # Mock AI analyzer
        with patch.object(
            integration.ai_analyzer, "generate_linkedin_content"
        ) as mock_content:
...
class TestWebPortfolioGenerator:
    ...
    @pytest.mark.asyncio
    async def test_web_portfolio_generation(
        self, db_session, export_achievements, tmp_path
    ):
        """Test web portfolio generation."""
