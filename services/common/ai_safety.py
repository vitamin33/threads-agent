...
class AISecurityMonitor:
    ...
    def check_prompt_injection(
        self, user_input: str, service: str = "unknown"
    ) -> Dict[str, Any]:
        """
        ...
        for pattern, category, severity in self.prompt_injection_patterns:
            if re.search(pattern, lower_input, re.IGNORECASE):
                detected_patterns.append(
                    {"pattern": pattern, "category": category, "severity": severity}
                )
        ...
        if detected_patterns:
            ...
            max_severity = max(
                detected_patterns,
                key=lambda x: {"low": 1, "medium": 2, "high": 3, "critical": 4}.get(
                    x["severity"], 0
                ),
            )

            # Emit Prometheus metrics
            AI_PROMPT_INJECTIONS.labels(
                service=service, severity=max_severity["severity"]
            ).inc()
            AI_SECURITY_INCIDENTS.labels(
                service=service, incident_type="prompt_injection"
            ).inc()
    ...
    def flag_potential_hallucination(
        self, ai_output: str, model: str = "unknown", service: str = "unknown"
    ) -> Dict[str, Any]:
        """
        ...
        for category, config in self.hallucination_keywords.items():
            ...
            if category_matches:
                ...
                if risk_scores.get(config["risk_level"], 0) > risk_scores.get(
                    max_risk_level, 0
                ):
        ...
        if flags:
            ...
            # Emit Prometheus metrics
            AI_HALLUCINATION_FLAGS.labels(
                model=model, service=service, risk_level=max_risk_level
            ).inc()
        ...
        return {
            ...
            "recommendation": self._get_hallucination_recommendation(
                max_risk_level, flags
            ),
            "timestamp": datetime.utcnow().isoformat(),
        }
    ...
    def check_content_safety(self, content: str) -> Dict[str, Any]:
        ...
        for pattern, category, severity in unsafe_patterns:
            if re.search(pattern, content, re.IGNORECASE):
                violations.append({"category": category, "severity": severity})
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hallucination_flag_rate": self.security_events["hallucination_flags"]
            / total,
            "content_violation_rate": self.security_events["content_violations"]
            / total,
            "security_events": dict(self.security_events),
        }

    def _get_hallucination_recommendation(
        self, risk_level: str, flags: List[Dict]
    ) -> str:
