...
class PersonaRuntimeIntegration:
    ...
    def __init__(self):
        self.pipeline_config = PipelineConfig(
            ...
            engagement_threshold=float(os.getenv("FINE_TUNING_ENGAGEMENT_THRESHOLD", "0.06")),
    ...
    def should_trigger_fine_tuning(self) -> bool:
        ...
        if self.pipeline.last_run_timestamp:
            days_since_last_run = (datetime.now() - self.pipeline.last_run_timestamp).days
        ...
        try:
            ...
            if result.status == "success" and result.model_version:
                ...
                registered_model = registry.register_fine_tuned_model(
                    ...
                    performance_metrics={
                        "training_examples": len(result.training_data_batch.hook_examples) if result.training_data_batch else 0,
...
def demonstrate_fine_tuning_workflow():
    ...
    print(f"   Minimum training examples: {integration.pipeline_config.training_data_threshold}")
    print(f"   Engagement threshold: {integration.pipeline_config.engagement_threshold}")
    print(f"   Weekly schedule: {integration.pipeline_config.weekly_schedule}")
    print(f"   A/B test duration: {integration.pipeline_config.a_b_test_duration_hours} hours")
