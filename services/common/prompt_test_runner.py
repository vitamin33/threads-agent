...
class PromptTestRunner:
    ...
    def _validate_output(
        self, actual: str, expected: str, rules: List[str]
        ...
        for rule in rules:
            ...
            if rule_name == ValidationRule.EXACT_MATCH.value:
                if actual != expected:
                    return (
                        False,
                        f"exact_match failed: Expected '{expected}', Actual '{actual}'",
                    )
                ...
                if not re.search(pattern, actual):
                    return (
                        False,
                        f"regex_match failed: Pattern '{pattern}' not found in '{actual}'",
                    )
                ...
                if not (min_len <= actual_len <= max_len):
                    return (
                        False,
                        f"length_range failed: Length {actual_len} not in range [{min_len}, {max_len}]",
                    )
                ...
                if not any(word.istitle() for word in actual.split()):
                    return (
                        False,
                        f"contains_name failed: No capitalized name found in '{actual}'",
                    )
                ...
                if not any(word.lower() in actual.lower() for word in greeting_words):
                    return (
                        False,
                        f"greeting_format failed: No greeting words found in '{actual}'",
                    )
                ...
                if len(actual.strip()) == 0:
                    return (
                        False,
                        "handles_empty_input failed: Output is empty or whitespace only",
                    )
    ...
    def _validate_performance(
        self, execution_time: float, rules: List[str]
    ) -> tuple[bool, Optional[str]]:
        """
        ...
        for rule in rules:
            ...
            if rule_name == ValidationRule.MAX_EXECUTION_TIME.value:
                ...
                if execution_time > max_time:
                    return (
                        False,
                        f"execution_time exceeded: {execution_time:.3f}s > {max_time}s",
                    )
