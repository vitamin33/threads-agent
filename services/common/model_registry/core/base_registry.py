...
class BaseModelRegistry(ABC):
    ...
    async def register_model(
        self, name: str, model_type: ModelType, metadata: Dict[str, Any], **kwargs
    ) -> str:
...
class UnifiedModelRegistry(BaseModelRegistry):
    ...
    async def register_model(
        ...
        try:
            ...
            logger.info(
                f"Successfully registered {model_type.value} model: {name} -> {model_id}"
            )
            return model_id
    ...
    async def promote_model(
        ...
        try:
            ...
            success = await adapter.promote_model(
    ...
    async def _log_registration_event(
        self, model_id: str, model_type: ModelType, metadata: Dict[str, Any]
    ):
        ...
        try:
            ...
            experiment_name = (
                self.config.mlflow.experiment_name or "model_registry_events"
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "client_manager": {
                "active_clients": len(self.client_manager._clients)
                if hasattr(self.client_manager, "_clients")
                else 0
            },
        }
