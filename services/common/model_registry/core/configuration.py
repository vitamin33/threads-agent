...
class MLflowConfig:
    ...
    def __post_init__(self):
        ...
        self.registry_uri = os.getenv(
            "MLFLOW_REGISTRY_URI", self.registry_uri or self.tracking_uri
        )
...
def _load_from_environment() -> Dict[str, Any]:
    ...
    if "MODEL_REGISTRY_CACHE_ENABLED" in os.environ:
        caching_config["enabled"] = (
            os.environ["MODEL_REGISTRY_CACHE_ENABLED"].lower() == "true"
        )
    ...
    if "MODEL_REGISTRY_ASYNC" in os.environ:
        performance_config["async_operations"] = (
            os.environ["MODEL_REGISTRY_ASYNC"].lower() == "true"
        )
    if "MODEL_REGISTRY_BATCH_SIZE" in os.environ:
        try:
            performance_config["batch_size"] = int(
                os.environ["MODEL_REGISTRY_BATCH_SIZE"]
            )
        except ValueError:
            pass
