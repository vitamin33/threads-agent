...
from ..core.base_registry import BaseModelRegistry, ModelStage, ModelLoadState, ModelRegistryError
...
class VLLMAdapter:
    ...
    def _find_vllm_config_file(self) -> Optional[Path]:
        ...
        potential_locations = [
            Path.cwd() / "services" / "vllm_service" / "config" / "multi_model_config.yaml",
            Path(__file__).parent.parent.parent.parent / "vllm_service" / "config" / "multi_model_config.yaml",
    ...
    def _load_default_models(self):
        ...
        default_models = {
            "llama_8b": {
                ...
                "memory_requirements": {"base_gb": 16.0, "optimized_gb": 12.0, "minimum_gb": 10.0},
                "content_types": ["general", "linkedin", "technical_articles"],
                "priority": 1,
                "optimization": {"quantization": "fp16", "backend": "metal"},
                "performance": {"target_latency_ms": 40, "max_tokens_per_request": 2048, "concurrent_requests": 3}
            },
            "qwen_7b": {
                "name": "Qwen/Qwen2.5-7B-Instruct",
                "display_name": "Qwen 2.5 7B Instruct",
                "memory_requirements": {"base_gb": 14.0, "optimized_gb": 10.0, "minimum_gb": 8.0},
                "content_types": ["technical_articles", "code_documentation"],
                "priority": 2,
                "optimization": {"quantization": "fp16", "backend": "metal"},
                "performance": {"target_latency_ms": 35, "max_tokens_per_request": 4096, "concurrent_requests": 3}
            },
            "mistral_7b": {
                ...
                "priority": 3,
                "optimization": {"quantization": "fp16", "backend": "metal"},
                "performance": {"target_latency_ms": 30, "max_tokens_per_request": 1024, "concurrent_requests": 4}
            },
            "llama_3b": {
                "name": "meta-llama/Llama-3.1-3B-Instruct",
                "display_name": "Llama 3.1 3B Instruct",
                "memory_requirements": {"base_gb": 6.0, "optimized_gb": 4.0, "minimum_gb": 3.0},
                "content_types": ["twitter", "speed"],
                "priority": 4,
                "optimization": {"quantization": "fp16", "backend": "metal"},
                "performance": {"target_latency_ms": 20, "max_tokens_per_request": 512, "concurrent_requests": 8}
            },
            "phi_mini": {
                "name": "microsoft/Phi-3.5-mini-instruct",
                "display_name": "Phi 3.5 Mini Instruct",
                "memory_requirements": {"base_gb": 8.0, "optimized_gb": 6.0, "minimum_gb": 4.0},
                "content_types": ["lightweight", "general"],
                "priority": 5,
                "optimization": {"quantization": "int8", "backend": "metal"},
                "performance": {"target_latency_ms": 15, "max_tokens_per_request": 1024, "concurrent_requests": 6}
            }
        }
        
        self._load_model_configs(default_models)
    
    async def register_model(self, name: str, metadata: Dict[str, Any], **kwargs) -> str:
        """Register a vLLM model."""
        # For vLLM models, we typically register configurations rather than actual model files
        model_id = kwargs.get('model_id', name.replace("/", "_").replace("-", "_").lower())
        
        # Create model configuration from metadata
        config = VLLMModelConfig(
            name=name,
            display_name=metadata.get('display_name', name),
            memory_requirements=MemoryRequirements(**metadata.get('memory_requirements', {
                'base_gb': 8.0, 'optimized_gb': 6.0, 'minimum_gb': 4.0
            })),
            content_types=[ContentType(ct) for ct in metadata.get('content_types', ['general'])],
            priority=metadata.get('priority', 99),
            optimization=OptimizationConfig(**metadata.get('optimization', {})),
            performance=PerformanceTargets(**metadata.get('performance', {
                'target_latency_ms': 50, 'max_tokens_per_request': 1024, 'concurrent_requests': 1
            })),
            model_id=model_id
        )
    ...
    async def promote_model(self, model_id: str, target_stage: ModelStage, validation_results: Optional[Dict] = None) -> bool:
    ...
    async def get_model(self, model_identifier: Union[str, Dict[str, Any]]) -> Optional[VLLMModelConfig]:
    ...
    async def list_models(self, stage: Optional[ModelStage] = None, **filters) -> List[Dict[str, Any]]:
        """List vLLM models with optional filtering."""
        results = []
        
        for model_id, config in self.models.items():
            # Apply stage filter
            if stage and stage == ModelStage.PRODUCTION and config.load_state != ModelLoadState.LOADED:
                continue
            if stage and stage == ModelStage.DEPRECATED and config.load_state != ModelLoadState.UNLOADED:
                continue
    ...
    def get_model_for_content_type(self, content_type: ContentType) -> Optional[VLLMModelConfig]:
    ...
    def _can_load_model_memory(self, required_memory: float) -> bool:
        ...
        available_memory = (self.total_memory_gb * self.memory_threshold) - current_usage
    ...
    async def _log_vllm_registration(self, model_id: str, config: VLLMModelConfig):
        ...
        try:
            ...
            with mlflow.start_run():
                ...
                mlflow.log_param("content_types", ",".join([ct.value for ct in config.content_types]))
                
                # Log memory requirements
                mlflow.log_param("memory_base_gb", config.memory_requirements.base_gb)
                mlflow.log_param("memory_optimized_gb", config.memory_requirements.optimized_gb)
                mlflow.log_param("memory_minimum_gb", config.memory_requirements.minimum_gb)
                
                # Log performance targets
                mlflow.log_param("target_latency_ms", config.performance.target_latency_ms)
                mlflow.log_param("max_tokens_per_request", config.performance.max_tokens_per_request)
                mlflow.log_param("concurrent_requests", config.performance.concurrent_requests)
    ...
    def get_memory_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "utilization_percent": (total_used / max_usable) * 100 if max_usable > 0 else 0,
