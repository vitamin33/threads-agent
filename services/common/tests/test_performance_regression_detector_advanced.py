...
class TestPerformanceRegressionDetectorScalabilityAndPerformance:
    ...
    def test_concurrent_regression_detection(self):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(run_regression_detection, i) for i in range(num_threads)
            ]
        ...
        assert all(
            result.is_regression is True for result in results
    ...
    def test_statistical_test_performance_comparison(self):
        ...
        for test_name, perf_data in performance_results.items():
            assert perf_data["execution_time"] < 5.0, (
                f"{test_name} took too long: {perf_data['execution_time']}s"
            )
    ...
    def test_microservice_performance_monitoring(self):
        ...
        for day in range(14):  # 2 weeks of data
            for hour in range(24):
                ...
                actual_latency = base_latency + np.random.exponential(
                    10
                )  # Exponential tail
                ...
                historical_latencies.append(
                    PerformanceData(
                        timestamp=datetime.now()
                        - timedelta(days=14 - day, hours=24 - hour),
        ...
        for week in range(4):
            for day in range(7):
                ...
                historical_accuracies.append(
                    PerformanceData(
                        timestamp=datetime.now()
                        - timedelta(weeks=4 - week, days=7 - day),
    ...
    def test_high_frequency_monitoring_performance(self):
        ...
        for second in range(0, 300, 10):  # 5 minutes of monitoring, every 10 seconds
            ...
            new_point = generate_data_point(
                current_timestamp, is_degraded=(second > 120)
            )
