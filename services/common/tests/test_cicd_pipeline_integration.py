...
class TestCICDPipelineIntegration:
    ...
    def test_successful_cicd_pipeline_flow(self, pipeline_components):
        ...
        current_data = self._generate_performance_data(
            baseline_mean=0.86, count=15
        )  # Slightly better
    ...
    def test_cicd_pipeline_with_performance_regression_and_rollback(
    ...
    def test_pipeline_error_handling_and_recovery(self, pipeline_components):
        ...
        mock_registry.rollback_to_model.side_effect = Exception(
    ...
    def _generate_performance_data(
        self, baseline_mean: float, count: int, std_dev: float = 0.05
    ) -> list:
