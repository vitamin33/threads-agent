...
class TestKubernetesOptimizedPipeline:
    ...
    async def test_redis_caching_operations(self, k8s_pipeline):
        ...
        with patch.object(k8s_pipeline.connection_manager, 'get_redis_connection') as mock_get_redis:
            mock_get_redis.return_value.__aenter__.return_value = mock_redis_client
            
            # Test caching metrics
            await k8s_pipeline.cache_evaluation_metrics("candidate", "test-ab-123", test_metrics)
            ...
            mock_redis_client.get.assert_called_once_with("model_metrics:test-ab-123:candidate")
    ...
    async def test_prometheus_metrics_generation(self, k8s_pipeline):
        ...
        assert 'fine_tuning_mlflow_circuit_breaker_state{state="CLOSED"} 1' in metrics_output
...
class TestPerformanceMonitoring:
    ...
    async def test_connection_pool_performance_tracking(self):
        ...
        assert duration < 0.3, f"Concurrent execution took {duration:.2f}s, expected < 0.3s"
    ...
    async def test_memory_efficiency_under_load(self):
        ...
        assert memory_increase < 50, f"Memory increased by {memory_increase:.2f}MB under load"
