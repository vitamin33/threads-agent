...
class TestGitHubWorkflowValidation:
    ...
    def test_workflow_files_exist(self, workflow_files):
        ...
        for workflow_file in workflow_files:
            ...
            assert workflow_file.stat().st_size > 0, f"Empty workflow file: {workflow_file}"
    ...
    def test_workflow_yaml_syntax(self, workflow_files):
        ...
        for workflow_file in workflow_files:
            try:
                ...
                assert workflow_content is not None, f"Empty YAML content: {workflow_file}"
    ...
    def test_workflow_required_fields(self, workflow_files):
        ...
        for workflow_file in workflow_files:
            ...
            for field in required_root_fields:
                assert field in workflow, f"Missing required field '{field}' in {workflow_file}"
            
            # Test jobs structure
            if 'jobs' in workflow:
                assert isinstance(workflow['jobs'], dict), f"Jobs must be a dict in {workflow_file}"
    ...
    def test_workflow_security_best_practices(self, workflow_files):
        ...
        for workflow_file in workflow_files:
            ...
            if any(pattern in workflow_content.lower() for pattern in ['password=', 'token=', 'key=']):
            ...
            assert len(security_issues) == 0, f"Security issues in {workflow_file}: {security_issues}"
    ...
    def test_ci_workflow_specific_requirements(self, workflow_files):
        ...
        for workflow_file in ci_workflows:
            ...
            assert test_job_found, f"CI workflow should have test-related jobs: {workflow_file}"
            ...
            'jobs': {
                'validate-prompts': {
                    ...
                    'steps': [
                        ...
                        {'name': 'Run prompt tests', 'run': 'python -m pytest services/common/tests/test_prompt_test_runner.py'},
                        {'name': 'Validate prompt syntax', 'run': 'python scripts/validate_prompts.py'},
                        {'name': 'Performance regression check', 'run': 'python scripts/check_performance_regression.py'}
                    ]
                }
            }
        }
            ...
            'jobs': {
                'performance-check': {
                    ...
                    'steps': [
                        {'uses': 'actions/checkout@v4'},
                        {'name': 'Setup Python', 'uses': 'actions/setup-python@v4'},
                        {'name': 'Collect performance metrics', 'run': 'python scripts/collect_metrics.py'},
                        {'name': 'Run regression detection', 'run': 'python scripts/detect_regression.py'},
                        {'name': 'Trigger rollback if needed', 'run': 'python scripts/auto_rollback.py'}
                    ]
                }
            }
        }
        ...
        workflow_content = {
            ...
            'jobs': {
                'emergency-rollback': {
                    ...
                    'steps': [
                        {'uses': 'actions/checkout@v4'},
                        {'name': 'Validate rollback target', 'run': 'python scripts/validate_rollback_target.py'},
                        {'name': 'Execute rollback', 'run': 'python scripts/execute_rollback.py'},
                        {'name': 'Verify rollback success', 'run': 'python scripts/verify_rollback.py'},
                        {'name': 'Notify team', 'run': 'python scripts/notify_rollback.py'}
                    ]
                }
            }
        }
    ...
    def test_gradual_rollout_workflow_integration(self):
        ...
        workflow_content = {
                ...
                'workflow_dispatch': {
                    'inputs': {
                        ...
                        'stage': {
                            ...
                            'required': False,
                            'type': 'choice',
                            'options': ['canary_10', 'canary_25', 'canary_50', 'full_rollout'],
                            'default': 'canary_10'
                        }
                    }
                }
            ...
            'jobs': {
                'gradual-rollout': {
                    'runs-on': 'ubuntu-latest',
                    'steps': [
                        {'name': 'Start rollout', 'run': 'python scripts/start_gradual_rollout.py'},
                        {'name': 'Monitor health', 'run': 'python scripts/monitor_deployment_health.py'},
                        {'name': 'Advance stage', 'run': 'python scripts/advance_rollout_stage.py'},
                        {'name': 'Complete rollout', 'run': 'python scripts/complete_rollout.py'}
                    ]
                }
            }
        }
        ...
        with patch('subprocess.run') as mock_run:
            ...
            for stage in expected_stages:
                mock_run.return_value.stdout = json.dumps({
                    'stage': stage,
                    'traffic_percentage': {'canary_10': 10, 'canary_25': 25, 'canary_50': 50, 'full_rollout': 100}[stage],
                    'health_status': 'healthy'
                })
                
                result = self._simulate_workflow_step(f"python scripts/advance_rollout_stage.py --stage {stage}")
                assert result['success'] is True
...
class TestWorkflowPerformanceAndReliability:
    ...
    def test_workflow_resource_usage_limits(self):
        ...
        for resource, limit in resource_limits.items():
            ...
            assert usage_percentage < 80, f"{resource} usage {usage_percentage:.1f}% too high"
    ...
    def test_workflow_failure_notification_and_alerting(self):
        ...
        failure_scenarios = [
            ...
            {'type': 'rollback_failure', 'severity': 'critical', 'channel': 'pagerduty'},
        ...
        for scenario in failure_scenarios:
            ...
            assert notification_sent['success'], f"Failed to send {scenario['type']} notification"
    ...
    def _simulate_notification(self, failure_type: str, severity: str, channel: str) -> Dict[str, Any]:
...
class TestWorkflowSecurityAndCompliance:
    ...
    def test_workflow_secret_management(self):
        ...
        for secret_ref in secure_secret_usage:
            assert secret_ref.startswith('${{ secrets.'), f"Invalid secret reference: {secret_ref}"
    ...
    def test_workflow_permission_restrictions(self):
        ...
        for permission, level in minimal_permissions.items():
            assert level in ['read', 'write', 'none'], f"Invalid permission level: {level}"
    ...
    def test_workflow_dependency_security(self):
        ...
        for action in insecure_actions:
            ...
            if len(parts) == 2:
                ...
                assert version not in insecure_versions, f"Insecure action version: {action}"
    ...
    def test_workflow_audit_and_compliance_logging(self):
        ...
        log_date = datetime.fromisoformat(audit_log_entry['timestamp'].replace('Z', '+00:00'))
        retention_limit = datetime.now() - timedelta(days=audit_requirements['retention_days'])
...
class TestWorkflowActIntegration:
    ...
    def act_available(self):
        ...
        try:
            ...
            pytest.skip("'act' tool not available - install from https://github.com/nektos/act")
        ...
        rollback_workflow = {
            ...
            'jobs': {
                'rollback': {
                    ...
                    'timeout-minutes': 5,
                    'steps': [
                        {'name': 'Validate rollback', 'run': 'echo "Validating rollback target"'},
                        {'name': 'Execute rollback', 'run': 'echo "Rolling back to previous version"'},
                        {'name': 'Verify rollback', 'run': 'echo "Rollback completed successfully"'}
                    ]
                }
            }
        }
