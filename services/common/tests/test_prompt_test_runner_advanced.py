...
class TestPromptTestRunnerAdvancedEdgeCases:
    ...
    def test_unicode_edge_cases_comprehensive(self, memory_limited_mock_model):
        ...
        test_cases = [
            ...
            {"input": "test\u200bzero\u200bwidth", "expected": "test\u200bzero\u200bwidth"},
    ...
    def test_model_exception_during_rendering_comprehensive(self, memory_limited_mock_model):
...
class TestPromptTestRunnerConcurrencyAndPerformance:
    ...
    def test_high_concurrency_execution(self, thread_safe_mock_model):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(run_tests_in_thread, i) 
    ...
    def test_memory_usage_scaling(self, thread_safe_mock_model):
        ...
        memory_per_test = [mem / size for size, mem in memory_measurements[1:]]  # Skip first measurement
        ...
        memory_ratio = max_memory_per_test / min_memory_per_test if min_memory_per_test > 0 else 1
        
        assert memory_ratio < 2.0, f"Memory usage scaling issue: {memory_ratio}x variation"
...
class TestPromptTestRunnerRealWorldScenarios:
    ...
    def test_production_like_test_suite_execution(self, realistic_prompt_model):
        ...
        production_test_cases = [
            ...
            TestCase(
                ...
                validation_rules=["contains:Large input processed", "max_execution_time:3.0"]
            ),
            ...
            TestCase(
                ...
                validation_rules=["contains:Slowly processed", "max_execution_time:1.0"]  # Will fail due to timeout
            ),
            # Error cases
            TestCase(
                name="error_handling",
        ...
        test_suite = TestSuite(
            name="production_integration_suite",
            test_cases=production_test_cases
        )
    ...
    def test_integration_with_model_registry_workflow(self, realistic_prompt_model):
        ...
        with patch('services.common.prompt_model_registry.get_mlflow_client'):
            ...
            assert summary["success_rate"] == 1.0  # All tests should pass for deployment
    ...
    def test_continuous_integration_pipeline_simulation(self, realistic_prompt_model):
        ...
        average_duration = sum(run["duration"] for run in ci_test_runs) / len(ci_test_runs)
        overall_success_rate = sum(run["success_rate"] for run in ci_test_runs) / len(ci_test_runs)
