...
class TestFineTuningPipeline:
    ...
    async def test_pipeline_run_orchestrates_all_components(self):
        ...
        hook_examples = [
            {"input": f"test_{i}", "output": f"hook_{i}"} for i in range(60)
        ]
        body_examples = [
            {"input": f"test_{i}", "output": f"body_{i}"} for i in range(60)
        ]
        ...
        with (
            ...
            ) as mock_data_collector,
            patch("services.common.fine_tuning_pipeline.ModelTrainer") as mock_trainer,
        ):
            # Setup mocks
            mock_data_collector.return_value.collect_training_data.return_value = (
                mock_training_data
            )
            mock_trainer.return_value.start_fine_tuning = AsyncMock(
            ...
            mock_trainer.return_value.start_fine_tuning.assert_called_once_with(
                mock_training_data
            )
    ...
    async def test_pipeline_skips_run_when_insufficient_data(self):
        ...
        with patch(
            ...
            assert (
                len(result.training_data_batch.hook_examples)
                < config.training_data_threshold
            )
        ...
        with patch(
            ...
            mock_posts = [
                Mock(
                    ...
                    hook="High engagement hook",
                    body="High engagement body",
                ),
                Mock(
                    engagement_rate=0.04,
                    hook="Low engagement hook",
                    body="Low engagement body",
                ),
                Mock(
                    engagement_rate=0.10,
                    hook="Very high engagement hook",
                    body="Very high engagement body",
                ),
            ]
            ...
            assert all(
                ex["engagement_rate"] >= 0.06 for ex in training_data.hook_examples
            )
...
class TestMLflowIntegration:
    ...
    def test_pipeline_tracks_experiment_in_mlflow(self):
        ...
        with (
            patch(
                "services.common.fine_tuning_pipeline.mlflow.start_run"
            ) as mock_start_run,
            patch(
                "services.common.fine_tuning_pipeline.mlflow.log_params"
            ) as mock_log_params,
            patch(
            ...
            tracker.start_experiment_run(config)
    ...
    def test_model_registry_integration(self):
        ...
        with patch(
            "services.common.fine_tuning_pipeline.get_mlflow_client"
        ) as mock_client:
            mock_client.return_value.create_registered_model.return_value = Mock()
            mock_client.return_value.create_model_version.return_value = Mock(
                version="1"
            )
            ...
            registered_model = registry.register_fine_tuned_model(
                model_version=model_version,
                performance_metrics={"engagement_lift": 0.25, "cost_efficiency": 0.15},
            )
...
class TestEndToEndIntegration:
    ...
    async def test_complete_pipeline_with_mlflow_tracking(self):
        ...
        with (
            ...
            patch("services.common.fine_tuning_pipeline.ModelTrainer") as mock_trainer,
            patch(
                "services.common.fine_tuning_pipeline.ModelEvaluator"
            ) as mock_evaluator,
            patch(
                "services.common.fine_tuning_pipeline.DeploymentManager"
            ) as mock_deployer,
            patch(
                "services.common.fine_tuning_pipeline.mlflow.start_run"
            ) as mock_mlflow_run,
            ...
            deployment_result = Mock(
                status="success", model_id="ft:gpt-3.5-turbo:test:1234"
            )
