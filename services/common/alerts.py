...
class AIAlertManager:
    ...
    def __init__(self, max_alerts: int = 1000):
        ...
        self.alert_cooldown = timedelta(
            minutes=15
        )  # Don't repeat same alert type for 15 min
    ...
    def check_and_alert(self, metrics: Dict[str, Any]) -> List[Alert]:
        ...
        for alert_type in [
            ...
            for severity in ["INFO", "WARNING", "ERROR", "CRITICAL"]:
                ...
                AI_ACTIVE_ALERTS.labels(alert_type=alert_type, severity=severity).set(
                    count
                )
    ...
    def _check_model_drift(
        self, ai_metrics: Dict[str, Any], alerts: List[Alert]
    ) -> None:
        """Check for model confidence drift."""
        confidence_trend = ai_metrics.get("drift_detection", {}).get(
            "model_confidence_trend", ""
        )

        if "significant_drift" in confidence_trend and self._should_alert(
            "MODEL_DRIFT"
        ):
            ...
            alert = Alert(
                ...
                metadata={
                    ...
                    "avg_confidence": ai_metrics.get("drift_detection", {}).get(
                        "avg_confidence", 0
                    ),
                },
            )
    ...
    def _check_latency(self, performance: Dict[str, Any], alerts: List[Alert]) -> None:
        ...
        if p99_latency > self.thresholds["latency_p99_ms"] and self._should_alert(
            "HIGH_LATENCY"
        ):
            alert = Alert(
        ...
        elif p95_latency > self.thresholds["latency_p95_ms"] and self._should_alert(
            "HIGH_LATENCY"
        ):
            alert = Alert(
                alert_type="HIGH_LATENCY",
    ...
    def _check_costs(self, cost_metrics: Dict[str, Any], alerts: List[Alert]) -> None:
        ...
        if cost_per_request > self.thresholds[
            "cost_per_request"
        ] and self._should_alert("HIGH_COST"):
    ...
    def _check_error_rate(
        self, performance: Dict[str, Any], alerts: List[Alert]
    ) -> None:
        """Check for high error rates."""
        error_rate = performance.get("error_rate", 0)

        if error_rate > self.thresholds["error_rate"] and self._should_alert(
            "HIGH_ERROR_RATE"
        ):
            alert = Alert(
    ...
    def _check_security(
        self, safety_metrics: Dict[str, Any], alerts: List[Alert]
    ) -> None:
        """Check for security incidents."""
        prompt_injections = safety_metrics.get("prompt_injection_attempts_24h", 0)

        if prompt_injections > self.thresholds[
            "security_incident_threshold"
        ] and self._should_alert("SECURITY_INCIDENT"):
            alert = Alert(
                ...
                metadata={
                    ...
                    "hallucination_flags": safety_metrics.get(
                        "hallucination_flags_24h", 0
                    ),
                    "content_violations": safety_metrics.get(
                        "content_violations_24h", 0
                    ),
                },
            )
            alerts.append(alert)
    ...
    def _check_quality(self, ai_metrics: Dict[str, Any], alerts: List[Alert]) -> None:
        ...
        if avg_confidence < self.thresholds["min_confidence"] and self._should_alert(
            "QUALITY_DEGRADATION"
        ):
            alert = Alert(
    ...
    def acknowledge_alert(self, alert_id: str) -> bool:
        ...
        for alert in self.alerts:
            if alert.id == alert_id:
                ...
                if (
                    alert.type in self.active_alerts
                    and self.active_alerts[alert.type].id == alert_id
                ):
