...
class PerformanceRegressionDetector:
    ...
    def detect_regression(
        ...
        baseline_std = (
            np.std(historical_values, ddof=1) if len(historical_values) > 1 else 0.0
        )
    ...
    def _filter_outliers(self, values: List[float]) -> Tuple[List[float], int]:
        ...
        filtered_values = [
            v for v, z in zip(values, z_scores) if z < self.outlier_threshold
        ]
    ...
    def generate_detailed_report(self, result: RegressionResult) -> Dict[str, Any]:
        ...
        return {
            ...
            "data_quality": {"outliers_filtered": result.outliers_filtered},
    ...
    def _generate_recommendations(self, result: RegressionResult) -> List[str]:
        ...
        if result.is_regression:
            ...
            if result.effect_size_magnitude == "large":
                recommendations.append(
                    "Large effect size - prioritize immediate investigation"
                )

            if result.consensus_score < 0.5:
                recommendations.append(
                    "Low consensus between tests - verify with additional metrics"
                )

        if result.outliers_filtered > 0:
            recommendations.append(
                f"Filtered {result.outliers_filtered} outliers - verify data quality"
            )
    ...
    def _determine_alert_severity(self, result: RegressionResult) -> str:
        ...
        if result.effect_size_magnitude == "large" and result.p_value < 0.01:
            ...
            result.effect_size_magnitude in ["large", "medium"]
