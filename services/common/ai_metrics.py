...
AI_REQUESTS_TOTAL = Counter(
    "ai_requests_total", "Total number of AI inference requests", ["model", "service"]
)
...
AI_COST_PER_REQUEST = Summary(
    "ai_cost_per_request", "Cost per AI request in dollars", ["model", "service"]
)
...
class AIMetricsTracker:
    ...
    def record_inference(
        ...
        if error:
            ...
            AI_ERRORS_TOTAL.labels(
                model=model_name, service=service, error_type="inference_error"
            ).inc()
            return
        ...
        # Emit Prometheus metrics
        AI_RESPONSE_TIME.labels(model=model_name, service=service).observe(
            response_time_ms
        )
        AI_TOKENS_TOTAL.labels(
            model=model_name, service=service, token_type="prompt"
        ).inc(prompt_tokens)
        AI_TOKENS_TOTAL.labels(
            model=model_name, service=service, token_type="completion"
        ...
        if confidence is not None:
            AI_CONFIDENCE_SCORE.labels(model=model_name, service=service).set(
                confidence
            )
    ...
    def _detect_confidence_drift(self) -> str:
        ...
        for model_name in self.model_metrics.keys():
            AI_CONFIDENCE_DRIFT.labels(model=model_name, service="global").set(
