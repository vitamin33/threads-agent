...
class TestEventLoopOptimization:
    ...
    async def test_subscriber_does_not_create_new_event_loops(self):
        ...
        with patch("asyncio.new_event_loop", side_effect=track_event_loop_creation):
            ...
            subscriber._message_callback(
                mock_channel,
                mock_method,
                mock_properties,
                '{"event_id": "test-123", "event_type": "test_event", "timestamp": "2024-01-01T00:00:00Z", "payload": {"key": "value"}}',
            )

            # ASSERTION THAT SHOULD FAIL: No new event loops should be created
            assert len(event_loop_creations) == 0, (
                "EventSubscriber should not create new event loops for message processing"
            )
    ...
    def test_subscriber_message_callback_is_sync_wrapper_for_async(self):
        ...
        assert "asyncio.new_event_loop()" in source, (
            "Current implementation creates new event loop (this is the performance issue)"
        )
        assert "loop.run_until_complete" in source, (
            "Current implementation uses run_until_complete (blocks the thread)"
        )
    ...
    async def test_subscriber_should_use_proper_async_context(self):
        ...
        assert hasattr(subscriber, "_process_message"), (
            "Subscriber should have async _process_message method"
        )
        ...
        result = await subscriber._process_message(
            mock_channel, mock_method, mock_properties, test_message
        )
    ...
    async def test_event_loop_reuse_performance(self):
        ...
        for message in messages:
            task = asyncio.create_task(
                subscriber._process_message(
                    mock_channel, mock_method, mock_properties, message
                )
            )
            tasks.append(task)
    ...
    def test_current_implementation_blocks_event_loop(self):
        ...
        assert "asyncio.new_event_loop()" in source, (
            "Creates new event loop (performance issue)"
        )
        assert "asyncio.set_event_loop(loop)" in source, (
            "Sets the loop (affects global state)"
        )
        assert "loop.run_until_complete" in source, (
            "Blocks the thread (prevents concurrency)"
        )
    ...
    async def test_optimized_subscriber_should_handle_concurrent_messages(self):
        ...
        assert hasattr(subscriber, "handle_message_async"), (
            "Should have async message handler"
        )
        ...
        tasks = [subscriber.handle_message_async(msg) for msg in test_messages]
        ...
        assert processing_time < 0.1, (
            f"Concurrent processing should be fast, took {processing_time}s"
        )
