...
class TestEventPublisher:
    ...
    def test_event_publisher_custom_retry_settings(self):
        ...
        publisher = EventPublisher(
            connection_manager=mock_connection_manager, max_retries=5, retry_delay=2.0
        )
    ...
    async def test_publish_event_success(self):
        ...
        result = await publisher.publish(
            event=event, exchange="test_exchange", routing_key="test.routing.key"
        )
    ...
    async def test_publish_event_with_retry_on_failure(self):
        ...
        publisher = EventPublisher(
            connection_manager=mock_connection_manager, max_retries=2
        )
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            result = await publisher.publish(
                event=event, exchange="test_exchange", routing_key="test.key"
            )
    ...
    async def test_publish_event_serialization(self):
        ...
        await publisher.publish(
            event=event, exchange="user_events", routing_key="user.created"
        )
