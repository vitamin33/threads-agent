...
class TestDashboardIntegration:
    ...
    async def test_get_metrics_endpoint(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            with patch(
                "services.dashboard_api.main.metrics_api.get_live_metrics"
            ) as mock_metrics:
    ...
    async def test_early_kill_event_processing(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            with patch(
                "services.dashboard_api.main.event_processor.handle_early_kill_event"
            ) as mock_handler:
    ...
    async def test_performance_update_event(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            with patch(
                "services.dashboard_api.main.event_processor.handle_performance_update"
            ) as mock_handler:
                mock_handler.return_value = None
                ...
                response = await client.post(
                    "/api/events/performance-update", json=event_data
                )
                assert response.status_code == 200
                assert response.json()["status"] == "processed"
