...
import pytest
import sys
sys.path.append('/Users/vitaliiserbyn/development/wt-a3-analytics/services/dashboard_api')

from traffic_driver_integration import TrafficDriverService, ContentToJobOpportunityPipeline
    ...
    async def _generate_test_conversion_data(self, service: TrafficDriverService):
        ...
        for visitor_data in test_visitors:
            ...
            behavior_data = {
                ...
                "utm_source": "linkedin" if "linkedin" in visitor_data["referrer_url"] else "devto"
            }
...
class TestContentToJobOpportunityPipeline:
    ...
    async def test_campaign_performance_analytics(self):
        ...
        test_visitors = [
            {
                ...
                "referrer_url": f"https://serbyn.pro/portfolio?utm_source=linkedin&utm_campaign=campaign_test",
                "current_url": "https://serbyn.pro/portfolio"
            }
            for i in range(3)
        ]
        ...
        assert any("content" in rec.lower() or "optimization" in rec.lower() for rec in recommendations)