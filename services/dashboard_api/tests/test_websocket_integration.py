...
class TestWebSocketIntegration:
    ...
    async def test_websocket_connection_lifecycle(self, mock_database_data):
        ...
        with patch('services.dashboard_api.variant_metrics.get_db_connection') as mock_db:
            with patch('services.dashboard_api.variant_metrics.get_redis_connection') as mock_redis:
                # Setup mocks
    ...
    async def test_websocket_initial_data_structure(self, mock_database_data):
        ...
        data = sent_data["data"]
        required_sections = [
            "summary", "active_variants", "early_kills_today", 
            "pattern_fatigue_warnings", "optimization_opportunities", "real_time_feed"
        ]
    ...
    async def test_websocket_message_handling_integration(self):
        ...
        await ws_handler.handle_message(mock_websocket, persona_id, '{"type": "refresh"}')
