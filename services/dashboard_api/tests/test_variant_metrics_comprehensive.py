...
class TestVariantMetricsAPIComprehensive:
    ...
    @pytest.fixture
    def metrics_api(self, mock_db, mock_redis, mock_early_kill_monitor, mock_fatigue_detector):
    ...
    @pytest.mark.asyncio
    async def test_get_active_variants_with_time_calculations(self, metrics_api, mock_db):
        ...
        assert abs(variant1["performance_vs_prediction"]["relative_delta"] - expected_delta) < 0.01
    ...
    async def test_get_optimization_suggestions_logic(self, metrics_api, mock_db):
        ...
        metrics_api.calculate_early_kill_rate = AsyncMock(return_value=0.40)  # High kill rate
        metrics_api.calculate_prediction_accuracy = AsyncMock(return_value=0.75)  # Low accuracy
        ...
        calibration_suggestions = [s for s in suggestions if s["type"] == "prediction_calibration"]
    ...
    async def test_get_recent_events_chronological_order(self, metrics_api, mock_db):
        ...
        mock_events = [
            ...
            {"event_type": "performance_update", "timestamp": now - timedelta(minutes=5)},
