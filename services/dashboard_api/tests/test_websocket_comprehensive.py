...
class TestVariantDashboardWebSocket:
    ...
    @pytest.mark.asyncio
    async def test_handle_connection_accepts_websocket(self, websocket_handler, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_handle_connection_sends_initial_data(self, websocket_handler, mock_websocket, mock_metrics_api):
    ...
    @pytest.mark.asyncio
    async def test_connection_cleanup_on_disconnect(self, websocket_handler, mock_websocket):
        ...
        assert persona_id not in websocket_handler.connections or \
               len(websocket_handler.connections.get(persona_id, set())) == 0
    ...
    async def test_handle_message_ping_pong(self, websocket_handler, mock_websocket):
        ...
        # Test ping message
        await websocket_handler.handle_message(mock_websocket, persona_id, '{"type": "ping"}')
    ...
    @pytest.mark.asyncio
    async def test_handle_message_data_refresh(self, websocket_handler, mock_websocket, mock_metrics_api):
    ...
    async def test_connection_limit_per_persona(self, websocket_handler):
        ...
        mock_connections = [AsyncMock(spec=WebSocket) for _ in range(max_connections + 5)]
        
        # Add connections up to limit
        websocket_handler.connections[persona_id] = set(mock_connections[:max_connections])
...
class TestDashboardEventProcessor:
    ...
    @pytest.mark.asyncio
    async def test_handle_early_kill_event(self, event_processor, mock_websocket_handler):
    ...
    @pytest.mark.asyncio
    async def test_handle_performance_update(self, event_processor, mock_websocket_handler):
    ...
    @pytest.mark.asyncio
    async def test_handle_pattern_fatigue_alert(self, event_processor, mock_websocket_handler):
    ...
    @pytest.mark.asyncio
    async def test_handle_event_missing_persona_id(self, event_processor, mock_websocket_handler):
    ...
    @pytest.mark.asyncio
    async def test_event_processing_concurrency(self, event_processor, mock_websocket_handler):
...
class TestWebSocketErrorHandling:
    ...
    async def test_memory_cleanup_on_errors(self, websocket_handler):
        ...
        assert persona_id not in websocket_handler.connections or \
               len(websocket_handler.connections[persona_id]) == 0
    ...
    async def test_partial_broadcast_failure(self, websocket_handler):
        ...
        websocket_handler.connections[persona_id] = {mock_ws_good1, mock_ws_good2, mock_ws_bad}
    ...
    async def test_websocket_reconnection_simulation(self, websocket_handler):
        ...
        await websocket_handler.broadcast_variant_update(persona_id, {"test": "reconnection"})
