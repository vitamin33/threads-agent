...
class TestCRA234AcceptanceCriteria:
    ...
    async def test_criteria_2_live_table_er_comparison(self):
        ...
        with patch.object(metrics_api, 'db', mock_db):
            with patch.object(metrics_api, 'get_variant_performance', AsyncMock(return_value={
    ...
    async def test_criteria_4_pattern_fatigue_warnings(self):
        ...
        with patch.object(metrics_api, 'get_fatigue_warnings', AsyncMock(return_value=[
            {
    ...
    async def test_criteria_5_optimization_suggestions(self):
        ...
        # Mock high kill rate
        with patch.object(metrics_api, 'calculate_early_kill_rate', AsyncMock(return_value=0.35)):
            with patch.object(metrics_api, 'get_recent_variants', AsyncMock(return_value=[])):
                suggestions = await metrics_api.get_optimization_suggestions("ai-jesus")
        
        assert len(suggestions) > 0
        kill_rate_suggestion = next(s for s in suggestions if s["type"] == "prediction_calibration")
