...
class TestDashboardE2EComplete:
    ...
    async def test_database(self):
        ...
        test_data = [
            # Active variants
            ('var_e2e_1', 'ai-jesus', 'E2E Test variant 1', 0.065, 0.058, now - timedelta(hours=2), 'active', 150, 2500),
            ('var_e2e_2', 'ai-jesus', 'E2E Test variant 2', 0.052, 0.067, now - timedelta(hours=1), 'active', 180, 2800),
            ('var_e2e_3', 'ai-jesus', 'E2E Test variant 3', 0.045, 0.048, now - timedelta(hours=3), 'active', 95, 2100),
            # Different persona
            ('var_e2e_buddha_1', 'ai-buddha', 'Buddha test variant', 0.055, 0.062, now - timedelta(hours=1), 'active', 140, 2300),
        ]
        ...
        # Insert kill data
        kill_data = [
            ('kill_1', 'var_e2e_killed', 'ai-jesus', 'low_engagement', 0.025, 150, now - timedelta(hours=1)),
            ('kill_2', 'var_e2e_killed_2', 'ai-jesus', 'negative_sentiment', 0.018, 200, now - timedelta(hours=2)),
        ]
        ...
        pattern_data = [
            ('pattern_1', 'ai-jesus', 'curiosity_gap', 15, now - timedelta(hours=1), 0.85),
            ('pattern_2', 'ai-jesus', 'social_proof', 8, now - timedelta(hours=2), 0.42),
        ]
    ...
    @pytest.mark.asyncio
    async def test_complete_dashboard_data_flow(self, test_database, mock_external_services):
        ...
        with patch('services.dashboard_api.variant_metrics.get_db_connection', side_effect=mock_get_db):
            with patch('services.dashboard_api.variant_metrics.get_redis_connection', return_value=mock_external_services['redis']):
                # Create metrics API instance
                metrics_api = VariantMetricsAPI()
                metrics_api.early_kill_monitor = mock_external_services['early_kill_monitor']
                metrics_api.fatigue_detector = mock_external_services['fatigue_detector']
    ...
    @pytest.mark.asyncio
    async def test_websocket_real_time_updates_e2e(self, test_database, mock_external_services):
        ...
        await event_processor.handle_early_kill_event('var_e2e_new_kill', kill_event_data)
    ...
    @pytest.mark.asyncio
    async def test_multi_persona_isolation_e2e(self, test_database, mock_external_services):
    ...
    async def test_websocket_connection_resilience_e2e(self):
        ...
        updates = [
            {'event_type': 'performance_update', 'variant_id': f'var_{i}', 'data': f'update_{i}'}
            for i in range(10)
        ]
    ...
    @pytest.mark.asyncio
    async def test_performance_under_load_e2e(self, test_database, mock_external_services):
        ...
        with patch('services.dashboard_api.variant_metrics.get_db_connection', side_effect=mock_get_db):
            with patch('services.dashboard_api.variant_metrics.get_redis_connection', return_value=mock_external_services['redis']):
                ...
                tasks = [
                    metrics_api.get_live_metrics('ai-jesus')
                    for _ in range(20)
                ]
                ...
                await ws_handler.broadcast_variant_update(persona_id, {'test': 'load_test'})
            ...
            with patch('services.dashboard_api.variant_metrics.get_redis_connection', return_value=mock_external_services['redis']):
                ...
                for initial_variant in initial_data['active_variants']:
                    matching_variant = next(
                        v for v in second_data['active_variants'] 
                        if v['id'] == initial_variant['id']
                    )
                    
                    # Performance metrics should be consistent
                    assert matching_variant['predicted_er'] == initial_variant['predicted_er']
    ...
    async def test_error_recovery_e2e(self, test_database, mock_external_services):
        ...
        with patch('services.dashboard_api.variant_metrics.get_db_connection', side_effect=failing_db_connection):
            metrics_api = VariantMetricsAPI()
        ...
        with patch('services.dashboard_api.variant_metrics.get_db_connection', side_effect=working_db_connection):
            with patch('services.dashboard_api.variant_metrics.get_redis_connection', return_value=mock_external_services['redis']):
                metrics_api = VariantMetricsAPI()
                metrics_api.early_kill_monitor = mock_external_services['early_kill_monitor']
                metrics_api.fatigue_detector = mock_external_services['fatigue_detector']
    ...
    async def test_real_time_event_ordering_e2e(self):
        ...
        events = [
            ('early_kill', 'var_1', {'persona_id': persona_id, 'reason': 'low_engagement', 'killed_at': '2024-01-01T10:00:00Z'}),
            ('performance_update', 'var_2', {'persona_id': persona_id, 'engagement_rate': 0.065, 'updated_at': '2024-01-01T10:01:00Z'}),
            ('early_kill', 'var_3', {'persona_id': persona_id, 'reason': 'negative_sentiment', 'killed_at': '2024-01-01T10:02:00Z'}),
            ('performance_update', 'var_4', {'persona_id': persona_id, 'engagement_rate': 0.078, 'updated_at': '2024-01-01T10:03:00Z'}),
        ]
...
class TestDashboardE2EScenarios:
    ...
    async def test_high_activity_period_e2e(self):
        ...
        for persona_idx, persona_id in enumerate(personas):
            ...
            expected_events = len([e for e in events if e[2]['persona_id'] == persona_id])
    ...
    async def test_dashboard_resilience_under_failures_e2e(self):
        ...
        update_tasks = []
        for i in range(20):
            task = ws_handler.broadcast_variant_update(persona_id, {
                'event_type': 'test',
                'sequence': i
            })
