...
class TestPerformanceCharacteristics:
    ...
    def performance_metrics_api(self):
        ...
        with patch(
            ...
            with patch(
                ...
                api.early_kill_monitor.get_kill_statistics.return_value = {
                    "total_kills_today": 0
                }
    ...
    async def test_large_dataset_performance(self, performance_metrics_api):
        ...
        expected_avg_er = (
            sum(v["live_metrics"]["engagement_rate"] for v in result["active_variants"])
            / 1000
        )
    ...
    async def test_websocket_broadcast_performance(self):
        ...
        await ws_handler.broadcast_variant_update(
            persona_id, {"event_type": "performance_test", "data": "test_payload"}
        )
    ...
    @pytest.mark.asyncio
    async def test_cpu_intensive_calculations_performance(
        self, performance_metrics_api
    ):
        ...
        for i in range(10000):
            ...
            delta = performance_metrics_api.calculate_performance_delta(
                actual_er, predicted_er
            )
...
class TestStressTests:
    ...
    async def test_connection_flood_stress(self):
        ...
        broadcast_tasks = [
            ws_handler.broadcast_variant_update(
                f"stress-persona-{i}", {"stress": "test"}
            )
    ...
    async def test_concurrent_stress_with_failures(self):
        ...
        for persona_connections in ws_handler.connections.values():
            assert (
                len(persona_connections) <= 10
            )  # Some unreliable connections should be removed
