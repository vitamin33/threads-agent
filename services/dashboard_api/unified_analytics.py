...
class AnalyticsAggregationService:
    ...
    async def calculate_conversion_summary(
        ...
        overall_conversion_rate = (
            (total_inquiries / total_visits * 100) if total_visits > 0 else 0.0
        )
        ...
        return {
            ...
            "best_converting_platform": await self.identify_best_platform(
                platform_metrics
            ),
            "platform_breakdown": {
                platform: {
                    "serbyn_pro_visits": data.get("conversion_data", {}).get(
                        "serbyn_pro_visits", 0
                    ),
                    "job_inquiries": data.get("conversion_data", {}).get(
                        "job_inquiries", 0
                    ),
                    "conversion_rate": (
                        data.get("conversion_data", {}).get("job_inquiries", 0)
                        / data.get("conversion_data", {}).get("serbyn_pro_visits", 1)
                        * 100
                    )
                    if data.get("conversion_data", {}).get("serbyn_pro_visits", 0) > 0
                    else 0.0,
                }
                for platform, data in platform_metrics.items()
            },
    ...
    async def get_platform_ranking(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "criteria": ["conversion_rate", "engagement_rate", "reach", "lead_quality"],
        }
...
class ConversionTracker:
    ...
    async def track_conversion(
        self, conversion_event: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def track_lead_conversion(self, lead_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if source_conversion_id and source_conversion_id in self.conversion_store:
            attributed_platform = self.conversion_store[source_conversion_id].get(
