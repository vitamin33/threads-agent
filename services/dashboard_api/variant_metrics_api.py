    ...
    def get_comprehensive_metrics(self) -> List[Dict[str, Any]]:
        ...
        for variant in variants:
            ...
            if self.early_kill_monitor:
                is_monitored = variant.variant_id in getattr(
                    self.early_kill_monitor, "active_sessions", {}
                )
                variant_data["early_kill_status"] = (
                    "monitoring" if is_monitored else "not_monitored"
                )
            else:
            ...
            if (
                self.pattern_fatigue_detector
                and hasattr(variant, "dimensions")
                ...
                if pattern:
                    ...
                    is_fatigued = self.pattern_fatigue_detector.is_pattern_fatigued(
                        pattern, persona_id
                    )
                    freshness_score = self.pattern_fatigue_detector.get_freshness_score(
                        pattern, persona_id
                    )
