...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'tech_doc_generator', 'app', 'services'))
...
async def demo_platform_collectors():
    ...
    for platform_name, collector in collectors:
        ...
        if platform_name == "LinkedIn":
            ...
            print(f"  AI hiring manager connections: {metrics['ai_hiring_manager_connections']}")
...
async def demo_unified_analytics():
    ...
    print("\n💼 Conversion Summary:")
    conversion_summary = await analytics_service.calculate_conversion_summary(all_metrics)
    ...
    print(f"  Overall conversion rate: {conversion_summary['overall_conversion_rate']:.2f}%")
    print(f"  Best converting platform: {conversion_summary['best_converting_platform']}")
...
async def demo_conversion_tracking():
    ...
    print(f"  ⏱️  Total conversion time: {attribution_chain['total_conversion_time_hours']} hours")
...
async def demo_api_endpoints():
    ...
    endpoints = [
        ...
        ("GET /api/analytics/websocket-info", "Get WebSocket connection info for real-time updates"),
    ]
...
async def main():
    ...
    print("🎯 Goal: Track conversion from content engagement to serbyn.pro visits and job inquiries")
    print("")
