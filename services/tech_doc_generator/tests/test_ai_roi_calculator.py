...
class TestAIROICalculator:
    ...
    async def test_company_size_impact(self, ai_roi_calculator, sample_roi_input):
        ...
        timeline_lengths = [
            len(result.implementation_timeline) for _, result in results
        ]
    ...
    async def test_revenue_impact_inclusion(self, ai_roi_calculator, sample_roi_input):
        ...
        assert (
            result_with_revenue.metrics.roi_percentage
            > result_no_revenue.metrics.roi_percentage
        )
        assert (
            result_with_revenue.metrics.total_benefits_annual
            > result_no_revenue.metrics.total_benefits_annual
        )
...
class TestROIInsightsAndRecommendations:
    ...
    async def test_recommendations_quality(self, ai_roi_calculator, sample_roi_input):
        ...
        assert any(
            action_word in recommendations_text
    ...
    async def test_risk_factors_identification(
        self, ai_roi_calculator, sample_roi_input
    ):
    ...
    async def test_implementation_timeline(self, ai_roi_calculator, sample_roi_input):
        ...
        assert any(
            time_word in timeline_text for time_word in ["week", "month", "phase"]
        )
