...
class TestContentScheduler:
    ...
    async def test_create_weekly_schedule(self, mock_achievement_data):
        ...
        with patch.object(
            scheduler.achievement_client,
            "__aenter__",
            return_value=scheduler.achievement_client,
        ):
            with patch.object(
                scheduler.achievement_client, "__aexit__", return_value=None
            ):
                with patch.object(
                    scheduler.achievement_client, "get_recent_highlights"
                ) as mock_recent:
                    with patch.object(
                        scheduler.achievement_client, "get_company_targeted"
                    ) as mock_targeted:
                        # Setup mocks to return achievement objects
                        from app.clients.achievement_client import Achievement

                        mock_achievements = [
                            Achievement(**data) for data in mock_achievement_data
                        ]

                        mock_recent.return_value = mock_achievements
                        mock_targeted.return_value = mock_achievements[
                            :1
                        ]  # Return 1 per company
                        ...
                        for entry in plan.scheduled_entries:
                            ...
                            assert entry.platform in [
                                Platform.LINKEDIN,
                                Platform.MEDIUM,
                                Platform.DEVTO,
                            ]
                            assert entry.content_type in [
    ...
    async def test_generate_scheduled_content(
        self, mock_achievement_data, mock_generated_content
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    scheduler.achievement_client, "get_achievement"
                ) as mock_get_achievement:
                    with patch.object(
                        scheduler.content_engine,
                        "__aenter__",
                        return_value=scheduler.content_engine,
                    ):
                        with patch.object(
                            scheduler.content_engine, "__aexit__", return_value=None
                        ):
                            with patch.object(
                                scheduler.content_engine,
                                ...
                                mock_get_achievement.return_value = Achievement(
                                    **mock_achievement_data[0]
                                )
                                mock_generate.return_value = mock_generated_content

                                # Generate content
                                success = await scheduler.generate_scheduled_content(
                                    entry
                                )
    ...
    async def test_process_weekly_schedule(
        self, mock_achievement_data, mock_generated_content
    ):
...
class TestContentSchedulerIntegration:
    ...
    async def test_real_achievement_integration(self):
        ...
        plan = await scheduler.create_weekly_schedule(target_companies=["anthropic"])
...
class TestContentSchedulerPerformance:
    ...
    async def test_batch_content_generation_performance(self, mock_achievement_data):
        ...
        with patch.object(
            scheduler, "generate_scheduled_content", return_value=True
        ) as mock_generate:
