...
class TestContentGenerator:
    ...
    def test_content_generator_initialization_with_settings(self, mock_settings):
        ...
        """
        with patch(
            "app.services.content_generator.get_settings", return_value=mock_settings
        ):
            with patch(
                "app.services.content_generator.openai.AsyncOpenAI"
            ) as mock_openai:
            ...
            with patch("app.services.content_generator.openai.AsyncOpenAI"):
                ...
                for style_name, style_config in generator.content_styles.items():
                    for field in required_fields:
                        assert field in style_config, (
                            f"Style {style_name} missing {field}"
                        )
    ...
    @pytest.mark.asyncio
    async def test_generate_article_basic_flow_calls_required_methods(
        ...
        with patch(
            ...
            with patch("app.services.content_generator.openai.AsyncOpenAI"):
                ...
                with patch.object(
                    generator,
                    "_select_optimal_style",
                    return_value="technical_deep_dive",
                ):
                    with patch.object(
                        generator, "_generate_content_angles", return_value=mock_angles
                    ):
                        with patch.object(
                            generator, "_select_best_angle", return_value=mock_angles[0]
                        ):
                            with patch.object(
                                generator,
    ...
    @pytest.mark.asyncio
    async def test_generate_content_angles_creates_structured_angles(
        self, mock_settings, sample_code_analysis
    ):
        """
    ...
    def test_extract_section_finds_content_after_marker(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.content_generator.openai.AsyncOpenAI"):
                ...
                assert (
                    "This is the hook content that continues on next line"
                    in hook_content
                )
    ...
    async def test_select_best_angle_scores_and_returns_highest(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.content_generator.openai.AsyncOpenAI"):
                ...
                with patch.object(
                    generator, "_score_angle", side_effect=[7.5, 8.5, 6.0]
                ):
                    best_angle = await generator._select_best_angle(
                        angles, Platform.DEVTO
                    )
    ...
    def test_extract_code_examples_processes_function_data(
        self, mock_settings, sample_code_analysis
    ):
        """
    ...
    def test_generate_tags_creates_relevant_tags(
        self, mock_settings, sample_code_analysis
    ):
        """
        ...
        with patch(
            ...
            with patch("app.services.content_generator.openai.AsyncOpenAI"):
                ...
                assert any(tag in expected_tags for tag in tags), (
                    f"No expected tags found in {tags}"
                )
    ...
    def test_select_optimal_style_chooses_appropriate_style(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.content_generator.openai.AsyncOpenAI"):
                ...
                style1 = generator._select_optimal_style(
                    ArticleType.ARCHITECTURE, Platform.DEVTO
                )
                assert style1 == "technical_deep_dive"

                style2 = generator._select_optimal_style(
                    ArticleType.ARCHITECTURE, Platform.LINKEDIN
                )
                assert style2 == "architecture_showcase"

                style3 = generator._select_optimal_style(
                    ArticleType.TUTORIAL, Platform.DEVTO
                )
                assert style3 == "learning_journey"

                # Test fallback for unknown combination
                style4 = generator._select_optimal_style(
                    ArticleType.DEEP_DIVE, Platform.GITHUB
                )
    ...
    def test_adapt_tags_for_platform_respects_limits(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.content_generator.openai.AsyncOpenAI"):
                ...
                devto_tags = generator._adapt_tags_for_platform(
                    many_tags, Platform.DEVTO
                )
                assert len(devto_tags) <= 4

                linkedin_tags = generator._adapt_tags_for_platform(
                    many_tags, Platform.LINKEDIN
                )
                assert len(linkedin_tags) <= 10

                twitter_tags = generator._adapt_tags_for_platform(
                    many_tags, Platform.TWITTER
                )
