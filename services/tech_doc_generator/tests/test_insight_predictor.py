...
class TestInsightPredictor:
    ...
    def test_insight_predictor_initialization_sets_up_components(self, mock_settings):
        ...
        """
        with patch(
            "app.services.insight_predictor.get_settings", return_value=mock_settings
        ):
            with patch(
                "app.services.insight_predictor.openai.AsyncOpenAI"
            ) as mock_openai:
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                for category in expected_categories:
                    ...
                    assert all(
                        ...
                        for keyword in predictor.trending_keywords[category]
                    )

    @pytest.mark.asyncio
    async def test_predict_insight_quality_returns_complete_score(
        self, mock_settings, sample_article_content, sample_code_analysis
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_extract_features_creates_comprehensive_feature_set(
        self, mock_settings, sample_article_content, sample_code_analysis
    ):
        """
...
- Monitor everything!
    ...
    def test_extract_structure_features_analyzes_content_structure(
        self, mock_settings, sample_article_content
    ):
        """
    ...
    def test_extract_code_features_processes_analysis_data(
        self, mock_settings, sample_code_analysis
    ):
        """
    ...
    async def test_predict_technical_depth_uses_multiple_factors(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                high_score = await predictor._predict_technical_depth(
                    high_tech_features
                )
    ...
    def test_calculate_readability_processes_text_correctly(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                assert (
                    technical_score >= 1.0
                )  # At minimum should get some technical adjustment
    ...
    async def test_predict_engagement_considers_multiple_factors(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                high_score = await predictor._predict_engagement(
                    engaging_features, Platform.LINKEDIN
                )
                ...
                low_score = await predictor._predict_engagement(
                    boring_features, Platform.LINKEDIN
                )
    ...
    def test_calculate_trend_alignment_scores_trending_content(
        self, mock_settings, sample_article_content
    ):
        """
        ...
        with patch(
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                trending_score = predictor._calculate_trend_alignment(
                    trending_content, Platform.DEVTO
                )
                ...
                boring_score = predictor._calculate_trend_alignment(
                    boring_content, Platform.DEVTO
                )
    ...
    def test_calculate_confidence_reflects_feature_completeness(self, mock_settings):
        ...
        with patch(
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                # Minimal features
                minimal_features = {"word_count": 200, "title_length": 3}
    ...
    @pytest.mark.asyncio
    async def test_get_improvement_suggestions_provides_actionable_advice(
        self, mock_settings, sample_article_content
    ):
        """
        ...
        with patch(
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                suggestions = await predictor.get_improvement_suggestions(
                    sample_article_content, low_score
                )
                ...
                assert any(
                    "technical" in suggestion.lower() for suggestion in suggestions
                )
                assert any("unique" in suggestion.lower() for suggestion in suggestions)

    @pytest.mark.asyncio
    async def test_update_model_with_feedback_logs_performance_data(
        self, mock_settings
    ):
        """
        ...
        with patch(
            ...
            with patch("app.services.insight_predictor.openai.AsyncOpenAI"):
                ...
                actual_metrics = {"engagement_rate": 0.06, "views": 1500, "shares": 45}
