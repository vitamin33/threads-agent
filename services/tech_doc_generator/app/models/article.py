...
class InsightScore(BaseModel):
    ...
    technical_depth: float = Field(
        ..., ge=0, le=10, description="Technical complexity and depth"
    )
    uniqueness: float = Field(
        ..., ge=0, le=10, description="How unique/novel the insights are"
    )
    readability: float = Field(
        ..., ge=0, le=10, description="How accessible the content is"
    )
    engagement_potential: float = Field(
        ..., ge=0, le=10, description="Predicted engagement"
    )
    trend_alignment: float = Field(
        ..., ge=0, le=10, description="Alignment with current trends"
    )
    overall_score: float = Field(
        ..., ge=0, le=10, description="Overall insight quality score"
    )
...
class CodeAnalysis(BaseModel):
    ...
    patterns: List[str] = Field(
        default_factory=list, description="Architectural patterns found"
    )
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Key dependencies"
    )
...
class ArticleRequest(BaseModel):
    ...
    source_path: str = Field(
        ..., description="Path to analyze (file, directory, commit hash)"
    )
    angles: List[ArticleType] = Field(
        default=[ArticleType.ARCHITECTURE], description="Article types to generate"
    )
    ...
    title_hint: Optional[str] = Field(
        None, description="Suggested title or topic focus"
    )
    custom_instructions: Optional[str] = Field(
        None, description="Custom generation instructions"
    )
    auto_publish: bool = Field(
        False, description="Whether to auto-publish after generation"
    )
