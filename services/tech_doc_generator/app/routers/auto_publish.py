...
async def publish_achievement_content(
    ...
    platforms: Optional[List[Platform]] = Query(
        default=[Platform.DEVTO],
        description="Platforms to publish to"
    ),
    test_mode: bool = Query(
        default=True,
        description="If true, publishes as draft on dev.to"
    ),
    days_lookback: int = Query(
        default=7,
        description="Number of days to look back for achievements"
    ),
    min_business_value: float = Query(
        default=50000,
        description="Minimum business value threshold"
    )
) -> Dict[str, Any]:
    """
...
async def preview_achievement_content(
    ...
    try:
        ...
        return {
            ...
            "total_business_value": sum(a.get("business_value", 0) for a in achievements)
        }
...
async def schedule_daily_publication(
    ...
    hour: int = Query(default=10, ge=0, le=23, description="Hour of day to publish (UTC)"),
