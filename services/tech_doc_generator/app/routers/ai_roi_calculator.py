...
class CalculateROIRequest(BaseModel):
    ...
    company_size: str = Field(
        ...,
        regex="^(startup|small|medium|large|enterprise)$",
        description="Company size category",
    )
    current_monthly_hours: float = Field(
        ..., ge=0, le=1000, description="Current monthly hours spent on task"
    )
    hourly_cost: float = Field(
        ..., ge=10, le=500, description="Cost per hour (salary + overhead)"
    )
    ai_monthly_cost: float = Field(
        ..., ge=0, le=100000, description="Monthly cost of AI solution"
    )
    implementation_cost: float = Field(
        ..., ge=0, le=1000000, description="One-time implementation cost"
    )
    expected_efficiency_gain: float = Field(
        ..., ge=0.1, le=0.95, description="Expected efficiency improvement (0.1 = 10%)"
    )
    revenue_impact: Optional[float] = Field(
        None, ge=0, le=1000000, description="Optional monthly revenue increase"
    )
    quality_improvement: Optional[float] = Field(
        None, ge=0, le=1.0, description="Quality score improvement"
    )
    time_horizon_months: int = Field(
        12, ge=6, le=60, description="ROI calculation period in months"
    )
...
async def calculate_ai_roi(
    request: CalculateROIRequest, background_tasks: BackgroundTasks
) -> ROIResponse:
    """
    ...
    try:
        ...
        return ROIResponse(
            ...
            benchmark_comparison={
                ...
                "your_vs_average": {
                    "roi_comparison": result.metrics.roi_percentage
                    - result.benchmark.average_roi,
                    "payback_comparison": result.benchmark.typical_payback_months
        ...
        logger.error(
            "roi_calculation_failed", error=str(e), request_data=request.dict()
        )
...
async def get_industry_benchmark(industry: IndustryType) -> IndustryBenchmarkResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch benchmark: {str(e)}"
        )
...
async def get_use_case_example(use_case: AIUseCase) -> UseCaseExampleResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch use case example: {str(e)}"
        )


@router.post("/request-consultation", response_model=Dict[str, Any])
async def request_consultation(
    request: LeadCaptureRequest, background_tasks: BackgroundTasks
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to process consultation request: {str(e)}"
        )
...
async def process_consultation_request(
    ...
    try:
        ...
        logger.info(
            "consultation_request_processed",
            report_id=report_id,
            company=company_name,
            email=contact_email,
        ...
        logger.error(
            "consultation_processing_failed", report_id=report_id, error=str(e)
        )
