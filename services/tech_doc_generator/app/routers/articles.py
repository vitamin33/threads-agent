...
async def generate_article(request: ArticleRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        article = Article(id=article_id, request=request, status=ArticleStatus.DRAFT)

        # Analyze code
        analysis = await code_analyzer.analyze_source(
            request.source_type, request.source_path
        )
        ...
        if request.auto_publish and best_score >= settings.min_insight_score:
            background_tasks.add_task(
                auto_publish_article, article_id, request.target_platforms
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Article generation failed: {str(e)}"
        )
...
async def publish_article(article_id: str, publish_request: PublishRequest):
    ...
    try:
        ...
        for platform in publish_request.platforms:
            custom_content = (
                publish_request.custom_content.get(platform)
                if publish_request.custom_content
                else None
            )
        ...
        logger.info(
            "Article publish attempt completed", article_id=article_id, results=results
        )

        return {"article_id": article_id, "results": results, "status": article.status}
...
async def predict_article_quality(request: PredictionRequest):
    ...
    try:
        ...
        content = ArticleContent(
            title=request.title, content=request.content, tags=request.tags
        )
...
async def get_improvement_suggestions(article_id: str):
    ...
    if not article.content or not article.insight_score:
        raise HTTPException(
            status_code=400, detail="Article needs content and quality score"
        )
    ...
    try:
        ...
        logger.error(
            "Suggestions generation failed", article_id=article_id, error=str(e)
        )
...
async def provide_feedback(article_id: str, feedback: dict):
    ...
    if not article.insight_score:
        raise HTTPException(
            status_code=400, detail="Article has no prediction to update"
        )
...
async def auto_publish_article(article_id: str, platforms: List):
    ...
    try:
        if article_id not in articles_db:
            logger.error(
                "Auto-publish failed: article not found", article_id=article_id
            )
            return
