...
async def generate_from_achievement(
    request: AchievementArticleRequest, background_tasks: BackgroundTasks
):
    """
    ...
    try:
        logger.info(
            "generating_articles_from_achievement",
        ...
        content_list = await achievement_generator.generate_from_achievement(
            request.achievement_id, request.article_types, request.platforms
        )
        ...
        logger.error(
            "achievement_article_generation_failed",
            achievement_id=request.achievement_id,
            error=str(e),
        )
...
async def generate_weekly_highlights(
    request: WeeklyHighlightsRequest, background_tasks: BackgroundTasks
):
    """
    ...
    try:
        ...
        for idx, content in enumerate(content_list):
            score = await insight_predictor.predict_insight_quality(
                content, {"type": "weekly_highlight", "index": idx}, content.platform
            )
...
async def generate_company_content(request: CompanyContentRequest):
    ...
    try:
        ...
        logger.error(
            "company_content_failed", company=request.company_name, error=str(e)
        )
        raise HTTPException(status_code=500, detail=str(e))
