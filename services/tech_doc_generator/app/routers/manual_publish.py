...
async def _create_devto_achievement(tracking_data: Dict[str, Any], db: Session):
    ...
    if any(
        tag in ["ai", "machine-learning", "python", "devops"] for tag in article_tags
    ):
    ...
    if any(
        indicator in tracking_data["title"].lower() for indicator in tech_indicators
    ):
...
async def get_devto_analytics(tracking_id: str, db: Session = Depends(get_db)):
