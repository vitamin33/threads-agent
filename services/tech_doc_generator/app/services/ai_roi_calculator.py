...
class AIROICalculator:
    ...
    async def calculate_roi(self, input_data: ROIInput) -> ROIResult:
        ...
        recommendations = await self._generate_recommendations(input_data, metrics, benchmark)
        ...
        success_probability = await self._calculate_success_probability(input_data, metrics, benchmark)
    ...
    async def _calculate_core_metrics(self, input_data: ROIInput) -> ROIMetrics:
        ...
        effective_efficiency_gain = min(0.95, input_data.expected_efficiency_gain * use_case_multiplier)
        
        # Calculate time and cost savings
        monthly_time_savings_hours = input_data.current_monthly_hours * effective_efficiency_gain
        ...
        total_costs_annual = (input_data.ai_monthly_cost * 12) + input_data.implementation_cost
        ...
        for month in range(1, input_data.time_horizon_months + 1):
            monthly_benefit = net_monthly_savings + (revenue_impact_annual / 12 if input_data.revenue_impact else 0)
            discounted_benefit = monthly_benefit / ((1 + discount_rate/12) ** month)
            npv += discounted_benefit
        npv -= input_data.implementation_cost
        
        # Break-even month
        break_even_month = int(payback_period_months) + 1 if payback_period_months != float('inf') else 0
    ...
    async def _generate_insights(self, input_data: ROIInput, metrics: ROIMetrics, benchmark: IndustryBenchmark) -> List[str]:
        ...
        if metrics.roi_percentage > benchmark.average_roi * 1.2:
            insights.append(f"🎯 Exceptional ROI potential: {metrics.roi_percentage:.1f}% vs industry average of {benchmark.average_roi:.1f}%")
        elif metrics.roi_percentage > benchmark.average_roi:
            insights.append(f"📈 Above-average ROI: {metrics.roi_percentage:.1f}% vs industry average of {benchmark.average_roi:.1f}%")
        else:
            insights.append(f"⚠️ Below-average ROI: {metrics.roi_percentage:.1f}% vs industry average of {benchmark.average_roi:.1f}%")
        
        # Payback period analysis
        if metrics.payback_period_months < benchmark.typical_payback_months:
            insights.append(f"⚡ Fast payback: {metrics.payback_period_months:.1f} months vs typical {benchmark.typical_payback_months:.1f} months")
        else:
            insights.append(f"⏰ Extended payback period: {metrics.payback_period_months:.1f} months vs typical {benchmark.typical_payback_months:.1f} months")
        
        # Time savings impact
        if metrics.monthly_time_savings_hours > 40:
            insights.append(f"🚀 Significant time savings: {metrics.monthly_time_savings_hours:.1f} hours/month freed for strategic work")
        elif metrics.monthly_time_savings_hours > 20:
            insights.append(f"⏱️ Moderate time savings: {metrics.monthly_time_savings_hours:.1f} hours/month for higher-value activities")
        
        # Cost efficiency
        if metrics.annual_cost_savings > 100000:
            insights.append(f"💰 Major cost impact: ${metrics.annual_cost_savings:,.0f} annual savings")
        elif metrics.annual_cost_savings > 25000:
            insights.append(f"💵 Solid cost savings: ${metrics.annual_cost_savings:,.0f} annually")
        
        # Success probability
        if metrics.roi_percentage > benchmark.average_roi and metrics.payback_period_months < benchmark.typical_payback_months:
            insights.append("✅ High success probability based on industry benchmarks and calculated metrics")
        
        return insights
    
    async def _generate_recommendations(self, input_data: ROIInput, metrics: ROIMetrics, benchmark: IndustryBenchmark) -> List[str]:
        """Generate actionable recommendations"""
        recommendations = []
        
        # Start with benchmark best practices
        recommendations.extend([f"• {practice}" for practice in benchmark.best_practices[:2]])
        ...
        if metrics.roi_percentage < 100:
            ...
            recommendations.append("• Consider combining multiple AI use cases for compound benefits")
        
        # Use case specific recommendations
        if input_data.use_case == AIUseCase.CONTENT_GENERATION:
            recommendations.append("• Start with content templates and gradually expand to full automation")
            recommendations.append("• Maintain quality control processes and brand guidelines")
            ...
            recommendations.append("• Ensure data quality and governance before implementation")
            recommendations.append("• Focus on reporting automation and insight generation")
        ...
        if input_data.company_size in ["startup", "small"]:
            ...
            recommendations.append("• Partner with AI vendors for implementation support")
        elif input_data.company_size in ["large", "enterprise"]:
            recommendations.append("• Evaluate build vs. buy decisions carefully")
            recommendations.append("• Establish center of excellence for AI initiatives")
        
        return recommendations[:6]  # Limit to top 6 recommendations
    
    async def _identify_risk_factors(self, input_data: ROIInput, metrics: ROIMetrics) -> List[str]:
        ...
        if input_data.implementation_cost > input_data.current_monthly_hours * input_data.hourly_cost * 6:
    ...
    async def _calculate_success_probability(self, input_data: ROIInput, metrics: ROIMetrics, benchmark: IndustryBenchmark) -> float:
