...
class PlatformPublisher:
    ...
    async def _publish_to_devto(self, content: ArticleContent, custom_content: Optional[str] = None) -> Dict[str, Any]:
        """Publish to Dev.to"""
        ...
        try:
            ...
            logger.error("Dev.to publishing failed", error=str(e), response=e.response.text)
    ...
    async def _publish_to_twitter(self, content: ArticleContent, custom_content: Optional[str] = None) -> Dict[str, Any]:
        ...
        tweets = custom_content.split("\n\n") if custom_content else self._format_for_twitter(content)
    ...
    async def _publish_to_github(self, content: ArticleContent, custom_content: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _publish_to_medium(self, content: ArticleContent, custom_content: Optional[str] = None) -> Dict[str, Any]:
        """Publish to Medium"""
    ...
    def _format_for_devto(self, content: ArticleContent) -> str:
        ...
        if content.code_examples:
            ...
            for i, example in enumerate(content.code_examples, 1):
                formatted += f"### Example {i}: {example.get('title', 'Code Sample')}\n\n"
        ...
        formatted += "Check out my [projects](https://github.com/vitamin33/threads-agent) or "
    ...
    def _format_for_linkedin(self, content: ArticleContent) -> str:
        ...
        # Add hashtags
        if content.tags:
            hashtags = [f"#{tag.replace(' ', '').replace('-', '')}" for tag in content.tags[:5]]
    ...
    def _format_for_twitter(self, content: ArticleContent) -> List[str]:
        ...
        if content.code_examples:
            ...
            code = content.code_examples[0]['code'][:150] + "..." if len(content.code_examples[0]['code']) > 150 else content.code_examples[0]['code']
            code_tweet += f"```\n{code}\n```"
    ...
    async def _publish_to_threads(self, content: ArticleContent, custom_content: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _format_for_threads(self, content: ArticleContent) -> str:
        ...
        if content.content:
            ...
            for sentence in sentences:
                if any(word in sentence.lower() for word in ['built', 'discovered', 'learned', 'found', 'achieved']):
                    text += f"\n\n{sentence.strip()}."
                    break
...
class PublishingScheduler:
    ...
    def get_optimal_posting_times(self, platforms: List[Platform]) -> Dict[Platform, datetime]:
