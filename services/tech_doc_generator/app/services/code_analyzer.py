...
from ..core.cache import get_cache_manager, cached_operation
...
class CodeAnalyzer:
    ...
    def __init__(self, repo_path: str):
        ...
        self.repo = git.Repo(repo_path) if os.path.exists(os.path.join(repo_path, '.git')) else None
        self.cache = get_cache_manager()
        
    async def analyze_source(self, source_type: SourceType, source_path: str) -> CodeAnalysis:
        """Main entry point for code analysis"""
        logger.info("Starting code analysis", source_type=source_type, source_path=source_path)
    ...
    async def _analyze_repository(self) -> CodeAnalysis:
        ...
        cached_analysis = await self.cache.get_cached_result("code_analysis", **cache_key_data)
        if cached_analysis:
        ...
        # Cache the result
        await self.cache.cache_result("code_analysis", analysis.dict(), **cache_key_data)
    ...
    async def _analyze_pr(self, pr_identifier: str) -> CodeAnalysis:
        ...
        return CodeAnalysis(
            patterns=["pr_analysis"],
            metrics={"pr_id": pr_identifier}
        )
    ...
    async def _calculate_complexity_score(self) -> float:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True, text=True, timeout=15  # Reduced timeout
            )
    ...
    async def _extract_functions_from_file(self, file_path: Path) -> List[Dict[str, Any]]:
    ...
    async def _find_functions_in_directory(self, directory: Path) -> List[Dict[str, Any]]:
