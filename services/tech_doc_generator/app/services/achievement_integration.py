...
class AchievementIntegration:
    ...
    async def auto_publish_achievement_content(
        ...
        try:
            ...
            if score < 7.0 and not test_mode:
                ...
                results["errors"].append(f"Content quality score {score:.1f} below threshold")
                return results
    ...
    def _extract_theme(self, achievement: Dict[str, Any]) -> str:
        ...
        if any(word in title + description for word in ["mlops", "pipeline", "ci/cd", "deployment"]):
            return "MLOps & Infrastructure"
        elif any(word in title + description for word in ["ai", "llm", "model", "ml", "machine learning"]):
            return "AI/ML Development"
        elif any(word in title + description for word in ["monitoring", "metrics", "observability", "grafana"]):
            return "Monitoring & Observability"
        elif any(word in title + description for word in ["api", "microservice", "backend", "fastapi"]):
            return "API & Microservices"
        elif any(word in title + description for word in ["performance", "optimization", "speed", "latency"]):
    ...
    def _select_best_theme(
        ...
        for theme, achievements in grouped.items():
            ...
            recency_bonus = len([a for a in achievements if self._is_recent(a, days=7)]) * 10
    ...
    def _create_article_prompt(
        ...
        achievements = sorted(
            achievements, 
            key=lambda x: x.get("business_value", 0), 
            reverse=True
        )[:5]  # Top 5
...
"""
    ...
    def _extract_technologies(self, achievements: List[Dict[str, Any]]) -> List[str]:
        ...
        tech_keywords = {
            ...
            "redis", "rabbitmq", "celery", "prometheus", "grafana",
            "langchain", "openai", "gpt", "react", "typescript",
            "github", "ci/cd", "mlops", "terraform", "aws"
        }
