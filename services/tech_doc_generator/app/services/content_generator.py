...
class ContentGenerator:
    ...
    async def _cached_openai_call(
        ...
        cached_response = await self.cache.get_cached_result(
            "openai_response", **cache_key_data
        )
        if cached_response:
        ...
        try:
            result = await resilient_api_call(_make_openai_call, service_name="openai")
    ...
    async def generate_article(
        ...
        style = style_preference or self._select_optimal_style(
            article_type, target_platform
        )
        ...
        content = await self._generate_full_content(
            analysis, best_angle, style, target_platform
        )

        return content

    async def _generate_content_angles(
        self, analysis: CodeAnalysis, article_type: ArticleType, style: str
    ) -> List[Dict[str, Any]]:
        ...
        user_prompt = f"""Create 3 varied content angles for a {
            article_type.value
        } article.
    ...
    async def _select_best_angle(
        self, angles: List[Dict[str, Any]], platform: Platform
    ) -> Dict[str, Any]:
        ...
        logger.info(
            ...
            score=max(scored_angles, key=lambda x: x[1])[1],
        )
    ...
    async def _generate_full_content(
        ...
        content_text = await self._generate_main_content(
            analysis, angle, style_config, platform
        )
        ...
        return ArticleContent(
            ...
            insights=insights,
            platform_specific=self._generate_platform_specific_content(
                platform, content_text
            ),
        )

    async def _generate_titles(
        self, angle: Dict[str, Any], platform: Platform
    ) -> List[str]:
    ...
    async def _generate_key_insights(
        self, analysis: CodeAnalysis, angle: Dict[str, Any]
    ) -> List[str]:
    ...
    def _generate_platform_specific_content(
        self, platform: Platform, content: str
        ...
        if platform == Platform.DEVTO:
            ...
            platform_content[platform] = {"post_type": "article", "include_pdf": False}
    ...
    def _select_optimal_style(
        self, article_type: ArticleType, platform: Platform
    ) -> str:
    ...
    async def generate_content_variations(
        self, base_content: ArticleContent, target_platforms: List[Platform]
    ...
    async def _adapt_content_for_platform(
        self, base_content: ArticleContent, target_platform: Platform
    ) -> ArticleContent:
        ...
        adapted_content = ArticleContent(
            ...
            platform_specific=self._generate_platform_specific_content(
                target_platform, base_content.content
            ),
        )

        return adapted_content

    def _adapt_tags_for_platform(
        self, tags: List[str], platform: Platform
    ) -> List[str]:
