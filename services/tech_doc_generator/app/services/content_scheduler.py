...
class WeeklyContentPlan(BaseModel):
    ...
    week_start: datetime
    target_posts: int = 3
    platforms: List[Platform] = Field(default_factory=lambda: [Platform.LINKEDIN, Platform.MEDIUM, Platform.DEVTO])
    target_companies: List[str] = Field(default_factory=lambda: ["anthropic", "notion", "stripe"])
    content_types: List[ContentType] = Field(default_factory=lambda: [ContentType.CASE_STUDY, ContentType.LINKEDIN_POST, ContentType.TECHNICAL_BLOG])
...
class ContentScheduler:
    ...
    async def create_weekly_schedule(
        ...
        if not target_companies:
            ...
            target_companies = self.company_rotation[week_number % len(self.company_rotation)]
        
        platforms = custom_platforms or [Platform.LINKEDIN, Platform.MEDIUM, Platform.DEVTO]
        
        logger.info("creating_weekly_schedule", 
        ...
        if len(achievements) < 3:
            logger.warning("insufficient_achievements", 
                          found=len(achievements), 
                          required=3)
        ...
        logger.info("weekly_schedule_created", 
                   plan_id=plan_id,
    ...
    async def _select_weekly_achievements(self, target_companies: List[str]) -> List[Dict[str, Any]]:
        ...
        async with self.achievement_client:
            ...
            for company in target_companies:
                company_achievements = await self.achievement_client.get_company_targeted(
    ...
    def _ensure_category_variety(self, achievements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _create_schedule_entries(
        ...
        for i, (day, platform, content_type) in enumerate(schedule_template):
            ...
            company = target_companies[i % len(target_companies)] if target_companies else None
    ...
    async def generate_scheduled_content(self, entry: ContentScheduleEntry) -> bool:
        ...
        try:
            logger.info("generating_scheduled_content", 
                       entry_id=entry.id,
            ...
            async with self.achievement_client:
                achievement = await self.achievement_client.get_achievement(entry.achievement_id)
                if not achievement:
                    logger.error("achievement_not_found", achievement_id=entry.achievement_id)
                    entry.status = "failed"
                    return False
            ...
            if result.engagement_score < 60.0 or result.quality_score < 70.0:
                logger.warning("content_quality_below_threshold",
                              entry_id=entry.id,
            ...
            logger.info("content_generation_successful",
                       entry_id=entry.id,
                       engagement_score=result.engagement_score,
                       quality_score=result.quality_score)
    ...
    async def process_weekly_schedule(self, plan_id: str) -> Dict[str, Any]:
        ...
        logger.info("processing_weekly_schedule", 
                   plan_id=plan_id,
        ...
        for entry in plan.scheduled_entries:
            ...
            results["entries"].append({
                ...
                "engagement_score": entry.generated_content.engagement_score if entry.generated_content else None
            })
        
        logger.info("weekly_schedule_processed",
                   plan_id=plan_id,
                   results=results)
            ...
            for entry in plan.scheduled_entries:
                if entry.generated_content:
                    ...
                    if platform not in platform_breakdown:
                        platform_breakdown[platform] = {"count": 0, "avg_engagement": 0.0}
                    platform_breakdown[platform]["count"] += 1
                    platform_breakdown[platform]["avg_engagement"] += entry.generated_content.engagement_score
                    
                    # Company performance
                    if entry.target_company:
                        company = entry.target_company
                        if company not in company_performance:
                            company_performance[company] = {"count": 0, "avg_engagement": 0.0}
                        company_performance[company]["count"] += 1
                        company_performance[company]["avg_engagement"] += entry.generated_content.engagement_score
