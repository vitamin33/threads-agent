...
class InsightPredictor:
    ...
    def __init__(self):
        ...
        self.trending_keywords = {
            'ai_ml': ['ai', 'ml', 'llm', 'gpt', 'machine learning', 'artificial intelligence', 'neural', 'model'],
            'cloud_native': ['kubernetes', 'k8s', 'docker', 'microservices', 'serverless', 'cloud'],
            'devops': ['devops', 'ci/cd', 'automation', 'monitoring', 'observability', 'infrastructure'],
            'performance': ['performance', 'optimization', 'scalability', 'latency', 'throughput'],
    ...
    async def predict_insight_quality(
        ...
        logger.info("Predicting insight quality", title=content.title, platform=target_platform)
    ...
    def _extract_structure_features(self, content: ArticleContent) -> Dict[str, float]:
        ...
        if content.code_examples:
            features['avg_code_length'] = np.mean([len(ex['code']) for ex in content.code_examples])
            features['has_code_explanations'] = sum(1 for ex in content.code_examples if ex.get('explanation')) / len(content.code_examples)
        else:
    ...
    def _extract_code_features(self, analysis: CodeAnalysis) -> Dict[str, float]:
        ...
        sophisticated_patterns = ['microservices', 'kubernetes', 'async_programming', 'observability']
        features['sophisticated_patterns'] = sum(1 for p in analysis.patterns if p in sophisticated_patterns)
        
        # Modern tech stack
        modern_deps = ['fastapi', 'kubernetes', 'prometheus', 'openai', 'asyncio']
        features['modern_dependencies'] = sum(1 for dep in analysis.dependencies 
                                            if any(modern in dep.lower() for modern in modern_deps))
        
        # Function analysis
        if analysis.interesting_functions:
            features['avg_function_complexity'] = np.mean([f.get('complexity', 1) for f in analysis.interesting_functions])
            features['async_functions_ratio'] = sum(1 for f in analysis.interesting_functions if f.get('is_async', False)) / len(analysis.interesting_functions)
        else:
    ...
    def _extract_platform_features(self, content: ArticleContent, platform: Platform) -> Dict[str, float]:
        ...
        features['tag_platform_alignment'] = sum(1 for tag in content.tags if tag in relevant_tags) / max(len(content.tags), 1)
    ...
    async def _predict_engagement(self, features: Dict[str, Any], platform: Platform) -> float:
        ...
        base_score += min(features.get('first_person', 0) * 0.1, 1.5)  # Personal stories
    ...
    def _calculate_trend_alignment(self, content: ArticleContent, platform: Platform) -> float:
        """Calculate alignment with current trends"""
        
        text = f"{content.title} {content.subtitle or ''} {' '.join(content.tags)}".lower()
    ...
    def _calculate_confidence(self, features: Dict[str, Any]) -> float:
        ...
        essential_features = [
            'word_count', 'technical_terms', 'code_blocks',
            'title_length', 'code_examples_count'
        ]
        
        available_features = sum(1 for feature in essential_features if feature in features)
    ...
    async def get_improvement_suggestions(
        ...
        if score.technical_depth < 7:
            suggestions.append("Add more technical details and code examples to increase depth")
            suggestions.append("Include architecture diagrams or system design explanations")
        
        # Uniqueness suggestions
        if score.uniqueness < 6:
            suggestions.append("Focus on unique aspects of your implementation or novel solutions")
            suggestions.append("Share personal insights and lessons learned that others might not know")
        
        # Readability suggestions
        if score.readability < 6:
            suggestions.append("Break up long paragraphs and add more subheadings")
            suggestions.append("Add code comments and explanations for complex examples")
        
        # Engagement suggestions
        if score.engagement_potential < 7:
            suggestions.append("Start with a compelling hook or relatable problem")
            suggestions.append("Add personal anecdotes and 'war stories' from your experience")
        ...
        if score.trend_alignment < 6:
            suggestions.append("Connect your solution to current trends like AI/ML or cloud-native technologies")
