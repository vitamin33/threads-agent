...
class BatchProcessor:
    ...
    def __init__(
        ...
        self.locks = {
            model: asyncio.Lock() for model in self.queues
        }
    ...
    async def _process_queue(self, model_key: str):
        ...
        async with self.locks[model_key]:
            ...
            self.metrics["avg_wait_time_ms"] = (
                (self.metrics["avg_wait_time_ms"] * (self.metrics["total_batches"] - 1) + wait_time_ms)
...
{combined_prompt}"""
    ...
    async def _call_llm(self, prompt: str, model: str, max_tokens: int) -> str:
        ...
        separator = "---BATCH_SEPARATOR_" + prompt.split("---BATCH_SEPARATOR_")[1].split("---")[0] + "---"
