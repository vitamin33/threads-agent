...
class TestMLflowHelmValues:
    ...
    def test_helm_values_contains_required_fields(self) -> None:
    ...
    def test_mlflow_image_configuration(self) -> None:
        ...
        assert image.get("repository") == "ghcr.io/mlflow/mlflow"
        assert "tag" in image
        assert image.get("pullPolicy") == "IfNotPresent"

    def test_mlflow_service_configuration(self) -> None:
    ...
    def test_postgresql_backend_configuration(self) -> None:
        ...
        assert postgresql.get("enabled") is False
        assert "auth" in postgresql
        assert postgresql["auth"].get("database") == "mlflow"

    def test_minio_artifact_storage_configuration(self) -> None:
