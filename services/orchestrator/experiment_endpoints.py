...
async def create_experiment(request: CreateExperimentRequest):
    ...
    with record_http_request("orchestrator", "POST", "/experiments/create"):
        try:
            # Validate variant count
            if len(request.variant_ids) < 2:
                raise HTTPException(
                    status_code=400,
                    detail="At least 2 variants required for experiment"
                )
            
            if len(request.variant_ids) > 10:
                raise HTTPException(
                    status_code=400,
                    detail="Maximum 10 variants allowed per experiment"
                )
...
async def start_experiment(experiment_id: str):
    ...
    with record_http_request("orchestrator", "POST", f"/experiments/{experiment_id}/start"):
        success = experiment_manager.start_experiment(experiment_id)
        
        if not success:
            raise HTTPException(
                status_code=400,
                detail="Experiment not found or already started"
            )
...
async def get_experiment_stats(experiment_id: str):
    ...
    with record_http_request("orchestrator", "GET", f"/experiments/{experiment_id}/stats"):
        stats = experiment_manager.get_experiment_stats(experiment_id)
...
async def get_significance_analysis(experiment_id: str):
    ...
    with record_http_request("orchestrator", "GET", f"/experiments/{experiment_id}/significance"):
        stats = experiment_manager.get_experiment_stats(experiment_id)
...
async def calculate_power_analysis(
    ...
    with record_http_request("orchestrator", "POST", f"/experiments/{experiment_id}/power-analysis"):
        ...
        if stats and stats['duration_hours'] > 0:
            current_rate = sum(v['impressions'] for v in stats['variants']) / stats['duration_hours']
...
async def get_next_scheduled_posts(hours: int = 1):
    ...
    with record_http_request("orchestrator", "GET", "/experiments/scheduler/next-posts"):
        ...
        return ScheduleResponse(
            ...
            posts=posts,
            next_window_start=datetime.utcnow()
        )
...
async def get_active_experiments():
    ...
    with record_http_request("orchestrator", "GET", "/experiments/active"):
        from sqlalchemy import create_engine
        from sqlalchemy.orm import Session
        from services.orchestrator.db.models import Experiment
        
        engine = create_engine(experiment_manager.analyzer.calculate_sample_size.__module__)
