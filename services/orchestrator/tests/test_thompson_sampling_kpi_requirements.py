...
class TestThompsonSamplingKPIRequirements:
    ...
    def test_early_kill_10_minute_requirement(self, db_session):
        ...
        db_session.add_all(
            [high_performer, low_performer_old, low_performer_new, new_variant]
        )
        db_session.commit()

        # Implement early kill logic
        def get_variants_to_kill(
            session, min_impressions=10, min_minutes=10, performance_threshold=0.03
        ):  # 50% of 6% target
            ...
            variants_to_kill = (
                session.query(VariantPerformance)
                .filter(
        ...
        assert "low_performer_old" in kill_list, (
            "Should kill low performer past 10 minutes"
        )
        assert "high_performer" not in kill_list, "Should not kill high performer"
        assert "low_performer_new" not in kill_list, (
            "Should not kill low performer under 10 minutes"
        )
        assert "new_variant" not in kill_list, (
            "Should not kill variant with too few impressions"
        )
    ...
    def test_15_percent_engagement_improvement(self, db_session):
        ...
        for i in range(100):
            ...
            variant = VariantPerformance(
                ...
                dimensions={
                    "hook_style": ["question", "statement", "story", "controversy"][
                        i % 4
                    ],
        ...
        for round_num in range(rounds):
            ...
            baseline_selected = np.random.choice(
                [v["variant_id"] for v in all_variants], size=10, replace=False
            )
            ...
            for variant_id in thompson_selected:
                ...
                update_variant_performance(
                    db_session, variant_id, impression=True, success=success
                )
