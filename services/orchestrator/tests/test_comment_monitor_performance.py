...
class TestCommentMonitorPerformance:
    ...
    def test_bulk_deduplication_performance_large_dataset(
        self, large_comment_dataset, mock_db_session_with_bulk_optimization
    ):
        """
        ...
        with self.memory_monitor() as get_memory_delta:
            ...
            unique_comments = comment_monitor._deduplicate_comments(
        ...
        assert execution_time < 5.0, (
            f"Bulk deduplication took {execution_time:.2f}s, expected < 5s"
        )
        assert memory_delta < 100, (
            f"Memory usage {memory_delta:.2f}MB too high, expected < 100MB"
        )
        ...
        assert query_count == len(large_comment_dataset), (
            "Should query once per comment for individual checks"
        )

        # Verify deduplication effectiveness
        expected_unique = (
            len(large_comment_dataset) * 0.7
        )  # 70% unique (30% duplicates)
        assert len(unique_comments) >= expected_unique * 0.9, (
            "Deduplication efficiency too low"
        )

        # Calculate performance metrics
        throughput = len(large_comment_dataset) / execution_time
        assert throughput > 2000, (
            f"Throughput {throughput:.0f} comments/s too low, expected > 2000/s"
        )

    def test_optimized_bulk_deduplication_vs_individual_queries(
        self, comment_dataset_with_duplicates
    ):
        """
        ...
        def individual_query_mock(filter_expr):
            ...
            result_mock.first.return_value = (
                None if individual_query_count["count"] % 3 != 0 else Mock()
            )
            return result_mock
        ...
        start_time = time.time()
        individual_result = comment_monitor_individual._deduplicate_comments(
        ...
        def bulk_query_mock(filter_expr):
            ...
            result_mock = Mock()
            result_mock.first.return_value = (
                None if bulk_query_count["count"] % 3 != 0 else Mock()
            )
            return result_mock
        ...
        comment_monitor_bulk = CommentMonitor(
            fake_threads_client=None, celery_client=Mock(), db_session=bulk_session
        )

        # Measure bulk query performance
        start_time = time.time()
        bulk_result = comment_monitor_bulk._deduplicate_comments(
            comment_dataset_with_duplicates
        )
        ...
        expected_unique_count = (
            len(comment_dataset_with_duplicates) * 0.67
        )  # ~67% unique after deduplication
        assert (
            abs(len(bulk_result) - expected_unique_count)
            < len(comment_dataset_with_duplicates) * 0.1
        )
    ...
    def test_concurrent_comment_processing_performance(self, large_comment_dataset):
        ...
        assert total_time < 10.0, (
            f"Concurrent processing took {total_time:.2f}s, expected < 10s"
        )
        assert memory_delta < 200, (
            f"Memory usage {memory_delta:.2f}MB too high for concurrent processing"
        )
        assert concurrent_throughput > 1000, (
    ...
    def test_memory_efficient_large_batch_processing(self, large_comment_dataset):
        ...
        for i in range(0, len(large_comment_dataset), batch_size):
            ...
            with self.memory_monitor() as get_memory_delta:
                ...
                comment_monitor._store_comments_in_db(
                    unique_comments, f"post_{i // batch_size}"
                )
                memory_delta = get_memory_delta()
        ...
        assert avg_memory_per_batch < 20, (
            f"Average memory per batch {avg_memory_per_batch:.2f}MB too high"
        )
        assert max_memory_per_batch < 50, (
            f"Peak memory per batch {max_memory_per_batch:.2f}MB too high"
        )

        # Verify memory usage doesn't grow significantly over time (leak detection)
        first_half_avg = sum(
            memory_usage_per_batch[: len(memory_usage_per_batch) // 2]
        ) / (len(memory_usage_per_batch) // 2)
        second_half_avg = sum(
            memory_usage_per_batch[len(memory_usage_per_batch) // 2 :]
        ) / (len(memory_usage_per_batch) - len(memory_usage_per_batch) // 2)

        memory_growth_ratio = (
            second_half_avg / first_half_avg if first_half_avg > 0 else 1
        )
        assert memory_growth_ratio < 1.5, (
            f"Memory usage grew {memory_growth_ratio:.2f}x, indicating potential leak"
        )

    @pytest.mark.parametrize(
        "comment_count,expected_max_time",
        [
