...
class TestPublishingTasks:
    ...
    def test_publish_content_task_success(self, mock_engine_class, mock_get_db):
        ...
        mock_engine = Mock()
        mock_engine.publish_to_platform = AsyncMock(return_value=PublishingResult(
            success=True,
            platform="dev.to",
            external_id="12345",
    ...
    @patch('services.orchestrator.publishing.tasks.get_db_session')
    @patch('services.orchestrator.publishing.tasks.PublishingEngine')
    @patch('services.orchestrator.publishing.tasks.retry_failed_publication_task.apply_async')
    def test_publish_content_task_failure_with_retry(self, mock_apply_async, mock_engine_class, mock_get_db):
    ...
    def test_publish_scheduled_content_task_finds_due_content(self, mock_get_db):
        ...
        with patch('services.orchestrator.publishing.tasks.publish_content_task.delay') as mock_delay:
    ...
    def test_retry_failed_publication_task(self, mock_engine_class, mock_get_db):
        ...
        mock_schedule.next_retry_time = datetime.now(timezone.utc) - timedelta(minutes=5)
        mock_schedule.content_item = Mock(spec=ContentItem)
