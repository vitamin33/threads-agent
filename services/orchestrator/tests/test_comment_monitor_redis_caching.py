...
class TestCommentMonitorRedisCaching:
    ...
    def mock_redis_client(self):
        ...
        class MockRedisClient:
            ...
            def get_metrics(self) -> CacheMetrics:
                ...
                avg_lookup_time = sum(self.metrics["lookup_times"]) / len(self.metrics["lookup_times"]) if self.metrics["lookup_times"] else 0
                
                # Estimate memory usage (simplified)
                memory_usage = sum(len(k) + len(v) for k, v in self.data.items()) / 1024 / 1024
    ...
    def cached_comment_monitor(self, mock_redis_client):
        ...
        class CachedCommentMonitor(CommentMonitor):
            ...
            def _cache_comment_existence(self, comment_id: str, exists: bool):
                ...
                cache_key = self._get_comment_cache_key(comment_id)
                self.redis_client.set(cache_key, "true" if exists else "false", ex=self.cache_ttl)

            def _deduplicate_comments_with_cache(self, comments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                if db_queries_needed and self.db_session:
                    for comment in db_queries_needed:
                        ...
                        # Query database
                        existing = self.db_session.query(Comment).filter(
    ...
    def test_redis_cache_hit_performance_improvement(
        ...
        first_run_result = cached_comment_monitor._deduplicate_comments_with_cache(test_comments)
        cold_cache_time = time.time() - start_time

        # Second run: use cached results (warm cache)
        start_time = time.time()
        second_run_result = cached_comment_monitor._deduplicate_comments_with_cache(test_comments)
        ...
        performance_improvement = cold_cache_time / warm_cache_time if warm_cache_time > 0 else 1
        ...
        assert len(first_run_result) == len(second_run_result), "Results should be consistent"
        assert all(c1["id"] == c2["id"] for c1, c2 in zip(first_run_result, second_run_result)), "Comment order should be consistent"
    ...
    def test_redis_cache_deduplication_accuracy(
        ...
        # Test with cache
        cached_result = cached_comment_monitor._deduplicate_comments_with_cache(all_comments)

        # Test without cache (direct DB - simulate existing comments)
        mock_db_session = Mock()
        existing_comment_ids = {f"accuracy_comment_{i}" for i in range(0, 100, 5)}  # Every 5th comment exists

        def mock_db_query(comment_id):
            return Mock(comment_id=comment_id) if comment_id in existing_comment_ids else None

        def mock_filter_chain(filter_expr):
            # Extract comment_id from filter (simplified)
            result_mock = Mock()
            result_mock.first = lambda: mock_db_query("accuracy_comment_0")  # Simplified
            return result_mock
        ...
        accuracy_difference = abs(len(cached_ids) - len(non_cached_ids)) / max(len(cached_ids), len(non_cached_ids), 1)
        ...
        assert len(cached_result) <= len(all_comments), "Cached result can't have more comments than input"
    ...
    def test_redis_cache_concurrent_access_consistency(
        ...
        expected_shared_processed = len(set(shared_comment_ids))  # Should be unique across all batches

        # Cache consistency assertions
        assert cache_metrics.cache_hits > 0, "Should have cache hits from concurrent access"
        ...
        assert all(r["processed_count"] <= r["original_count"] for r in results), "Processed count should not exceed original"

        print(f"Concurrent cache consistency test: {len(results)} batches, {concurrent_throughput:.0f} comments/s")
    ...
    def test_redis_cache_memory_efficiency_and_eviction(
        ...
        for i in range(0, len(large_comment_set), batch_size):
            ...
            # Process batch
            start_time = time.time()
            unique_comments = cached_comment_monitor._deduplicate_comments_with_cache(batch)
            processing_time = time.time() - start_time
            ...
            memory_snapshots.append({
                ...
                "cache_entries": cache_metrics.cache_sets - cache_metrics.cache_deletes,
                "processing_time": processing_time,
        ...
        assert peak_memory < 50, f"Peak cache memory {peak_memory:.2f}MB exceeds 50MB limit"
        ...
        memory_growth_ratio = last_batch_memory / first_batch_memory if first_batch_memory > 0 else 1
    ...
    def test_redis_cache_invalidation_strategies(
        ...
        first_result = cached_comment_monitor._deduplicate_comments_with_cache(initial_comments)
        ...
        changed_comment_ids = [c["id"] for c in changed_comments if c["id"] in [ic["id"] for ic in initial_comments]]
        cached_comment_monitor._invalidate_comment_cache(changed_comment_ids)

        # Process changed comments
        second_result = cached_comment_monitor._deduplicate_comments_with_cache(changed_comments)
        post_invalidation_metrics = mock_redis_client.get_metrics()

        # Invalidation effectiveness assertions
        assert post_invalidation_metrics.cache_deletes > 0, "Should have deleted cache entries"

        # Verify cache consistency after invalidation
        overlapping_ids = set(c["id"] for c in initial_comments).intersection(set(c["id"] for c in changed_comments))
        ...
        for comment_id in changed_comment_ids:
            ...
            assert not mock_redis_client.exists(cache_key), f"Invalidated key {cache_key} should not exist in cache"

        # Test selective invalidation
        all_cache_keys_before = set(mock_redis_client.keys())
        cached_comment_monitor._invalidate_comment_cache(["invalidation_comment_0", "invalidation_comment_1"])
        ...
        print(f"Cache invalidation test: {post_invalidation_metrics.cache_deletes} entries invalidated")

    @pytest.mark.parametrize("cache_ttl,expected_behavior", [
    ...
    def test_redis_cache_ttl_expiration_behavior(
        ...
        class TTLTestMonitor(CommentMonitor):
            def __init__(self):
                super().__init__(fake_threads_client=None, celery_client=Mock(), db_session=mock_db_session)
                self.redis_client = mock_redis_client
            ...
            def _check_comment_cached(self, comment_id: str) -> Optional[bool]:
                ...
                self.redis_client.set(cache_key, "exists" if exists else "not_exists", ex=self.cache_ttl)
                return exists
        ...
        assert mock_redis_client.exists(cache_key), "Cache entry should exist after initial check"
        ...
        if expected_behavior == "expired":
            ...
            assert expired_result is None, f"Cache entry should expire after {cache_ttl}s"
            
            # Verify cache miss after expiration
            post_expiry_metrics = mock_redis_client.get_metrics()
            assert post_expiry_metrics.cache_misses > 0, "Should have cache misses after expiration"

        elif expected_behavior == "cached":
            # Entry should still exist
            cached_result = mock_redis_client.get(cache_key)
            assert cached_result is not None, f"Cache entry should not expire with TTL {cache_ttl}s"
            
            # Verify cache hits
            second_check = ttl_monitor._check_comment_cached(test_comment_id)
            post_check_metrics = mock_redis_client.get_metrics()
            assert post_check_metrics.cache_hits > 0, "Should have cache hits for non-expired entries"
