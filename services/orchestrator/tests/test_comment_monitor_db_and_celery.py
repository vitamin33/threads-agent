...
class TestCommentMonitorDatabaseConnectionPooling:
        ...
        class MockConnectionPool:
            ...
            def get_metrics(self) -> ConnectionPoolMetrics:
                ...
                return ConnectionPoolMetrics(
                    ...
                    connection_wait_time=sum(self.connection_wait_times) / len(self.connection_wait_times) if self.connection_wait_times else 0,
                    query_execution_time=sum(self.query_times) / len(self.query_times) if self.query_times else 0,
            ...
            def query(self, model):
                ...
                def filter_mock(filter_expr):
                    ...
                    def first():
                        ...
                        self.connection.execute_query(f"SELECT * FROM comments WHERE comment_id = ?")
    ...
    def test_connection_pool_efficiency_under_concurrent_load(
        ...
        assert avg_batch_time < 0.5, f"Average batch processing time {avg_batch_time:.3f}s too slow"
    ...
    def test_connection_pool_recovery_after_exhaustion(self, mock_connection_pool):
        ...
        small_pool = self.mock_connection_pool(max_connections=5, min_connections=2)
        ...
        def exhaust_pool_worker(worker_id):
            ...
            comments = [{"id": f"comment_{worker_id}", "post_id": "test", "text": "test", "author": "test", "timestamp": "2024-01-01T10:00:00Z"}]
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            ...
            exhaustion_results = [future.result() for future in as_completed(exhaustion_futures)]
        
        # Verify pool was exhausted
        initial_metrics = small_pool.get_metrics()
        assert initial_metrics.pool_exhaustion_events > 0, "Pool should have been exhausted"
        ...
        def recovery_worker(worker_id):
            ...
            comments = [
                {"id": f"recovery_comment_{worker_id}_{i}", "post_id": f"recovery_post_{worker_id}", "text": f"Recovery comment {i}", "author": f"user_{i}", "timestamp": "2024-01-01T11:00:00Z"}
                for i in range(10)
            ]
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            recovery_futures = [
                executor.submit(recovery_worker, worker_id)
                for worker_id in range(4)
            ]
            
            recovery_results = [future.result() for future in as_completed(recovery_futures)]
        
        # Verify recovery
        final_metrics = small_pool.get_metrics()
        
        assert len(recovery_results) == 4, "All recovery workers should complete"
        assert all(r["processed"] > 0 for r in recovery_results), "All workers should process comments"
        
        # Recovery time should be reasonable
        avg_recovery_time = sum(r["time"] for r in recovery_results) / len(recovery_results)
        assert avg_recovery_time < 1.0, f"Recovery time {avg_recovery_time:.3f}s too slow"
...
class TestCommentMonitorCeleryBatchProcessing:
        ...
        class MockCeleryBatchClient:
            ...
            def send_task_batch(self, tasks):
                ...
                try:
                    for task in tasks:
                        task_data = {
                            "task_name": task.get("task_name", "analyze_comment_intent"),
                            "args": task.get("args", []),
            ...
            def get_metrics(self) -> CeleryBatchMetrics:
                ...
                return CeleryBatchMetrics(
                    ...
                    batch_size_avg=sum(self.batch_sizes) / len(self.batch_sizes) if self.batch_sizes else 0,
                    queue_time_avg=sum(self.queue_times) / len(self.queue_times) if self.queue_times else 0,
    ...
    def test_batch_comment_analysis_queuing(self, mock_celery_client_with_batching):
        ...
        assert batch_metrics.total_tasks_queued == len(comments), "All comments should be queued"
        ...
        queue_time_improvement = individual_queue_time / batch_queue_time if batch_queue_time > 0 else 1
        ...
        assert batch_metrics.task_distribution["analyze_comment_intent"] == len(comments)

    def test_celery_queue_performance_under_high_load(self, mock_celery_client_with_batching):
        """
        ...
        queue_time_variance = max_queue_time / min_queue_time if min_queue_time > 0 else float('inf')
    ...
    def test_celery_task_priority_and_ordering(self, mock_celery_client_with_batching):
        ...
        comment_monitor._queue_comments_for_analysis(normal_priority_comments, "regular_post")
        ...
        comment_monitor._queue_comments_for_analysis(high_priority_comments, "urgent_post")
        ...
        urgent_tasks = [task for task in queued_tasks if "urgent_post" in str(task.get("args", []))]
        regular_tasks = [task for task in queued_tasks if "regular_post" in str(task.get("args", []))]
        
        assert len(urgent_tasks) == 10, "Should have 10 urgent tasks"
        assert len(regular_tasks) == 50, "Should have 50 regular tasks"

    @pytest.mark.parametrize("batch_size,expected_efficiency", [
    ...
    def test_batch_size_optimization_efficiency(
        ...
        batch_size, 
        expected_efficiency,
        mock_celery_client_with_batching
    ):
        """
        ...
        metrics = mock_celery_client_with_batching.get_metrics()
        throughput = metrics.total_tasks_queued / processing_time if processing_time > 0 else 0
        
        # Efficiency assertions
        min_expected_throughput = expected_efficiency * 1000  # Base throughput of 1000/s for individual
        ...
        assert metrics.total_tasks_queued == total_comments, "All comments should be queued"
        ...
        print(f"Batch size {batch_size}: {throughput:.0f} tasks/s, "
