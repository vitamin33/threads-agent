...
class TestViralEngineIntegration:
    ...
    def test_quality_check_calls_viral_engine_api(self, mock_viral_api, db_session: Session):
        """
    ...
    def test_quality_scored_event_updates_content_status(self, db_session: Session):
        ...
        quality_event_data = {
            ...
            "improvement_suggestions": ["Add more compelling hook", "Increase emotional appeal"]
        }
        ...
        db_session.refresh(db_session.query(ContentItem).filter(ContentItem.id == content_id).first())
        updated_content = db_session.query(ContentItem).filter(ContentItem.id == content_id).first()
