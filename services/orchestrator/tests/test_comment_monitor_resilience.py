...
class TestCommentMonitorErrorHandling:
    ...
    def failing_db_session(self):
        ...
        class FailingDBSession:
            ...
            def query(self, model):
                ...
                if self.failure_mode == "connection_timeout" and self.failure_count < self.max_failures:
                    self.failure_count += 1
                    raise psycopg2.OperationalError("connection timeout")
                
                elif self.failure_mode == "deadlock" and self.failure_count < self.max_failures:
                    self.failure_count += 1
                    raise psycopg2.OperationalError("deadlock detected")
                
                elif self.failure_mode == "connection_lost" and self.failure_count < self.max_failures:
                    self.failure_count += 1
            ...
            def add(self, obj):
                ...
                if self.failure_mode == "disk_full" and self.failure_count < self.max_failures:
                    self.failure_count += 1
                    raise psycopg2.OperationalError("disk full")

            def commit(self):
                """Mock commit with failure simulation."""
                if self.failure_mode == "commit_failure" and self.failure_count < self.max_failures:
                    self.failure_count += 1
    ...
    def failing_celery_client(self):
        ...
        class FailingCeleryClient:
            ...
            def send_task(self, task_name, args=None, kwargs=None, **options):
                ...
                if self.failure_mode == "broker_connection_error" and self.failure_count < self.max_failures:
                    self.failure_count += 1
                    raise ConnectionError("broker connection failed")
                
                elif self.failure_mode == "queue_full" and self.failure_count < self.max_failures:
                    self.failure_count += 1
                    raise Exception("queue is full")
                
                elif self.failure_mode == "serialization_error" and self.failure_count < self.max_failures:
                    self.failure_count += 1
    ...
    def resilient_comment_monitor(self, failing_db_session, failing_celery_client):
        ...
        class ResilientCommentMonitor(CommentMonitor):
            ...
            def _retry_with_backoff(self, operation: Callable, max_retries: int = 3, backoff_base: float = 1.0):
            ...
            def _deduplicate_comments_with_resilience(self, comments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                try:
                    return self._retry_with_backoff(deduplication_operation, max_retries=3)
                except Exception as e:
            ...
            def _store_comments_with_resilience(self, comments: List[Dict[str, Any]], post_id: str):
            ...
            def _queue_comments_with_resilience(self, comments: List[Dict[str, Any]], post_id: str):
                ...
                try:
                    ...
                    if len([log for log in self.error_log if log.get("operation") == "queuing"]) > 5:
            ...
            def get_resilience_metrics(self) -> ResilienceMetrics:
                ...
                successful_ops = sum(1 for log in self.error_log if "success" in log.get("operation", ""))
    ...
    def test_database_connection_failure_recovery(
        ...
        unique_comments = resilient_comment_monitor._deduplicate_comments_with_resilience(test_comments)
        ...
        # Recovery assertions
        assert len(unique_comments) > 0, "Should successfully process comments after DB recovery"
        assert metrics.retry_attempts > 0, "Should have attempted retries for DB failures"
        assert metrics.fallback_activations <= 1, "Should recover without excessive fallback usage"

        # Processing should complete in reasonable time despite retries
        assert processing_time < 10.0, f"Processing time {processing_time:.2f}s too long for recovery scenario"

        # Verify actual recovery occurred
        assert failing_db_session.failure_count >= 2, "Should have encountered configured failures"
        assert failing_db_session.operation_count > failing_db_session.failure_count, "Should have successful operations after failures"

        print(f"DB failure recovery: {metrics.retry_attempts} retries, {len(unique_comments)} comments processed")
    ...
    def test_celery_broker_failure_handling(
        ...
        failing_celery_client.set_failure_mode("broker_connection_error", max_failures=2)
        ...
        start_time = time.time()
        resilient_comment_monitor._queue_comments_with_resilience(test_comments, "celery_failure_post")
        processing_time = time.time() - start_time
        ...
        assert metrics.retry_attempts > 0, "Should have attempted retries for Celery failures"
        
        # Should eventually succeed or gracefully degrade
        queue_errors = [log for log in resilient_comment_monitor.error_log if log.get("operation") == "queuing"]
        ...
        if failing_celery_client.failure_count >= 2:
            ...
            assert len(failing_celery_client.queued_tasks) > 0, "Should have successfully queued tasks after recovery"
        else:
            # Graceful degradation - should have logged errors appropriately
            assert len(queue_errors) > 0, "Should have logged queue failures"

        print(f"Celery failure handling: {metrics.retry_attempts} retries, {len(failing_celery_client.queued_tasks)} tasks queued")
    ...
    def test_concurrent_failure_scenarios(
        ...
        def process_batch_with_failures(batch_id: int):
            ...
            if batch_id % 3 == 0:
                failing_db_session.set_failure_mode("connection_timeout", max_failures=1)
            elif batch_id % 3 == 1:
                failing_celery_client.set_failure_mode("broker_connection_error", max_failures=1)
            ...
            try:
                ...
                unique_comments = resilient_comment_monitor._deduplicate_comments_with_resilience(comments)
                resilient_comment_monitor._store_comments_with_resilience(unique_comments, f"concurrent_failure_post_{batch_id}")
                resilient_comment_monitor._queue_comments_with_resilience(unique_comments, f"concurrent_failure_post_{batch_id}")
        ...
        assert success_rate > 0.6, f"Success rate {success_rate:.1%} too low under concurrent failures"
        
        # Should have reasonable retry behavior
        assert metrics.retry_attempts > 0, "Should have attempted retries under concurrent failures"
        assert metrics.retry_attempts < num_batches * 10, "Should not have excessive retry attempts"

        # Circuit breaker should prevent cascading failures
        if len(failed_batches) > 3:
            assert metrics.circuit_breaker_trips > 0, "Circuit breaker should trip under high failure rate"

        # Performance should be reasonable despite failures
        if successful_batches:
            avg_processing_time = sum(r["processing_time"] for r in successful_batches) / len(successful_batches)
            assert avg_processing_time < 5.0, f"Average processing time {avg_processing_time:.2f}s too high under failures"

        print(f"Concurrent failures: {success_rate:.1%} success rate, {metrics.retry_attempts} retries, {metrics.circuit_breaker_trips} circuit breaker trips")
    ...
    def test_cascading_failure_prevention(
        ...
        failing_db_session.set_failure_mode("connection_lost", max_failures=999)  # Persistent failure
        failing_celery_client.set_failure_mode("queue_full", max_failures=999)  # Persistent failure
        ...
        try:
            unique_comments = resilient_comment_monitor._deduplicate_comments_with_resilience(test_comments)
            resilient_comment_monitor._store_comments_with_resilience(unique_comments, "cascading_failure_post")
            resilient_comment_monitor._queue_comments_with_resilience(unique_comments, "cascading_failure_post")
        ...
        assert metrics.fallback_activations > 0, "Should activate fallback mechanisms under persistent failures"
        
        # Circuit breaker should engage
        assert metrics.circuit_breaker_trips > 0, "Circuit breaker should trip to prevent cascading failures"
        
        # Should complete processing in reasonable time (not hang indefinitely)
        assert processing_time < 30.0, f"Processing time {processing_time:.2f}s too long, system may be hanging"
        
        # Should still process some comments via fallback
        unique_comments = resilient_comment_monitor._deduplicate_comments_with_resilience(test_comments)
        assert len(unique_comments) > 0, "Should process comments via fallback mechanisms"

        print(f"Cascading failure prevention: {metrics.fallback_activations} fallbacks, {metrics.circuit_breaker_trips} circuit breaker trips")

    @pytest.mark.parametrize("failure_scenario", [
        FailureScenario("db_timeout", "connection_timeout", 0.3, 5.0, "retry_and_recover"),
        FailureScenario("celery_connection", "broker_connection_error", 0.5, 3.0, "graceful_degradation"),
        FailureScenario("disk_full", "disk_full", 0.8, 2.0, "fallback_activation"),
        FailureScenario("serialization", "serialization_error", 0.4, 4.0, "error_handling"),
    ])
    def test_specific_failure_scenario_handling(
        ...
        if "db" in failure_scenario.failure_type or "disk" in failure_scenario.failure_type:
            failing_db_session.set_failure_mode(failure_scenario.failure_type, max_failures)
        else:
            failing_celery_client.set_failure_mode(failure_scenario.failure_type, max_failures)
        ...
        try:
            unique_comments = resilient_comment_monitor._deduplicate_comments_with_resilience(test_comments)
            resilient_comment_monitor._store_comments_with_resilience(unique_comments, f"scenario_{failure_scenario.name}_post")
            resilient_comment_monitor._queue_comments_with_resilience(unique_comments, f"scenario_{failure_scenario.name}_post")
        except Exception as e:
        ...
        if failure_scenario.expected_behavior == "retry_and_recover":
            assert processing_successful, f"Should recover from {failure_scenario.name} failures"
            assert metrics.retry_attempts > 0, "Should attempt retries for recoverable failures"
            
        elif failure_scenario.expected_behavior == "graceful_degradation":
            # Should either succeed or fail gracefully without crashing
            assert processing_time < failure_scenario.duration_seconds * 3, "Should not hang during graceful degradation"
            
        elif failure_scenario.expected_behavior == "fallback_activation":
            assert metrics.fallback_activations > 0, f"Should activate fallbacks for {failure_scenario.name}"
            
        elif failure_scenario.expected_behavior == "error_handling":
            # Should handle errors appropriately (log, retry, or fail gracefully)
            assert len(resilient_comment_monitor.error_log) > 0, "Should log errors appropriately"

        # Performance should be reasonable for all scenarios
        assert processing_time < 15.0, f"Processing time {processing_time:.2f}s too long for {failure_scenario.name}"

        print(f"Scenario {failure_scenario.name}: {'SUCCESS' if processing_successful else 'HANDLED'}, "
