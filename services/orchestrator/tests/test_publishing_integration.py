...
class TestPublishingIntegration:
    ...
    async def test_end_to_end_devto_publishing(self):
        ...
        # Test tag filtering
        filtered_tags = devto_adapter._filter_tags_for_devto(
    ...
    async def test_end_to_end_linkedin_publishing(self):
        ...
        is_valid = await linkedin_adapter.validate_content(
            content_item, platform_config
        )
        assert is_valid is True
        ...
        hashtags = linkedin_adapter._format_hashtags_for_linkedin(
            content_item.content_metadata["tags"]
        )
    ...
    def test_scheduled_content_processing_integration(self, mock_delay, mock_get_db):
        ...
        schedule3 = Mock()
        schedule3.id = 3
        schedule3.scheduled_time = now + timedelta(
            minutes=30
        )  # Future - shouldn't be processed
