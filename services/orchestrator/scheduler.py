...
class ExperimentScheduler:
    ...
    def schedule_next_posts(
        ...
        for experiment in active_experiments:
            ...
            if not (posting_hours.get('start', 0) <= current_hour < posting_hours.get('end', 24)):
                continue
            ...
            for variant in variants_to_post:
                # Check rate limits
                if posts_per_persona[experiment.persona_id] >= self.rate_limits.get(experiment.persona_id, 10):
                    break
    ...
    def _needs_more_samples(self, experiment: Experiment) -> bool:
        ...
        if experiment.started_at:
            duration = (datetime.utcnow() - experiment.started_at).total_seconds() / 3600
    ...
    def _select_variants_to_post(
        ...
        with Session(engine) as session:
            variants = session.query(ExperimentVariant).filter_by(
                experiment_id=experiment.experiment_id
            ).filter(
    ...
    def _select_bandit_variants(
        ...
        for i, variant in enumerate(variants):
            ...
            bandit.failures[i] = variant.impressions_total - variant.engagements_total + 1
    ...
    def _calculate_optimal_time(
        ...
        if selected_hour >= current_hour:
            ...
            # Next day
            post_time = (start_time + timedelta(days=1)).replace(hour=selected_hour, minute=minutes, second=0)
    ...
    def _calculate_priority(
        ...
        if experiment.started_at:
            hours_running = (datetime.utcnow() - experiment.started_at).total_seconds() / 3600
            if hours_running < 24:
                priority += 10
    ...
    def execute_scheduled_post(self, schedule: PostingSchedule) -> bool:
        ...
        with Session(engine) as session:
            variant = session.query(ExperimentVariant).filter_by(
