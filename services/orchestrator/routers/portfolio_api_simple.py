...
async def get_portfolio_summary(
    ...
    try:
        ...
        achievement_summary = db.execute(text("""
            SELECT 
        ...
        recent_achievements = db.execute(text("""
        ...
        experiment_metrics = db.execute(text("""
            SELECT 
        ...
        portfolio_data = {
            "summary": {
                ...
                "avg_complexity_score": float(achievement_summary.avg_complexity_score or 0),
                "total_time_saved": float(achievement_summary.total_time_saved or 0),
                "portfolio_ready_count": int(achievement_summary.portfolio_ready_count or 0)
            },
            "recent_achievements": [
                {
                    "title": ach.title,
                    "description": ach.description[:200] + "..." if len(ach.description) > 200 else ach.description,
            ...
            "business_impact": {
                ...
                "completed_experiments": int(experiment_metrics.completed_experiments or 0),
                "active_experiments": int(experiment_metrics.active_experiments or 0),
                "revenue_potential": 20000,
                "engagement_improvement": float(ab_metrics.best_success_rate or 0) * 100,
...
async def get_live_kpis(
    ...
    try:
        ...
        ab_performance = db.execute(text("""
            SELECT 
                COUNT(*) as total_variants,
                AVG(success_rate) as avg_success_rate,
                MAX(success_rate) as best_success_rate,
        ...
        experiment_status = db.execute(text("""
            SELECT 
                COUNT(*) as total_experiments,
                COUNT(CASE WHEN status = 'active' THEN 1 END) as active_experiments,
                COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed_experiments,
        ...
        achievement_metrics = db.execute(text("""
            SELECT 
                COUNT(*) as total_achievements,
        ...
        kpi_data = {
            ...
            "technical_kpis": {
                ...
                "avg_impact_score": round(float(achievement_metrics.avg_impact or 0), 2),
...
async def get_ab_testing_showcase(
    ...
    try:
        ...
        experiment_results = db.execute(text("""
        ...
        showcase_data = {
            "algorithm_showcase": {
                ...
                "statistical_methods": ["Beta Distributions", "Two-proportion z-tests", "Confidence Intervals"],
            ...
            "experiment_results": [
                {
                    ...
                    "duration_days": (exp.end_time - exp.start_time).days if exp.end_time and exp.start_time else None
                }
    ...
    try:
        ...
        real_time_data = {
            "current_performance": {
                ...
                "average_engagement_rate": round(float(current_metrics.avg_rate or 0) * 100, 2),
            ...
            "recent_activity": {
                ...
                "updated_experiments_24h": int(recent_activity.updated_experiments or 0)
            },
            "system_status": {
