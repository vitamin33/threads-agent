...
async def get_portfolio_data(
    include_recent: int = Query(
        5, description="Number of recent achievements to include"
    ),
    ...
    try:
        ...
        recent_achievements = (
            ...
            .filter(Achievement.portfolio_ready == True)
            .order_by(desc(Achievement.completed_at))
...
async def get_achievements(
    ...
    portfolio_ready: bool = Query(
        True, description="Only portfolio-ready achievements"
    ),
...
async def get_kpis(db: Session = Depends(get_db_session)) -> JSONResponse:
    ...
    try:
        ...
        achievement_kpis = db.query(
            ...
            func.avg(Achievement.impact_score).label("avg_impact"),
            func.sum(Achievement.time_saved_hours).label("total_time_saved"),
            func.avg(Achievement.performance_improvement_pct).label(
        ...
        real_time_metrics = {
            "current_engagement_rate": float(ab_testing_kpis.avg_success_rate or 0)
            * 100,
        ...
        achievements = (
            db.query(Achievement)
            .filter(Achievement.skills_demonstrated.isnot(None))
            .all()
        )
        for ach in achievements:
        ...
        top_skills_with_count = sorted(
            skills_frequency.items(), key=lambda x: x[1], reverse=True
        )[:8]
        ...
        kpi_response = {
            "achievement_metrics": {
                ...
                "average_performance_gain": float(
                    achievement_kpis.avg_performance_gain or 0
                ),
            },
            "technical_metrics": {
...
async def get_achievement_detail(
    achievement_id: int, db: Session = Depends(get_db_session)
) -> JSONResponse:
    ...
    try:
        ...
        duration = time.time() - start_time
        record_http_request(
            "GET", f"/portfolio/achievements/{achievement_id}", 200, duration
        )
        ...
        duration = time.time() - start_time
        record_http_request(
            "GET", f"/portfolio/achievements/{achievement_id}", 500, duration
        )
...
async def get_live_metrics(db: Session = Depends(get_db_session)) -> JSONResponse:
    ...
    try:
        ...
        top_variant = (
            ...
            .order_by(desc(VariantPerformance.success_rate))
            .first()
        )
        ...
        live_metrics = {
            "ab_testing_performance": {
                ...
                "average_success_rate": float(variant_metrics.avg_success_rate or 0)
                * 100,
                "best_success_rate": float(variant_metrics.best_success_rate or 0)
                * 100,
            ...
            "top_performer": {
                ...
                "success_rate": float(top_variant.success_rate * 100)
                if top_variant
                else 0,
...
async def get_portfolio_section(
    section: str, db: Session = Depends(get_db_session)
) -> JSONResponse:
    ...
    try:
        ...
        achievements = (
            ...
            .order_by(desc(Achievement.display_priority))
            .all()
        )
        ...
        section_data = {
            ...
            "section_metrics": {
                "avg_impact": sum(ach.impact_score or 0 for ach in achievements)
                / len(achievements)
                if achievements
                else 0,
                "total_time_saved": sum(
                    ach.time_saved_hours or 0 for ach in achievements
                ),
...
async def get_portfolio_updates_stream(
    ...
    try:
        ...
        # Recent achievements
        recent_achievements = (
            db.query(Achievement).filter(Achievement.created_at >= since_time).all()
        )

        # Recent variant updates
        recent_variants = (
            db.query(VariantPerformance)
            .filter(VariantPerformance.last_used >= since_time)
            .all()
        )

        # Recent experiment activity
        recent_experiments = (
            db.query(Experiment).filter(Experiment.updated_at >= since_time).all()
        )
...
async def portfolio_api_health(db: Session = Depends(get_db_session)) -> JSONResponse:
    ...
    try:
        ...
        portfolio_ready_count = (
            db.query(Achievement).filter(Achievement.portfolio_ready == True).count()
        )
