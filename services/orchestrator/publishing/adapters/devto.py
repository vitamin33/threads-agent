        ...
        try:
            ...
            payload = {
                "article": {
                    ...
                    "tags": self._filter_tags_for_devto(
                        content_item.content_metadata.get("tags", [])
                        if content_item.content_metadata
                        else []
                    ),
                    "description": content_item.content_metadata.get(
                        "description", content_item.title
                    )
                    if content_item.content_metadata
                    else content_item.title,
                }
            }
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.api_base_url}/articles", json=payload, headers=headers
                )
