...
def _handle_publishing_failure(
    db: Session, schedule: ContentSchedule, error_message: str
) -> Dict[str, Any]:
    ...
    if schedule.retry_count < schedule.max_retries:
        ...
        schedule.next_retry_time = datetime.now(timezone.utc) + timedelta(
        ...
        retry_failed_publication_task.apply_async(
            args=[schedule.id], countdown=retry_delay_seconds
        )
