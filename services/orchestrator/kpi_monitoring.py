...
PATTERN_USAGE = Gauge(
    'pattern_usage_7d',
    'Pattern usage count in last 7 days',
    ['pattern_hash']
)

ENGAGEMENT_IMPROVEMENT = Gauge(
    'engagement_improvement_percent',
...
class KPIMonitor:
    ...
    def check_early_kill_candidates(self):
        ...
        poor_performers = self.db_session.query(VariantPerformance).filter(
    ...
    def calculate_engagement_improvement(self):
        ...
        recent_variants = self.db_session.query(VariantPerformance).filter(
    ...
    def track_pattern_fatigue(self):
        ...
        for variant in recent_variants:
            ...
            pattern = f"{dims.get('hook_style', 'unknown')}_{dims.get('emotion', 'unknown')}"
    ...
    def track_cost_per_post(self, variants_generated, e3_cache_hits, e3_cache_misses):
        ...
        generation_cost = (
            variants_generated * AVG_TOKENS_PER_VARIANT * 
            COST_PER_1K_TOKENS_CHEAP / 1000
        )
        
        e3_cost = (
            e3_cache_misses * E3_TOKENS_PER_PREDICTION * 
