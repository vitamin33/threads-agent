...
class VLLMModelRegistry:
    ...
    def get_models_by_content_type(self, content_type: ContentType) -> List[VLLMModelConfig]:
        """Get models that support a specific content type."""
        return [
            model for model in self.vllm_adapter.models.values()
    ...
    def get_loading_priority_order(self) -> List[str]:
        ...
        models_by_priority = sorted(
            self.vllm_adapter.models.items(),
    ...
    async def log_to_mlflow(self, model_id: str, metrics: Dict[str, float], parameters: Dict[str, Any] = None) -> None:
    ...
    def get_model_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "loaded_models": len([m for m in self.vllm_adapter.models.values() if m.load_state == ModelLoadState.LOADED]),
            **memory_summary,
            "models": {}
        }
        ...
        for content_type in ContentType:
            ...
            if routing:
                ...
                for model_id in primary_models:
                    if model_id not in self.vllm_adapter.models:
                        errors.append(f"Unknown model '{model_id}' in routing for '{content_type.value}'")
    ...
    async def promote_model(self, model_id: str, target_stage: ModelStage, validation_results: Optional[Dict] = None) -> bool:
        ...
        return await self.unified_registry.promote_model(model_id, target_stage, validation_results)
...
def get_model_for_content_type(content_type: Union[ContentType, str]) -> Optional[VLLMModelConfig]:
    """Get the best model for a specific content type."""
