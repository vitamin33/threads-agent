...
class ConfigValidator:
    ...
    def _validate_basic_structure(self) -> None:
        ...
        required_sections = [
            'version', 'platform', 'total_memory_gb', 'memory_safety_threshold',
            'models', 'resource_management', 'content_routing', 'apple_silicon'
        ]
        ...
        if self.config.get('platform') != 'apple-silicon-m4-max':
            self.warnings.append("Configuration is optimized for apple-silicon-m4-max platform")
        
        # Validate memory settings
        total_memory = self.config.get('total_memory_gb', 0)
        if total_memory < 16:
            self.errors.append(f"Total memory ({total_memory}GB) too low for multi-model deployment")
        ...
        if not 0.5 <= threshold <= 0.95:
            self.errors.append(f"Memory safety threshold ({threshold}) should be between 0.5 and 0.95")
    ...
    def _validate_models(self) -> None:
        ...
        if len(models) != 5:
            self.warnings.append(f"Expected 5 models for multi-model deployment, got {len(models)}")
        ...
        for model_id, model_config in models.items():
            ...
            required_fields = [
                'name', 'display_name', 'memory_requirements',
                'content_types', 'priority', 'optimization', 'performance'
            ]
            ...
            if model_id in expected_models:
                ...
                if actual_name != expected_name:
                    self.warnings.append(f"Model {model_id} name mismatch: expected {expected_name}, got {actual_name}")
            ...
            for field in required_mem_fields:
                if field not in mem_req:
                    self.errors.append(f"Model {model_id} missing memory requirement: {field}")
                elif not isinstance(mem_req[field], (int, float)) or mem_req[field] <= 0:
                    self.errors.append(f"Model {model_id} invalid memory requirement {field}: {mem_req[field]}")
            ...
            if all(field in mem_req for field in required_mem_fields):
                ...
                if not (minimum <= optimized <= base):
                    self.errors.append(f"Model {model_id} memory requirements not ordered: minimum <= optimized <= base")
            ...
            if 'target_latency_ms' in performance:
                ...
                if not isinstance(latency, int) or latency <= 0:
                    self.errors.append(f"Model {model_id} invalid target latency: {latency}")
                elif latency > 100:
                    self.warnings.append(f"Model {model_id} high target latency: {latency}ms")
    ...
    def _validate_resource_management(self) -> None:
        ...
        if concurrent_limit < 1 or concurrent_limit > 5:
            self.errors.append(f"Concurrent models limit ({concurrent_limit}) should be between 1 and 5")
    ...
    def _validate_content_routing(self) -> None:
        ...
        expected_content_types = [
            'twitter', 'linkedin', 'technical_articles', 
            'code_documentation', 'general'
        ]
        
        for content_type in expected_content_types:
            if content_type not in content_routing:
                self.warnings.append(f"Missing routing configuration for content type: {content_type}")
                continue
            ...
            for model_id in primary:
                if model_id not in models:
                    self.errors.append(f"Unknown model '{model_id}' in {content_type} primary routing")
            
            # Validate fallback models
            fallback = routing.get('fallback', [])
            for model_id in fallback:
                if model_id not in models:
                    self.errors.append(f"Unknown model '{model_id}' in {content_type} fallback routing")
            
            # Validate max tokens
            max_tokens = routing.get('max_tokens')
            if max_tokens and (not isinstance(max_tokens, int) or max_tokens <= 0):
                self.errors.append(f"Invalid max_tokens for {content_type}: {max_tokens}")
    ...
    def _validate_apple_silicon_config(self) -> None:
        ...
        for preset in expected_presets:
            ...
            if gpu_util and (not 0.1 <= gpu_util <= 1.0):
                self.errors.append(f"Invalid GPU memory utilization in {preset}: {gpu_util}")
    ...
    def _validate_mlflow_integration(self) -> None:
        ...
        if not experiment_name:
            self.errors.append("MLflow experiment name is required when integration is enabled")
    ...
    def _validate_memory_constraints(self) -> None:
        ...
        concurrent_limit = self.config.get('resource_management', {}).get('concurrent_models_limit', 3)
        ...
        for model_id, model_config in models.items():
            ...
            if minimum_memory > available_memory:
                self.errors.append(f"Model {model_id} minimum memory ({minimum_memory}GB) exceeds available memory ({available_memory}GB)")
        ...
        if top_n_memory > available_memory:
            self.warnings.append(f"Top {concurrent_limit} largest models require {top_n_memory:.1f}GB, but only {available_memory:.1f}GB available")
        ...
        if min_concurrent_memory > available_memory:
            self.errors.append(f"Even minimum memory requirements for {concurrent_limit} models ({min_concurrent_memory:.1f}GB) exceed available memory")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Validate multi-model vLLM configuration")
    parser.add_argument(
        '--config-path',
        default=None,
    ...
    if args.config_path:
        ...
        if not config_path.exists():
            ...
            print("Please specify --config-path or ensure multi_model_config.yaml exists")
            sys.exit(1)
