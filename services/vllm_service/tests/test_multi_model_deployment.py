...
def mock_apple_silicon_m4_max():
    ...
    with patch('platform.machine', return_value='arm64'), \
        ...
        with patch('psutil.virtual_memory') as mock_memory:
            ...
            mock_memory.return_value.available = M4_MAX_MEMORY_GB * 0.8 * 1024 * 1024 * 1024
            yield
...
class TestMultiModelDeploymentLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_load_single_model_llama_8b_succeeds(self, mock_multi_model_manager, mock_apple_silicon_m4_max):
    ...
    @pytest.mark.asyncio 
    async def test_load_all_five_models_sequentially(self, mock_multi_model_manager, mock_apple_silicon_m4_max):
...
class TestMemoryManagementAndLimits:
    ...
    @pytest.mark.asyncio
    async def test_memory_limit_enforcement_prevents_overload(self, mock_multi_model_manager):
        ...
        models_to_load = ["llama_8b", "qwen_7b", "mistral_7b"]  # 16+14+14 = 44GB > 30GB limit
        ...
        assert manager.get_total_memory_usage() <= M4_MAX_MEMORY_GB * MEMORY_LIMIT_THRESHOLD
    ...
    @pytest.mark.asyncio
    async def test_cleanup_when_approaching_memory_limits(self, mock_multi_model_manager):
        ...
        with patch.object(manager, 'get_memory_pressure', return_value=0.9):  # 90% usage
...
class TestConcurrentModelServing:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_inference_different_models(self, mock_multi_model_manager):
    ...
    @pytest.mark.asyncio
    async def test_request_routing_based_on_content_type(self, mock_multi_model_manager):
...
class TestAppleSiliconOptimization:
    ...
    @pytest.mark.asyncio
    async def test_metal_backend_activation_all_models(self, mock_multi_model_manager, mock_metal_backend):
    ...
    async def test_quantization_effectiveness_per_model(self, mock_multi_model_manager):
        ...
        for model_key, config in MULTI_MODEL_CONFIG.items():
            ...
            assert quant_stats.performance_impact_percent < 0.1  # Less than 10% slowdown
        
        # Verify all models benefit from quantization
        assert len(quantization_results) == 5
    
    @pytest.mark.asyncio
    async def test_unified_memory_utilization_efficiency(self, mock_multi_model_manager, mock_apple_silicon_m4_max):
    ...
    @pytest.mark.asyncio
    async def test_hardware_specific_performance_metrics(self, mock_multi_model_manager):
...
class TestErrorHandlingAndRecovery:
    ...
    async def test_model_loading_failure_recovery(self, mock_multi_model_manager):
        ...
        with patch.object(manager, '_load_model_impl', side_effect=Exception("Loading failed")):
            result = await manager.load_model(
                model_name=MULTI_MODEL_CONFIG["llama_8b"]["name"],
                content_type="general"
            )
    ...
    async def test_out_of_memory_error_handling(self, mock_multi_model_manager):
        ...
        with patch.object(manager, '_check_available_memory', return_value=1.0):  # 1GB available
            result = await manager.load_model(
    ...
    async def test_circuit_breaker_behavior_multi_model(self, mock_multi_model_manager):
        ...
        with patch.object(manager, '_generate_impl', side_effect=Exception("Generation failed")):
    ...
    @pytest.mark.asyncio
    async def test_graceful_degradation_when_models_unavailable(self, mock_multi_model_manager):
...
class TestIntegrationScenarios:
    ...
    @pytest.mark.asyncio
    async def test_full_multi_model_workflow(self, mock_multi_model_manager, mock_apple_silicon_m4_max):
        ...
        async def background_inference():
            ...
            for i in range(10):
                try:
                    response = await manager.generate(
                        content_type="speed",
                        messages=[{"role": "user", "content": f"Background request {i}"}]
                    )
