...
class ModelCacheManager:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        logger.info(
            f"CacheManager initialized: {self.cache_dir} (max {self.max_cache_size_gb}GB)"
        )
    ...
    async def preload_priority_models(self) -> Dict[str, bool]:
        ...
        models_by_priority = sorted(
            self.registry.models.items(), key=lambda x: x[1].priority
        )
    ...
    async def optimize_for_content_types(
    ...
    async def intelligent_cleanup(self) -> CacheOptimizationResult:
        ...
        for model_id in models_to_remove:
            try:
                ...
                if model_cache_dir.exists():
                    ...
                    logger.info(
                        f"Removed cached model: {model_id} ({model_size:.1f}GB)"
                    )
        ...
        logger.info(
            f"Cache cleanup completed: removed {removed_count} models, freed {space_freed:.1f}GB"
        )
        return result
        ...
        for cache_info in cached_models:
            try:
                ...
                if last_accessed < cutoff_date:
                    ...
                    logger.debug(
                        f"Cleanup candidate: {cache_info.model_id} (not used since {cache_info.last_accessed})"
                    )
            except Exception:
    ...
    def _generate_optimization_recommendations(self) -> List[str]:
        ...
        if hit_rate < self.cache_hit_threshold:
            recommendations.append(
                f"Cache hit rate ({hit_rate:.1%}) below target ({self.cache_hit_threshold:.1%})"
            )
        ...
        if self.max_cache_size_gb > 150:
            recommendations.append(
                "Consider reducing max cache size for M4 Max optimization"
            )
        ...
        if total_models > 5:
            recommendations.append(
                "Consider model pruning - focus on top 3-5 models for best performance"
            )
...
class M4MaxCacheOptimizer:
    ...
    async def optimize_for_m4_max(self) -> Dict[str, Any]:
        ...
        optimization_results["memory_efficiency"] = min(
            ...
            (cache_stats.total_size_gb / self.unified_memory_gb)
            * 2,  # Rough efficiency metric
        )
    ...
    async def _optimize_memory_usage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_target_gb": self.unified_memory_gb
            * 0.2,  # 20% of unified memory for cache metadata
        }
