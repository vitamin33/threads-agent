...
class ModelDownloader:
    ...
    async def download_model(
        ...
        """
        if not HF_AVAILABLE:
            raise RuntimeError("HuggingFace Hub not available - install transformers and huggingface_hub")
        ...
        try:
            ...
            logger.info(f"✅ Model download completed: {model_id} in {download_time:.1f}s")
            return cache_info
    ...
    async def _download_with_progress(
        ...
        try:
            ...
            for step in range(total_steps):
                ...
                progress.downloaded_gb = (progress.total_size_gb * progress.progress_percent) / 100
    ...
    async def _ensure_storage_space(self, required_gb: float):
        ...
        logger.info(f"Insufficient storage space. Need {required_gb:.1f}GB, have {available_space:.1f}GB")
    ...
    async def _cleanup_old_models(self, target_free_gb: float):
        ...
        for model_info in cached_models:
            ...
            try:
                ...
                if model_info.cache_path.exists():
                    ...
                    logger.info(f"Removed cached model: {model_info.model_id} ({model_info.size_gb:.1f}GB)")
        ...
        logger.info(f"Cleanup completed: freed {freed_space:.1f}GB by removing {len(cleaned_models)} models")
        ...
        if model_cache_path.exists():
            try:
                ...
                return ModelCacheInfo(
                    model_id=model_id,
                    model_name=self.registry.get_model_config(model_id).name if self.registry.get_model_config(model_id) else model_id,
        ...
        for model_dir in self.cache_dir.iterdir():
            if model_dir.is_dir():
                try:
                    ...
                    logger.warning(f"Failed to get info for cached model {model_dir.name}: {e}")
    ...
    async def download_all_registry_models(
        ...
        for model_id, config in models_by_priority:
            try:
                ...
                cache_info = await self.download_model(model_id, model_progress_callback)
        ...
        logger.info(f"Download batch completed: {len(results)}/{len(models_by_priority)} successful")
        return results
    ...
    async def verify_all_models(self) -> Dict[str, bool]:
        ...
        for cache_info in cached_models:
            try:
                # Re-verify integrity
                verification_hash = await self._verify_model_integrity(cache_info.cache_path)
                verification_results[cache_info.model_id] = (verification_hash == cache_info.verification_hash)
...
class CacheManager:
    ...
    async def preload_for_content_types(self, content_types: List[ContentType]) -> Dict[str, bool]:
        ...
        logger.info(f"Preloading models for content types: {[ct.value for ct in content_types]}")
    ...
    async def optimize_cache_for_m4_max(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Verified: {valid_models} models, Cleaned: {len(removed_models)} models")
