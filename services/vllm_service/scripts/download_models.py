...
from services.vllm_service.model_downloader import get_model_downloader, DownloadProgress
...
class DownloadOrchestrator:
    ...
    def progress_callback(self, model_id: str, progress: DownloadProgress):
        ...
        print(f"  {model_id}: {progress.progress_percent:.1f}% ({progress.downloaded_gb:.1f}/{progress.total_size_gb:.1f}GB)")
    ...
    async def download_all_models(self) -> Dict[str, bool]:
        ...
        for model_id in models:
            ...
            print(f"   Priority: {config.priority}, Memory: {config.memory_requirements.base_gb:.1f}GB")
            
            try:
                cache_info = await self.downloader.download_model(
                    model_id, 
                    lambda p: self.progress_callback(model_id, p)
                )
    ...
    async def download_for_content_type(self, content_type: ContentType) -> Dict[str, bool]:
    ...
    async def optimize_cache(self) -> None:
        ...
        print(f"   Hit rate improvement: +{cleanup_results.cache_hit_rate_improvement:.1%}")
...
async def main():
    ...
    parser.add_argument("--optimize", action="store_true", help="Run cache optimization")
    ...
    try:
        if args.stats:
            ...
            cache_info = await orchestrator.downloader.download_model(
                model_id,
                lambda p: orchestrator.progress_callback(model_id, p)
            )
            print(f"✅ Downloaded: {cache_info.size_gb:.1f}GB")
