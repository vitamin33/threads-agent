...
class LitmusChaosManager:
    ...
    async def create_chaos_experiment(
        self, experiment_type: ExperimentType, experiment_spec: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if experiment_type == ExperimentType.NETWORK_PARTITION:
            env_vars = chaos_engine_spec["spec"]["experiments"][0]["spec"][
                "components"
            ]["env"]
    ...
    async def wait_for_experiment_completion(
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout:
            ...
            if status.get("status", {}).get("phase") in [
                "Completed",
                "Failed",
                "Stopped",
            ]:
