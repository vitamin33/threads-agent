...
class TestChaosExperimentExecutor:
    ...
    @pytest.mark.asyncio
    async def test_execute_experiment_with_safety_violation_returns_failed_result(self, executor):
    ...
    @pytest.mark.asyncio
    async def test_execute_experiment_updates_prometheus_metrics(self, executor, mock_prometheus_client):
        ...
        metric_calls = [call[0][0] for call in mock_prometheus_client.inc.call_args_list]
    ...
    async def test_emergency_stop_during_experiment_execution(self, executor):
        ...
        experiment_task = asyncio.create_task(executor.execute_experiment(experiment_config))
