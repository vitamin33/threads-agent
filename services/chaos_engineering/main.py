...
class ExperimentRequest(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Type of experiment (pod_kill, network_partition, cpu_stress, memory_pressure)",
    )
    ...
    safety_threshold: Optional[float] = Field(
        0.8, description="Safety threshold (0.0-1.0)"
    )
...
async def create_experiment(
    experiment: ExperimentRequest, background_tasks: BackgroundTasks
):
...
async def list_experiments():
    ...
    if litmus_manager is None:
        raise HTTPException(
            status_code=503, detail="LitmusChaos manager not initialized"
        )
...
@app.get(
    "/api/v1/experiments/{experiment_name}", response_model=ExperimentStatusResponse
)
...
async def get_system_health():
    ...
    try:
        ...
        return {
            ...
            "status": "healthy"
            if health_score >= chaos_executor.safety_threshold
            else "degraded",
        }
