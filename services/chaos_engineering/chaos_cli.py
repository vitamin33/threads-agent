...
from chaos_experiment_executor import ChaosExperimentExecutor, ExperimentState
...
@click.option('--type', 'experiment_type', 
              type=click.Choice(['pod_kill', 'network_partition', 'cpu_stress', 'memory_pressure']),
...
@click.option('--safety-threshold', type=float, default=0.8, help='Safety threshold (0.0-1.0)')
@click.option('--output', type=click.Choice(['text', 'json']), default='text', help='Output format')
def run(experiment_type, name, namespace, target_app, duration, config, safety_threshold, output):
    ...
    try:
        ...
        if config:
            ...
            if not experiment_type:
                raise click.ClickException("Experiment type is required when not using config file")
            if not name:
                raise click.ClickException("Experiment name is required when not using config file")
            if not target_app:
                raise click.ClickException("Target app is required when not using config file")
        ...
        if experiment_config.get('type') not in valid_types:
            raise click.ClickException(f"Invalid experiment type. Must be one of: {', '.join(valid_types)}")
        ...
        if output == 'json':
            ...
            click.echo(f"Experiment {result.experiment_name} completed with status: {result.status.value}")
            click.echo(f"Execution time: {result.execution_time:.1f} seconds")
            click.echo(f"Safety checks: {'PASSED' if result.safety_checks_passed else 'FAILED'}")
...
def create(yaml_file, namespace):
    ...
    try:
        ...
        click.echo("Note: This is a dry-run. Use appropriate tools to apply the ChaosEngine to Kubernetes.")
