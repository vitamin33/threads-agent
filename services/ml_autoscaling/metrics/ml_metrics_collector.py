...
class MLMetricsCollector:
    ...
    async def collect_inference_metrics(
        ...
        if result["status"] == "success" and result["data"]["result"]:
            metrics.p95_latency_ms = (
                float(result["data"]["result"][0]["value"][1]) * 1000
            )

        # Query request rate
        rps_query = (
            f'rate(vllm_requests_total{{job="{service_name}"}}[{lookback_minutes}m])'
        )
    ...
    async def calculate_scaling_recommendation(
        ...
        if inference_metrics.p95_latency_ms > 500:
            ...
            recommendation.reason = (
                f"High P95 latency: {inference_metrics.p95_latency_ms:.0f}ms"
            )
        ...
        # Check for high GPU utilization
            ...
            recommendation.reason = (
                f"High GPU utilization: {gpu_metrics.avg_utilization:.1f}%"
            )
    ...
    async def get_batch_processing_metrics(self) -> BatchProcessingMetrics:
        ...
        if result["status"] == "success" and result["data"]["result"]:
            ...
            metrics.estimated_processing_time_minutes = metrics.queue_size / (
                metrics.items_per_second * 60
            )
    ...
    async def get_predictive_metrics(
        ...
        result = await self.prometheus.query_range(
            metric_name, start_time, end_time, step="5m"
        )
    ...
    async def get_ml_workload_metrics(self) -> MLWorkloadMetrics:
        ...
        inference_load = await self.prometheus.query(
            "sum(rate(model_inference_requests_total[5m]))"
        )
        if inference_load["status"] == "success" and inference_load["data"]["result"]:
            metrics.total_inference_load = float(
                inference_load["data"]["result"][0]["value"][1]
            )
        ...
        if training_jobs["status"] == "success" and training_jobs["data"]["result"]:
            metrics.active_training_jobs = int(
                float(training_jobs["data"]["result"][0]["value"][1])
            )
    ...
    async def detect_anomaly(
        ...
        result = await self.prometheus.query_range(
            metric_name, start_time, end_time, step="1m"
        )
    ...
    @classmethod
    def for_multi_cluster(
        cls, cluster_configs: List[Dict[str, str]]
