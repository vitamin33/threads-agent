...
class PredictiveScaler:
    ...
    async def predict_scaling_needs(
        ...
        reasoning = self._generate_reasoning(
            patterns, forecasts, current_replicas, predicted_replicas
        )
    ...
    def _detect_trend(self, values: np.ndarray) -> Optional[HistoricalPattern]:
        ...
        if std_dev > mean_value * 0.5:
            return HistoricalPattern(
                pattern_type="volatile", confidence=0.7, trend=TrendType.VOLATILE
            )

        return HistoricalPattern(
            pattern_type="stable", confidence=0.9, trend=TrendType.STABLE
        )
    ...
    def _detect_anomalies(self, values: np.ndarray) -> Optional[HistoricalPattern]:
        ...
        if (
            outlier_ratio > 0.05 or extreme_outliers > 0
        ):  # More than 5% outliers or any extreme outlier
            confidence = min(0.9, max(outlier_ratio * 5, extreme_outliers * 0.5))
            return HistoricalPattern(
                pattern_type="anomaly_detected", confidence=confidence
            )
    ...
    async def _generate_forecast(
        ...
        for minutes_ahead in range(0, forecast_horizon_minutes, 5):
            ...
            # Apply daily pattern
            daily_pattern = next(
                (p for p in patterns if p.pattern_type == "daily_cycle"), None
            )
            if daily_pattern:
            ...
            weekly_pattern = next(
                (p for p in patterns if p.pattern_type == "weekly_cycle"), None
            )
            if weekly_pattern:
            ...
            # Apply trend
            trend_pattern = next(
                (p for p in patterns if p.pattern_type == "trend"), None
            )
    ...
    def _calculate_predicted_replicas(
        self, forecasts: List[WorkloadForecast], current_replicas: int
    ) -> int:
        ...
        return max(
            self.policy.min_replicas, min(self.policy.max_replicas, max_replicas)
        )

    def _calculate_confidence(
        self, patterns: List[HistoricalPattern], forecasts: List[WorkloadForecast]
    ) -> float:
        ...
        pattern_confidence = (
            np.mean([p.confidence for p in patterns]) if patterns else 0.5
        )
    ...
    def _generate_reasoning(
        ...
        for pattern in patterns:
            if pattern.pattern_type == "daily_cycle":
                reasons.append(
                    f"Daily cycle detected with {pattern.confidence:.0%} confidence"
                )
            elif pattern.pattern_type == "weekly_cycle":
                reasons.append("Weekly pattern detected (weekday/weekend difference)")
        ...
        if forecasts:
            ...
            if max_load > min_load * 2:
                reasons.append(
                    f"Significant load variation expected ({min_load:.0f}-{max_load:.0f})"
                )

        # Scaling decision
        if predicted_replicas > current_replicas:
            reasons.append(
                f"Scale up recommended: {current_replicas} → {predicted_replicas} replicas"
            )
        elif predicted_replicas < current_replicas:
            reasons.append(
                f"Scale down possible: {current_replicas} → {predicted_replicas} replicas"
            )
        else:
    ...
    async def should_scale_proactively(self, prediction: PredictionResult) -> bool:
        ...
        if prediction.scale_up_at:
            minutes_until_scale = (
                prediction.scale_up_at - datetime.now()
            ).total_seconds() / 60
