...
def add_achievement():
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            print(
                f"   Performance Improvement: {result.get('performance_improvement_pct')}%"
            )
            print("")
            print("📊 Key Metrics Stored:")
            print("   - P95 Latency: 59ms (93% improvement)")
            print("   - Throughput: 920 RPS (9.2x improvement)")
            print("   - Error Rate: 0% (perfect reliability)")
            print("")
        ...
        print(
            "  2. Port forward: kubectl port-forward svc/achievement-collector 8001:8001"
        )
        print(
            "  3. Or run locally: cd services/achievement_collector && uvicorn main:app --port 8001"
        )
        return False
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("")
        print(
            "🚀 Your 59ms latency achievement is now part of your professional portfolio!"
        )
    else:
        print("")
        print(
            "Please ensure the achievement_collector service is running and try again."
        )
