...
def port_forwards() -> Iterator[None]:
    """Establish local forwards to all services needed for e2e tests."""
    pf1 = _port_forward("orchestrator", ORCH_PORT, 8080)
    pf2 = _port_forward("fake-threads", THREADS_PORT, 9009)
    pf3 = _port_forward("qdrant", QDRANT_PORT, 6333)
    pf4 = _port_forward("postgres", POSTGRES_PORT, 5432)
    time.sleep(3)  # give k8s a moment for all port forwards
    try:
        yield
    finally:
        pf1.terminate()
