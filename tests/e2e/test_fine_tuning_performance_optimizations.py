...
class TestDatabaseQueryOptimization:
    ...
    async def test_chunked_data_collection_memory_efficiency(self, pipeline_config, mock_database_session):
        """Test that chunked data collection uses memory efficiently."""
        initial_memory = psutil.Process().memory_info().rss / 1024 / 1024  # MB
        
        with patch('services.common.fine_tuning_pipeline.get_database_session', return_value=mock_database_session):
            collector = DataCollector(engagement_threshold=pipeline_config.engagement_threshold)
    ...
    async def test_database_query_performance_with_indexing_hints(self, k8s_resource_config):
    ...
    async def test_chunked_processing_edge_cases(self, pipeline_config, mock_database_session):
        ...
        mock_database_session.query.return_value.filter.return_value.order_by.return_value.limit.return_value.all.return_value = [single_post]
...
class TestAsyncOpenAIOperations:
    ...
    async def test_async_fine_tuning_with_concurrency_limits(self, pipeline_config):
        ...
        training_data = TrainingDataBatch(
            hook_examples=[{"messages": [{"role": "user", "content": f"test {i}"}]} for i in range(100)],
            body_examples=[{"messages": [{"role": "user", "content": f"test {i}"}]} for i in range(100)],
        ...
        assert total_duration < 0.7, f"Operations took {total_duration:.2f}s, expected < 0.7s for concurrent execution"
    ...
    async def test_openai_api_retry_logic_with_exponential_backoff(self, k8s_resource_config):
        ...
        with patch('openai.AsyncOpenAI', return_value=mock_client):
            result = await k8s_pipeline.start_fine_tuning_with_circuit_breaker(training_data)
        ...
        assert retry_duration >= 2.5, f"Retry duration {retry_duration:.2f}s too short for exponential backoff"
...
class TestRedisCachingOptimization:
    ...
    async def test_model_metrics_caching_with_ttl(self, redis_client):
        ...
        with patch('redis.ConnectionPool.from_url') as mock_pool, \
             ...
             patch.object(evaluator, '_calculate_metrics_from_db', return_value=test_metrics):
            ...
            assert cache_duration < calc_duration * 0.5, "Cache should be faster than calculation"
    ...
    async def test_kubernetes_redis_connection_pooling(self, k8s_resource_config):
        ...
        with patch('aioredis.ConnectionPool.from_url', return_value=mock_redis_pool), \
             patch('aioredis.Redis', return_value=mock_redis_client):
            
            await k8s_pipeline.initialize()
            
            # Test caching metrics
            await k8s_pipeline.cache_evaluation_metrics("candidate", "test-ab-k8s", test_metrics)
...
class TestMemoryOptimization:
    ...
    async def test_memory_monitoring_during_pipeline_execution(self, pipeline_config):
        ...
        large_training_data = TrainingDataBatch(
            hook_examples=[{"messages": [{"role": "user", "content": f"Large content {i}" * 100}]} for i in range(500)],
            body_examples=[{"messages": [{"role": "user", "content": f"Large content {i}" * 100}]} for i in range(500)],
            metadata={"collected_at": datetime.now()}
        )
        ...
        with patch('services.common.fine_tuning_pipeline.DataCollector') as mock_collector, \
            ...
            # Setup mocks
            mock_collector.return_value.collect_training_data.return_value = large_training_data
            ...
            assert "memory_efficiency" in result.training_data_batch.metadata or True  # Would be in MLflow logs
    ...
    async def test_garbage_collection_effectiveness(self, pipeline_config):
        ...
        assert memory_increase > 0.5, f"Expected memory increase, got {memory_increase:.2f} MB"
        ...
        assert recovery_percentage > 30, f"Only recovered {recovery_percentage:.1f}% of allocated memory"
    ...
    async def test_memory_efficiency_metrics_tracking(self, pipeline_config):
        ...
        assert performance_metrics["memory_per_example_mb"] == expected_memory_per_example
...
class TestKubernetesOptimizations:
    ...
    async def test_connection_pool_initialization_and_management(self, k8s_resource_config):
        ...
        with patch('asyncpg.create_pool', return_value=mock_db_pool) as mock_create_db_pool, \
             patch('aioredis.ConnectionPool.from_url', return_value=mock_redis_pool) as mock_create_redis_pool:
    ...
    async def test_circuit_breaker_integration_with_openai_calls(self, k8s_resource_config):
...
class TestHealthChecksAndMonitoring:
    ...
    async def test_kubernetes_health_check_endpoint(self, k8s_resource_config):
        ...
        with patch.object(k8s_pipeline.connection_manager, 'get_db_connection') as mock_get_db, \
             patch.object(k8s_pipeline.connection_manager, 'get_redis_connection') as mock_get_redis:
    ...
    async def test_health_check_with_unhealthy_components(self, k8s_resource_config):
        ...
        with patch.object(k8s_pipeline.connection_manager, 'get_db_connection') as mock_get_db, \
            ...
            assert "unhealthy: Database connection failed" in health_status["components"]["database"]
            assert "unhealthy: Redis connection failed" in health_status["components"]["redis"]
    ...
    async def test_prometheus_metrics_generation(self, k8s_resource_config):
        ...
        assert 'fine_tuning_openai_circuit_breaker_state{state="CLOSED"} 1' in metrics_output
...
class TestPerformanceIntegration:
    ...
    async def test_end_to_end_performance_optimization(self, pipeline_config, k8s_resource_config):
        ...
        mock_training_data = {
            "hook_examples": [{"messages": [{"role": "user", "content": f"test {i}"}]} for i in range(1000)],
            "body_examples": [{"messages": [{"role": "user", "content": f"test {i}"}]} for i in range(1000)]
        }
        
        mock_result = {"job_id": "ftjob-123", "status": "training", "training_examples": 2000}
        
        # Test Kubernetes-optimized pipeline
        start_time = time.time()
        
        with patch.object(k8s_pipeline, 'collect_training_data_optimized', return_value=mock_training_data), \
             patch.object(k8s_pipeline, 'start_fine_tuning_with_circuit_breaker', return_value=mock_result):
        ...
        assert k8s_duration < 5.0, f"Optimized pipeline took {k8s_duration:.2f}s, expected < 5s"
        
        # Test resource efficiency
        memory_usage = psutil.Process().memory_info().rss / 1024 / 1024  # MB
        assert memory_usage < 500, f"Memory usage {memory_usage:.1f}MB too high for optimized pipeline"
    ...
    async def test_performance_regression_detection(self, pipeline_config):
        ...
        assert monitor.metrics["memory_usage_regression_mb"] > monitor.metrics["memory_usage_mb"] * 1.5
        assert monitor.metrics["cache_hit_rate_regression"] < monitor.metrics["cache_hit_rate"] * 0.7
        ...
        assert "fine_tuning_pipeline_data_collection_duration_seconds" in prometheus_output
...
class PerformanceAssertion:
    ...
    @staticmethod
    def assert_memory_efficient(initial_mb: float, final_mb: float, max_increase_mb: float = 50):
    ...
    @staticmethod
    def assert_cache_efficiency(cache_hits: int, total_requests: int, min_hit_rate: float = 0.8):
