...
class TestCommentMonitorK8sResourceConstraints:
    ...
    def resource_constrained_comment_monitor(self, mock_k8s_resource_monitor):
        ...
        class ResourceConstrainedCommentMonitor(CommentMonitor):
            ...
            def _simulate_cpu_intensive_operation(self, duration_ms: float):
                ...
                # Simulate CPU usage
                cpu_usage = min(50 + (duration_ms / 10), 100)  # Scale CPU with processing time
                memory_usage = self.resource_monitor.current_memory_mb + (duration_ms / 100)
            ...
            def _deduplicate_comments_with_resource_monitoring(self, comments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                """Enhanced deduplication with resource monitoring."""
                try:
                    # Simulate resource usage based on comment count
                    processing_intensity = min(len(comments) * 0.5, 100)  # 0.5ms per comment
            ...
            def get_performance_metrics(self) -> Dict[str, float]:
                ...
                throughput = len(self.processing_times) / (sum(self.processing_times) / 1000) if self.processing_times else 0
    ...
    def test_performance_under_cpu_limits(
        self, 
        resource_constrained_comment_monitor, 
        mock_k8s_resource_monitor
    ):
        """
        ...
        for i in range(0, len(test_comments), batch_size):
            ...
            start_time = time.time()
            unique_comments = resource_constrained_comment_monitor._deduplicate_comments_with_resource_monitoring(batch)
        ...
        performance_metrics = resource_constrained_comment_monitor.get_performance_metrics()
        ...
        avg_cpu_usage = sum(b["resource_metrics"].cpu_usage_percent for b in batch_results) / len(batch_results)
        ...
        print(f"CPU constraint test: {avg_cpu_usage:.1f}% avg CPU, {performance_metrics['throughput_ops_per_second']:.1f} ops/s")
    ...
    def test_memory_usage_under_memory_limits(
        ...
        for i in range(100):
            ...
            large_text = f"Large memory test comment {i} with extensive content. " * 50  # ~2.5KB per comment
        ...
        for i in range(0, len(large_comments), batch_size):
            ...
            memory_snapshots.append({
                ...
                "memory_delta": post_metrics.memory_usage_mb - pre_metrics.memory_usage_mb,
                "processing_time": processing_time,
                "success": success,
                "processed_count": len(unique_comments)
            })
        ...
        assert final_metrics.oom_kills == 0, f"OOM kills occurred: {final_metrics.oom_kills}"
        ...
        assert success_rate > 0.8, f"Success rate {success_rate:.1%} too low under memory constraints"

        # Memory growth per batch should be reasonable
        avg_memory_delta = sum(s["memory_delta"] for s in memory_snapshots) / len(memory_snapshots)
        assert avg_memory_delta < 20, f"Average memory growth {avg_memory_delta:.2f}MB per batch too high"

        print(f"Memory constraint test: {peak_memory:.2f}MB peak, {success_rate:.1%} success rate")
    ...
    def test_concurrent_processing_under_resource_limits(
        ...
        def process_comments_with_constraints(worker_id: int):
            ...
            start_time = time.time()
            try:
                unique_comments = resource_constrained_comment_monitor._deduplicate_comments_with_resource_monitoring(comments)
                processing_time = time.time() - start_time
        ...
        assert success_rate > 0.7, f"Concurrent success rate {success_rate:.1%} too low under resource limits"
        ...
        if successful_workers:
            avg_processing_time = sum(r["processing_time"] for r in successful_workers) / len(successful_workers)
        ...
        print(f"Concurrent constraint test: {success_rate:.1%} success, "
    ...
    def test_resource_scaling_behavior(
        ...
        for scenario in resource_scenarios:
            ...
            start_time = time.time()
            try:
                unique_comments = resource_constrained_comment_monitor._deduplicate_comments_with_resource_monitoring(test_comments)
                processing_time = time.time() - start_time
                success = True
                error_message = None
            except Exception as e:
            ...
            scaling_results.append({
                ...
                "processing_time": processing_time,
                "processed_count": len(unique_comments),
                "throughput": len(unique_comments) / processing_time if processing_time > 0 else 0,
        ...
        # Scaling assertions
        assert len(successful_scenarios) >= 3, "At least 3 resource scenarios should succeed"
        ...
        if len(successful_scenarios) >= 2:
            ...
            high_resource_throughput = max(r["throughput"] for r in successful_scenarios)
            
            scaling_factor = high_resource_throughput / low_resource_throughput if low_resource_throughput > 0 else 1
        ...
        high_resource_success_rate = sum(1 for r in high_resource_scenarios if r["success"]) / len(high_resource_scenarios) if high_resource_scenarios else 0
        ...
        for result in scaling_results:
            print(f"Scaling {result['scenario']}: "
    ...
    def test_resource_monitoring_and_alerting(
        ...
        for i in range(0, len(high_load_comments), batch_size):
            ...
            # Process batch
            resource_constrained_comment_monitor._deduplicate_comments_with_resource_monitoring(batch)
        ...
        assert alert_rate < 0.8, f"Alert rate {alert_rate:.1%} too high, may indicate over-sensitive thresholds"

        # Critical alerts (OOM kills) should be minimal
        assert len(oom_alerts) <= 1, f"Too many OOM kill alerts: {len(oom_alerts)}"

        print(f"Resource monitoring test: {len(cpu_alerts)} CPU alerts, "
