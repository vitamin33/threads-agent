...
class TestCommentMonitorChaosEngineering:
    ...
    def chaos_infrastructure_simulator(self):
        ...
        class ChaosInfrastructureSimulator:
            ...
            def inject_resource_exhaustion(self, resource_type: str, exhaustion_level: float, duration: float):
    ...
    def chaos_comment_monitor(self, chaos_infrastructure_simulator):
        ...
        class ChaosCommentMonitor(CommentMonitor):
            ...
            def _simulate_service_call(self, service: str, operation: Callable, *args, **kwargs):
                ...
                try:
                    ...
                    if self.chaos_simulator.should_corrupt_data():
                        ...
                        if isinstance(result, list):
                            ...
                            for item in result[:len(result)//4]:  # Corrupt 25% of data
            ...
            def _deduplicate_comments_with_chaos(self, comments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                """Deduplication with chaos simulation."""
                return self._simulate_service_call("database", self._deduplicate_comments, comments)

            def _store_comments_with_chaos(self, comments: List[Dict[str, Any]], post_id: str):
                """Storage with chaos simulation."""
                return self._simulate_service_call("database", self._store_comments_in_db, comments, post_id)

            def _queue_comments_with_chaos(self, comments: List[Dict[str, Any]], post_id: str):
                """Queuing with chaos simulation."""
                return self._simulate_service_call("celery", self._queue_comments_for_analysis, comments, post_id)
            ...
            def get_chaos_metrics(self) -> Dict[str, Any]:
                ...
                return {
                    ...
                    "success_rate": len(successful_requests) / len(self.request_metrics),
    ...
    def test_database_outage_chaos_experiment(
        self, 
        chaos_comment_monitor, 
        chaos_infrastructure_simulator
    ):
        """
        ...
        chaos_infrastructure_simulator.inject_service_outage("database", experiment.duration_seconds)
        ...
        for i in range(0, len(test_comments), batch_size):
            ...
            try:
                unique_comments = chaos_comment_monitor._deduplicate_comments_with_chaos(batch)
                chaos_comment_monitor._store_comments_with_chaos(unique_comments, f"chaos_post_{i}")
                chaos_comment_monitor._queue_comments_with_chaos(unique_comments, f"chaos_post_{i}")
        ...
        outage_results = [r for r in results if r.get("processing_time", 0) < experiment.duration_seconds]
        if outage_results:
            outage_success_rate = len([r for r in outage_results if r["success"]]) / len(outage_results)
            assert outage_success_rate < 0.5, "Should have high failure rate during database outage"
        ...
        assert chaos_metrics["data_loss_events"] <= experiment.success_criteria["max_data_loss"], (
            f"Data loss events {chaos_metrics['data_loss_events']} exceed threshold"
        )

        print(f"Database outage chaos: {len(successful_batches)}/{len(results)} batches succeeded, "
    ...
    def test_network_partition_chaos_experiment(
        ...
        def process_batch(batch_id, comments):
            try:
                batch_start = time.time()
                unique_comments = chaos_comment_monitor._deduplicate_comments_with_chaos(comments)
                chaos_comment_monitor._store_comments_with_chaos(unique_comments, f"partition_batch_{batch_id}")
                chaos_comment_monitor._queue_comments_with_chaos(unique_comments, f"partition_batch_{batch_id}")
        ...
        if successful_batches:
            ...
            assert max_response_time <= experiment.success_criteria["max_response_time_ms"], (
                f"Max response time {max_response_time:.0f}ms exceeds limit"
            )

        # System should continue operating during partition
        assert chaos_metrics["total_requests"] > 0, "Should have attempted operations during partition"

        print(f"Network partition chaos: {success_rate:.1%} success rate, "
    ...
    def test_resource_exhaustion_chaos_experiment(
        ...
        chaos_infrastructure_simulator.inject_resource_exhaustion("cpu", 0.9, experiment.duration_seconds)
        chaos_infrastructure_simulator.inject_resource_exhaustion("memory", 0.8, experiment.duration_seconds)

        # Generate resource-intensive test workload
        large_comments = []
        for i in range(40):
            # Create comments with substantial content
            large_text = f"Resource exhaustion test comment {i} " * 100  # ~4KB per comment
        ...
        for i in range(0, len(large_comments), batch_size):
            ...
            try:
                unique_comments = chaos_comment_monitor._deduplicate_comments_with_chaos(batch)
                chaos_comment_monitor._store_comments_with_chaos(unique_comments, f"exhaustion_post_{i}")
        ...
        success_rate = len(successful_batches) / len(exhaustion_results) if exhaustion_results else 0
        ...
        if successful_batches:
            avg_exhaustion_time = sum(r["processing_time_ms"] for r in successful_batches) / len(successful_batches)
            degradation_factor = avg_exhaustion_time / baseline_time if baseline_time > 0 else 1
            
            assert degradation_factor <= experiment.success_criteria["max_degradation_factor"], (
                f"Performance degraded by {degradation_factor:.1f}x, exceeds limit"
            )

        # System should not crash under resource pressure
        assert chaos_metrics["total_requests"] > 0, "System should continue operating under resource exhaustion"

        print(f"Resource exhaustion chaos: {success_rate:.1%} success rate, "
    ...
    def test_data_corruption_chaos_experiment(
        ...
        chaos_infrastructure_simulator.inject_data_corruption(0.1, experiment.duration_seconds)
        ...
        for i in range(0, len(integrity_comments), batch_size):
            ...
            try:
                ...
                chaos_comment_monitor._store_comments_with_chaos(unique_comments, f"integrity_post_{i}")
        ...
        corruption_impact = data_integrity_violations / len(integrity_comments) if integrity_comments else 0
        assert corruption_impact <= experiment.success_criteria["max_corruption_impact"], (
            f"Data corruption impact {corruption_impact:.2%} exceeds maximum"
        )

        # Should detect corruption events
        assert chaos_metrics["data_loss_events"] > 0, "Should detect data corruption events"

        # Data integrity should be mostly preserved
        integrity_preserved = 1.0 - corruption_impact
        assert integrity_preserved >= experiment.success_criteria["data_integrity_preservation"], (
            f"Data integrity {integrity_preserved:.2%} below minimum threshold"
        )

        print(f"Data corruption chaos: {corruption_impact:.1%} corruption impact, "
    ...
    def test_combined_chaos_scenario(
        ...
        chaos_infrastructure_simulator.inject_latency("database", 500, experiment.duration_seconds)  # 500ms latency
        chaos_infrastructure_simulator.inject_resource_exhaustion("cpu", 0.7, experiment.duration_seconds)
        chaos_infrastructure_simulator.inject_data_corruption(0.05, experiment.duration_seconds)  # 5% corruption
        ...
        def process_chaos_batch(batch_id, comments):
            ...
            try:
                unique_comments = chaos_comment_monitor._deduplicate_comments_with_chaos(comments)
                chaos_comment_monitor._store_comments_with_chaos(unique_comments, f"combined_batch_{batch_id}")
                chaos_comment_monitor._queue_comments_with_chaos(unique_comments, f"combined_batch_{batch_id}")
        ...
        success_rate = len(successful_batches) / len(combined_results) if combined_results else 0
        ...
        assert mean_response_time <= experiment.success_criteria["max_mean_response_time"], (
            f"Mean response time {mean_response_time:.0f}ms exceeds limit under combined chaos"
        )

        # System should remain stable (no crashes)
        assert chaos_metrics["total_requests"] > 0, "System should remain operational under combined chaos"
        assert len(combined_results) > 0, "Should complete processing under combined chaos"
        ...
        print(f"Combined chaos experiment: {success_rate:.1%} success rate, "
