...
async def test_queue(connection_manager, test_exchange) -> str:
    ...
    try:
        ...
        channel.queue_bind(
            exchange=test_exchange, queue=queue_name, routing_key="test.event"
        )
        channel.close()
    except Exception as e:
...
def rabbitmq_management_api(rabbitmq_container):
    ...
    return {"get_queue_info": get_queue_info, "purge_queue": purge_queue}
...
def message_ordering_verifier():
    ...
    def verify_ordering(
        received_events: List[BaseEvent], expected_order: List[str]
    ) -> bool:
        ...
        for i, event in enumerate(received_events):
            if event.event_id != expected_order[i]:
                logger.error(
                    f"Order mismatch at position {i}: expected {expected_order[i]}, got {event.event_id}"
                )
                return False
...
class EventCapture:
    ...
    def create_handler(self, handler_name: str, should_fail: bool = False):
        ...
        async def handler(event: BaseEvent):
            self.handler_calls[handler_name] = (
                self.handler_calls.get(handler_name, 0) + 1
            )

            if should_fail:
                error = ValueError(
                    f"Handler {handler_name} failed for event {event.event_id}"
                )
            ...
            logger.info(
                f"Handler {handler_name} captured event {event.event_id} (call #{self.handler_calls[handler_name]})"
            )
