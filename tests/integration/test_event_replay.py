...
class TestEventReplay:
    ...
    async def test_replay_with_combined_filters(self, event_store):
        ...
        filtered_events = await event_store.replay_events(
            start_time=start_time, end_time=end_time, event_type="important.event"
        )
    ...
    async def test_replay_with_complex_payload_integrity(self, event_store):
        ...
        for i in range(10):
            ...
            event = BaseEvent(event_type="complex.payload", payload=complex_payload)
        ...
        for i, replayed_event in enumerate(replayed):
            ...
            assert (
                replayed_event.payload["user"]["id"]
                == original_event.payload["user"]["id"]
            )
            assert (
                replayed_event.payload["user"]["profile"]["name"]
                == original_event.payload["user"]["profile"]["name"]
            )
            assert (
                replayed_event.payload["user"]["profile"]["preferences"]
                == original_event.payload["user"]["profile"]["preferences"]
            )
            assert (
                replayed_event.payload["metadata"]["tags"]
                == original_event.payload["metadata"]["tags"]
            )
            assert (
                replayed_event.payload["metrics"]["score"]
                == original_event.payload["metrics"]["score"]
            )
            assert (
                replayed_event.payload["metrics"]["count"]
                == original_event.payload["metrics"]["count"]
            )
    ...
    async def test_concurrent_store_and_replay(self, event_store):
        ...
        replay_tasks = [
            replay_events() for _ in range(3)
        ]  # Multiple concurrent replays

        # Execute storage and replay concurrently
        results = await asyncio.gather(
            *store_tasks, *replay_tasks, return_exceptions=True
        )
