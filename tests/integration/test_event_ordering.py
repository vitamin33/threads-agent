...
class TestEventOrdering:
    ...
    async def test_fifo_ordering_single_consumer(
        ...
        async def order_preserving_handler(event: BaseEvent):
            ...
            logger.info(f"Received event {event.event_id}, sequence: {event.payload['sequence']}")
        ...
        await async_timeout(
            self._wait_for_events(received_events, num_events),
            10.0
        )
    ...
    async def test_timestamp_based_ordering_preservation(
        ...
        # Wait for processing
        await async_timeout(
            self._wait_for_events(received_events, len(events)),
            5.0
        )
        ...
        replayed_events = await event_store.replay_events(event_type="timestamp.ordering")
    ...
    async def test_ordering_with_processing_delays(
        ...
        await async_timeout(
            self._wait_for_processed_events(processed_events, num_events),
            20.0
        )
    ...
    async def test_ordering_under_high_load(
        ...
        expected_sequences = set(range(num_events))
        assert received_sequences == expected_sequences, "Some events were lost under high load"
        ...
        for batch, positions in batches_received.items():
            ...
            assert positions == expected_positions, f"Batch {batch} lost internal ordering"
    ...
    async def test_ordering_across_reconnections(
        ...
        async def reconnection_handler(event: BaseEvent):
            received_events.append(event)
            logger.info(f"Received event {event.payload['sequence']} after reconnection test")
        ...
        await async_timeout(
            self._wait_for_events(received_events, first_batch_size),
            3.0
        )
        ...
        await async_timeout(
            self._wait_for_events(received_events, total_expected),
            5.0
        )
        ...
        first_batch_events = [e for e in received_events if e.payload["batch"] == "first"]
        second_batch_events = [e for e in received_events if e.payload["batch"] == "second"]
    ...
    async def test_ordering_with_multiple_routing_keys(
        ...
        try:
            ...
            channel.queue_bind(exchange=test_exchange, queue=priority_queue, routing_key="priority.event")
            channel.queue_bind(exchange=test_exchange, queue=normal_queue, routing_key="normal.event")
            channel.close()
        ...
        async def routing_handler(event: BaseEvent):
            received_events.append(event)
            logger.info(f"Received {event.payload['priority']} event {event.payload['sequence']}")
        ...
        # Wait for all events to be processed
        await async_timeout(
            self._wait_for_events(received_events, len(events_data)),
            8.0
        )
        ...
        for priority, sequences in groups.items():
            if len(sequences) > 1:
                ...
                original_sequences = [event.payload["sequence"] for event in published_events 
                                    if event.payload["priority"] == priority]
                assert sequences == sorted(original_sequences), f"Ordering lost in {priority} group"
    ...
    async def _wait_for_processed_events(self, processed_list: List[Tuple], expected_count: int):
