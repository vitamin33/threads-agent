...
class TestEndToEndEventFlow:
    ...
    async def test_multiple_events_sequential_flow(
        ...
        for original_event in sample_events:
            ...
            assert retrieved is not None, (
                f"Event {original_event.event_id} not found in store"
            )
    ...
    async def test_concurrent_publishing_and_consuming(
        ...
        events = [
            BaseEvent(
                event_type="load.test", payload={"sequence": i, "batch": "concurrent"}
            )
        ...
        assert successful_publishes == num_events, (
            f"Only {successful_publishes}/{num_events} events published successfully"
        )

        # Wait for all events to be consumed
        await async_timeout(
            self._wait_for_events(event_capture, expected_count=num_events), 15.0
        )
        ...
        received_sequences = {
            event.payload["sequence"] for event in event_capture.events
        }
        expected_sequences = set(range(num_events))
        assert received_sequences == expected_sequences, (
            "Some events were lost or duplicated"
        )
        ...
        for i, original_event in enumerate(events):
            ...
            handled_event = next(
                (
                    ...
                    if e.event_id == original_event.event_id
                ),
                None,
            )
    ...
    async def _wait_for_events(
        self, event_capture, expected_count: int, check_interval: float = 0.1
    ):
