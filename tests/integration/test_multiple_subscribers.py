...
class TestMultipleSubscribers:
    ...
    async def test_multiple_handlers_same_event_type(
        ...
        test_event = BaseEvent(
            event_type="multi.handler.test",
            payload={"data": "shared event"}
        )
        ...
        # Wait for processing
        await async_timeout(self._wait_for_handler_calls(event_capture, expected_calls=3), 3.0)
    ...
    async def test_multiple_subscribers_same_queue(
        ...
        def create_handler_for_capture(capture, handler_name):
            async def handler(event: BaseEvent):
                ...
                capture["handler_calls"][handler_name] = capture["handler_calls"].get(handler_name, 0) + 1
        ...
        total_processed = len(capture_1["events"]) + len(capture_2["events"]) + len(capture_3["events"])
        ...
        processed_counts = [len(capture_1["events"]), len(capture_2["events"]), len(capture_3["events"])]
        ...
        assert active_subscribers >= 2, "Load balancing should distribute events across multiple subscribers"
        
        # Verify no duplicate processing (each event processed exactly once)
        all_processed_ids = set()
        for capture in [capture_1, capture_2, capture_3]:
            for event in capture["events"]:
                assert event.event_id not in all_processed_ids, f"Duplicate processing of event {event.event_id}"
    ...
    async def test_selective_event_handling_multiple_subscribers(
        ...
        def create_specialized_handler(capture, handler_name, event_types: Set[str]):
            async def handler(event: BaseEvent):
                if event.event_type in event_types:
                    capture["events"].append(event)
                    capture["handler_calls"][handler_name] = capture["handler_calls"].get(handler_name, 0) + 1
                    logger.info(f"Specialized handler {handler_name} processed {event.event_type}")
            
            handler.__name__ = handler_name
            return handler
        
        # Register specialized handlers
        user_handler = create_specialized_handler(user_capture, "user_specialist", {"user.created", "user.updated"})
        order_handler = create_specialized_handler(order_capture, "order_specialist", {"order.placed", "order.shipped"})
        notification_handler = create_specialized_handler(notification_capture, "notification_specialist", {"notification.sent", "notification.failed"})
        ...
        notification_subscriber.register_handler("notification.sent", notification_handler)
        notification_subscriber.register_handler("notification.failed", notification_handler)
        ...
        events = [
            ...
            BaseEvent(event_type="notification.sent", payload={"notification_id": "n1"}),
            BaseEvent(event_type="user.updated", payload={"user_id": "u1"}),
            BaseEvent(event_type="order.shipped", payload={"order_id": "o1"}),
            BaseEvent(event_type="notification.failed", payload={"notification_id": "n2"}),
            BaseEvent(event_type="unknown.event", payload={"data": "ignored"}),  # Should be ignored
        ]
        ...
        await async_timeout(
            self._wait_for_total_events(
                [user_capture, order_capture, notification_capture],
                expected_total=6
            ),
            5.0
        )
        ...
        assert len(notification_capture["events"]) == 2  # notification.sent, notification.failed
        ...
        notification_types = {event.event_type for event in notification_capture["events"]}
    ...
    async def test_handler_failure_isolation_multiple_subscribers(
        ...
        async def reliable_handler(event: BaseEvent):
            ...
            reliable_capture["handler_calls"]["reliable"] = reliable_capture["handler_calls"].get("reliable", 0) + 1
        ...
        async def unreliable_handler(event: BaseEvent):
            unreliable_capture["handler_calls"]["unreliable"] = unreliable_capture["handler_calls"].get("unreliable", 0) + 1
        ...
        events = [
            BaseEvent(event_type="isolation.test", payload={"id": 1, "should_fail": False}),
            BaseEvent(event_type="isolation.test", payload={"id": 2, "should_fail": True}),
            BaseEvent(event_type="isolation.test", payload={"id": 3, "should_fail": False}),
            BaseEvent(event_type="isolation.test", payload={"id": 4, "should_fail": True}),
            BaseEvent(event_type="isolation.test", payload={"id": 5, "should_fail": False}),
        ]
        ...
        logger.info(f"Unreliable handler: {successful_unreliable} successful, {failed_unreliable} failed")
        
        # Both subscribers should have been active despite failures
        total_processed = len(reliable_capture["events"]) + len(unreliable_capture["events"])
        assert total_processed >= 3, "Despite failures, some events should have been processed"
    ...
    async def test_concurrent_handler_execution(
        ...
        num_events = 10
        processing_times = {"handler_slow": [], "handler_fast": [], "handler_medium": []}
        ...
        async def create_timed_handler(name: str, delay: float):
            async def handler(event: BaseEvent):
                ...
                logger.info(f"Handler {name} processed {event.event_id} in {end_time - start_time:.3f}s")
        ...
        for i in range(num_events):
            event = BaseEvent(
                event_type="concurrent.processing",
                payload={"sequence": i}
            )
            success = await publisher.publish(
                event=event,
        ...
        await async_timeout(
            self._wait_for_handler_processing(processing_times, expected_events=num_events),
            15.0
        )
        ...
        logger.info(f"Total processing time: {total_time:.3f}s, Sequential estimate: {sequential_time_estimate:.3f}s")
        ...
        avg_slow = sum(processing_times["handler_slow"]) / len(processing_times["handler_slow"])
        avg_fast = sum(processing_times["handler_fast"]) / len(processing_times["handler_fast"])
        avg_medium = sum(processing_times["handler_medium"]) / len(processing_times["handler_medium"])
        
        assert avg_fast < avg_medium < avg_slow, "Handler timing should reflect their delays"
    ...
    async def _wait_for_handler_processing(self, processing_times: Dict, expected_events: int):
