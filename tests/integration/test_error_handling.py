...
class TestErrorHandling:
    ...
    async def test_handler_failure_no_ack(
        ...
        failing_handler = event_capture.create_handler("failing_handler", should_fail=True)
    ...
    async def test_partial_handler_failure_with_multiple_handlers(
        ...
        # Create test event
        test_event = BaseEvent(
            event_type="partial.failure",
            payload={"data": "test"}
        )
        
        # Register multiple handlers - some that succeed, some that fail
        success_handler = event_capture.create_handler("success_handler", should_fail=False)
        failing_handler = event_capture.create_handler("failing_handler", should_fail=True)
        another_success_handler = event_capture.create_handler("another_success", should_fail=False)
        ...
        assert event_capture.handler_calls.get("failing_handler", 0) == 1
        assert event_capture.handler_calls.get("another_success", 0) == 0  # Stopped after first failure
        
        # Verify one error occurred
        assert len(event_capture.errors) == 1
        
        # In current implementation, successful handlers execute but message isn't acked due to failure
        assert len(event_capture.events) == 1  # Only success_handler completed before failure
    ...
    async def test_publisher_retry_on_connection_failure(self, connection_manager):
        ...
        test_event = BaseEvent(
            event_type="connection.failure.test",
            payload={"retry": "test"}
        )
    ...
    async def test_event_store_failure_resilience(self, postgres_url):
        ...
        invalid_store = PostgreSQLEventStore("postgresql://invalid:invalid@localhost:9999/invalid")
        
        test_event = BaseEvent(
            event_type="store.failure.test",
            payload={"data": "test"}
        )
    ...
    async def test_event_store_connection_recovery(self, event_store, postgres_url):
        ...
        recovery_events = [
            BaseEvent(event_type="recovery.test", payload={"phase": "recovery", "id": i})
            for i in range(2)
        ]
    ...
    async def test_concurrent_error_scenarios(
        ...
        def create_conditional_handler():
            async def handler(event: BaseEvent):
                ...
                event_capture.handler_calls["conditional"] = event_capture.handler_calls.get("conditional", 0) + 1
    ...
    async def test_dead_letter_queue_simulation(
        ...
        test_events = [
            BaseEvent(
                event_type="dlq.test",
                payload={"sequence": i, "data": f"dlq_test_{i}"}
            )
            for i in range(5)
        ]
    ...
    async def test_exception_propagation_and_logging(
        ...
        def create_error_handler():
            async def handler(event: BaseEvent):
                ...
                event_capture.handler_calls["error_handler"] = event_capture.handler_calls.get("error_handler", 0) + 1
