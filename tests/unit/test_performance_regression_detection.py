...
from services.common.fine_tuning_pipeline import PerformanceMonitor, MLflowExperimentTracker
from services.common.kubernetes_fine_tuning_optimization import KubernetesOptimizedPipeline, KubernetesResourceConfig
...
class TestPerformanceMonitor:
    ...
    def test_operation_timing(self, performance_monitor):
        ...
        assert performance_monitor.metrics["test_operation_duration_seconds"] == duration
    ...
    def test_multiple_operation_timing(self, performance_monitor):
        ...
        for i, operation in enumerate(operations):
            ...
            assert performance_monitor.metrics[f"{operation}_duration_seconds"] == durations[i]
    ...
    def test_prometheus_metrics_generation(self, performance_monitor):
        ...
        performance_monitor.record_metric("operation_name", "test_operation")  # String metric
...
class TestPerformanceRegressionDetection:
    ...
    class PerformanceRegressionDetector:
        ...
        def detect_regression(self, metric_name: str, current_value: float) -> Dict[str, Any]:
            """Detect if current value represents a performance regression."""
            if metric_name not in self.baselines or len(self.baselines[metric_name]) < 3:
    ...
    def test_performance_regression_detection(self, regression_detector):
        ...
        result = regression_detector.detect_regression("api_response_time", 0.85)  # 50%+ increase
...
class TestMLflowPerformanceTracking:
    ...
    def test_performance_metrics_integration(self, mlflow_tracker):
        ...
        with patch('services.common.fine_tuning_pipeline.mlflow.log_metrics') as mock_log_metrics:
...
class TestPerformanceThresholdValidation:
    ...
    def test_performance_threshold_violations(self, threshold_validator):
        ...
        assert violations_by_metric["response_time_seconds"]["type"] == "exceeds_maximum"
    ...
    def test_alert_generation(self, threshold_validator):
        ...
        assert "ALERT: response_time_seconds = 7.5 exceeds maximum threshold of 5.0" == max_alert
        assert "ALERT: cache_hit_rate = 0.65 below minimum threshold of 0.8" == min_alert
...
class TestAsyncPerformanceMonitoring:
    ...
    async def test_performance_monitoring_under_load(self):
        ...
        async def high_frequency_operation(operation_id: int):
            ...
            monitor.record_metric(f"operation_{operation_id}_memory", operation_id * 1.5)
            return duration
...
class TestPerformanceOptimizationValidation:
    ...
    async def test_database_chunking_performance_improvement(self):
        ...
        performance_improvement = (non_chunked_duration - chunked_duration) / non_chunked_duration
    ...
    async def test_connection_pooling_performance_benefit(self):
        ...
        async def perform_operations(manager: MockConnectionManager, operation_count: int):
            monitor = PerformanceMonitor()
        ...
        non_pooled_duration = await perform_operations(non_pooled_manager, operation_count)
        ...
        performance_improvement = (non_pooled_duration - pooled_duration) / non_pooled_duration
