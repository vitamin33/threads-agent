...
class EvaluationRunner:
    ...
    def run_suite(self) -> SuiteResult:
        ...
        for test_case in self.suite_config['test_cases']:
            ...
            print(f"  {status} {result.test_id}: {result.score:.2f} ({result.latency_ms:.0f}ms)")
        ...
        gate_status = self._evaluate_gates(success_rate, avg_latency, total_cost, weighted_score)
    ...
    def _score_test_result(self, test_case: Dict[str, Any], result: Dict[str, Any]) -> float:
    ...
    def _evaluate_gates(self, success_rate: float, avg_latency: float, 
                       total_cost: float, weighted_score: float) -> str:
...
def main():
    ...
    parser.add_argument("--exit-code", action="store_true", 
                       help="Exit with non-zero code if gates fail")
    ...
    try:
        ...
        if args.output:
            runner = EvaluationRunner(DEV_SYSTEM_ROOT / "evals" / "suites" / f"{args.suite}.yaml")
