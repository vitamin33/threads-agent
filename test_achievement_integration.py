...
async def test_integration():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(
                f"{achievement_collector_url}/achievements/", json=achievement_data
            )
        ...
        print(
            "   Make sure to port-forward: kubectl port-forward svc/achievement-collector 8090:8090"
        )
        return
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                for article in articles:
                    ...
                    print(
                        f"      Insight Score: {article['insight_score']['overall_score']}"
                    )
        ...
        async with client:
            ...
            # Test by category
            automation_achievements = await client.get_by_category(
                "automation", limit=5
            )
            ...
            if response.status_code == 200:
                ...
                if achievements:
                    ...
                    for ach in sorted_achievements:
                        print(
                            f"   - {ach['title']} (Score: {ach.get('impact_score', 0)})"
                        )
