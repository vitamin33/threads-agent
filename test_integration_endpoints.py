    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            response = await client.post(
                ...
                params={"days": 30, "min_impact_score": 70, "limit": 5},
            )
        ...
        try:
            ...
            list_response = await client.get(
                f"{base_url}/achievements/", params={"limit": 5}
            )
        ...
        try:
            for company in ["notion", "anthropic", "jasper"]:
                ...
                if response.status_code == 200:
                    ...
                    if targeted:
                        print(
                            f"   Top match: {targeted[0]['title']} (Score: {targeted[0]['impact_score']})"
                        )
        ...
        try:
            response = await client.get(
                f"{base_url}/tech-doc-integration/stats/content-opportunities"
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   High-impact opportunities: {stats['high_impact_opportunities']}"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for a in filtered[:3]:
                    print(
                        f"   - {a['title']} ({a['category']}, Score: {a['impact_score']})"
                    )
        ...
        try:
            response = await client.get(
                f"{base_url}/tech-doc-integration/content-ready", params={"limit": 10}
            )
