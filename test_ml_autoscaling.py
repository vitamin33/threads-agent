        ...
        while time.time() - start_time < duration_seconds:
            try:
                ...
                if request_count % 100 == 0:
                    ...
                    print(f"  📊 Sent {request_count} requests, {errors} errors, {actual_rps:.1f} RPS")
...
def generate_rabbitmq_load(queue_name: str = "celery", message_count: int = 100):
    ...
    print(f"🐰 Generating RabbitMQ load: {message_count} messages to queue '{queue_name}'")
    ...
    try:
        ...
        print("   Make sure to port-forward: kubectl port-forward svc/rabbitmq 30672:5672")
...
async def monitor_scaling(duration_seconds: int = 120):
    ...
    while time.time() - start_time < duration_seconds:
        try:
            ...
            print("   Run: kubectl get events --sort-by='.lastTimestamp' | grep -i scale")
...
async def test_predictive_scaling():
    ...
    print(f"   - Forecast next 30 minutes based on pattern")
...
def main():
    ...
    print("\n📝 Test 2: HTTP Load-based Scaling")
    print("-" * 40)
    asyncio.run(generate_http_load(
