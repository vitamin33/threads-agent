...
class MLAutoscalingDemo:
    ...
    async def setup_environment(self):
        ...
        if not success:
            ...
            print(
                f"{Colors.WARNING}Please ensure k3d cluster is running: just dev-start{Colors.ENDC}"
            )
            return False
        ...
        success, output = run_command(
            "kubectl get deploy -n keda keda-operator", silent=True
        )
        if not success:
        ...
        if not success or "ml-autoscaling" not in output:
            print(
                f"{Colors.WARNING}⚠️  ML Autoscaling not deployed. Deploying...{Colors.ENDC}"
            )
    ...
    async def demonstrate_predictive_scaling(self):
        ...
        patterns = [
            ...
            (
                "Anomaly Detection",
                "92% accuracy",
                "Black Friday traffic spike detected",
            ),
        ]
        ...
        actions = [
            (
                ...
                "1 → 4 replicas",
                "Prevents morning latency spike",
            ),
    ...
    def show_performance_metrics(self):
        ...
        print_kpi(
            "Scale-to-Zero Success", "100%", "All GPU workloads support idle shutdown"
        )
    ...
    async def simulate_live_scaling(self):
        ...
        print(
            f"{Colors.CYAN}Monitoring scaling events for 30 seconds...{Colors.ENDC}\n"
        )
    ...
    def generate_report(self):
        ...
        print("  Project: MLOPS-008 - ML Infrastructure Auto-scaling with KEDA")
        ...
        print("  Initial Investment: ~40 hours development")
        print("  Monthly Savings: $11,280")
        print(f"  Payback Period: {Colors.GREEN}2 days{Colors.ENDC}")
        print(f"  Annual ROI: {Colors.GREEN}47x{Colors.ENDC}")

        # Save report to file
        report_file = (
            f"ml_autoscaling_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        report_data = {
            "project": "MLOPS-008",
...
async def main():
    ...
    try:
        ...
        # Live simulation
        print(
            f"\n{Colors.CYAN}Would you like to see a live scaling simulation? (y/n): {Colors.ENDC}",
            end="",
        )
...
if __name__ == "__main__":
    ...
    if sys.version_info < (3, 10):
        print(
            f"{Colors.FAIL}Python 3.10+ required. Current: {sys.version}{Colors.ENDC}"
        )
        sys.exit(1)
